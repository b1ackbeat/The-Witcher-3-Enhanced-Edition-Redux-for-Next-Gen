...
import class CScriptSoundSystem extends CObject
{
	...
	function CalculateThreat()
	{
		...
		var npc : CNewNPC;
		var actors : array<CActor>;
		var threatLevels : array<int>;
		var i, tempThreat, maxThreat, finalSoundValue : int;
		...
		for(i=0; i<actors.Size(); i+=1)
		{
			...
			{
				if( maxThreat > 0 )
					threatLevels.PushBack(maxThreat);
				maxThreat = tempThreat;
			}
			else
				threatLevels.PushBack(tempThreat);
			
			if( npc.useSoundValue == true )
			{
				finalSoundValue += npc.GetSoundValue();
				LogSound("finalSoundValue is " + finalSoundValue); 
			}
		...
		desiredThreatRating = maxThreat;
		for(i=0; i<threatLevels.Size(); i+=1)
		{
			switch(threatLevels[i])
			{
				case 10:	desiredThreatRating *= 1.10f;	break;
				case 50:	desiredThreatRating *= 1.25f;	break;
				default:	break;
			}
			
			if( desiredThreatRating >= 100 )
			{
				desiredThreatRating = 100;
				break;
			}
		}
		desiredThreatRating += finalSoundValue;
	...
	function UpdateThreatDamp()
	{
		...
		//Kolaris - 2H Sword Stance, Plasticmetal - AnimTweak
        if(!thePlayer.IsCiri() && !thePlayer.IsHoldingItemInLHand() && ( Options().GetUse2HSwordStance() || currentThreatRating >= 95.0 || GetWitcherPlayer().GetStatPercents(BCS_Vitality) <= 0.3f) )
