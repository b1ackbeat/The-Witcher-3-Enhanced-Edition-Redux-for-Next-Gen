...
function ModifyHitSeverityReaction(target : CActor, type : EHitReactionType) : EHitReactionType
{
	...
	return ModifySeverity( type, severityReduction );
}
...
function ModifyHitSeverityReactionFromDamage( out damageData : W3DamageAction )
{
	...
	if( attackAction && ( attackAction.IsParried() || attackAction.IsCountered() ) )
	{
	...
	damage = damageData.GetDamageDealt();
	...
	type = ModifySeverity(damageData.GetHitReactionType(), severityReduction);
	if(type == EHRT_None)
	{
		damageData.SetCanPlayHitParticle(false);
	}
...
function MonsterCriticalDamageBonusToCategory(type : name) : EMonsterCategory
{
	switch(type)
	{
		case 'vsBeast_critical_hit_damage_bonus' :				return MC_Beast;
		case 'vsCursed_critical_hit_damage_bonus' :				return MC_Cursed;
		case 'vsDraconide_critical_hit_damage_bonus' :			return MC_Draconide;
		case 'vsHuman_critical_hit_damage_bonus' :				return MC_Human;
		case 'vsHybrid_critical_hit_damage_bonus' :				return MC_Hybrid;
		case 'vsInsectoid_critical_hit_damage_bonus' :			return MC_Insectoid;
		case 'vsMagicals_critical_hit_damage_bonus' :			return MC_Magicals;
		case 'vsNecrophage_critical_hit_damage_bonus' :			return MC_Necrophage;
		case 'vsRelic_critical_hit_damage_bonus' :				return MC_Relic;
		case 'vsSpecter_critical_hit_damage_bonus' :			return MC_Specter;
		case 'vsTroll_critical_hit_damage_bonus' :				return MC_Troll;
		case 'vsVampire_critical_hit_damage_bonus' :			return MC_Vampire;
		
		default :				return MC_NotSet;
	}
}

function MonsterResistReductionToCategory(type : name) : EMonsterCategory
{
	switch(type)
	{
		case 'vsBeast_resist_reduction' :				return MC_Beast;
		case 'vsCursed_resist_reduction' :				return MC_Cursed;
		case 'vsDraconide_resist_reduction' :			return MC_Draconide;
		case 'vsHuman_resist_reduction' :				return MC_Human;
		case 'vsHybrid_resist_reduction' :				return MC_Hybrid;
		case 'vsInsectoid_resist_reduction' :			return MC_Insectoid;
		case 'vsMagicals_resist_reduction' :			return MC_Magicals;
		case 'vsNecrophage_resist_reduction' :			return MC_Necrophage;
		case 'vsRelic_resist_reduction' :				return MC_Relic;
		case 'vsSpecter_resist_reduction' :				return MC_Specter;
		case 'vsTroll_resist_reduction' :				return MC_Troll;
		case 'vsVampire_resist_reduction' :				return MC_Vampire;
...
enum EStaminaActionType
{
	...
	ESAT_HeavySpecial,
	// W3EE - Begin
	ESAT_CounterattackKick,
	ESAT_CounterattackBash,
	ESAT_SpecialAttackLight,
	ESAT_SpecialAttackHeavy
	// W3EE - End
}
