...
function activateJournalBestiaryEntryWithAlias(entryAlias:string, journalManager:CWitcherJournalManager):void
{
	...
	if ( resource )
	{
		...
		if ( entryBase )
		{
			//Kolaris - Hide Initial Bestiary Updates
			if( FactsQuerySum("SWMIntroComplete") > 0 )
				journalManager.ActivateEntry( entryBase, JS_Active );
			else
				journalManager.ActivateEntry( entryBase, JS_Active, false );
					...
					for ( j = 0; j < childEntries.Size(); j += 1 )
					{
						...
						if ( descriptionEntry )
						{
							...
							if( FactsQuerySum("SWMIntroComplete") > 0 )
								...
								journalManager.ActivateEntry( descriptionEntry, JS_Active, false );
						}
					}
					return;
				}
