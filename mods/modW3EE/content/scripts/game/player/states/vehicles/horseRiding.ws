...
state HorseRiding in CR4Player extends UseGenericVehicle
{
	...
	{	
		...
		if(Options().EnableImmersiveCam())
		{
		if( theInput.IsActionPressed( 'CamOffsetIncrease' ) ) 	parent.IncreaseHBOffset();
		if( theInput.IsActionPressed( 'CamOffsetDecrease' ) ) 	parent.DecreaseHBOffset();
		if( theInput.IsActionPressed( 'CamDepthIncrease' ) ) 	parent.IncreaseHBDepth();
		if( theInput.IsActionPressed( 'CamDepthDecrease' ) )	parent.DecreaseHBDepth();
		if( theInput.IsActionPressed( 'CamHeightIncrease' ) )	parent.IncreaseHBHeight();
		if( theInput.IsActionPressed( 'CamHeightDecrease' ) )	parent.DecreaseHBHeight();
		DampVectorSpring( moveData.cameraLocalSpaceOffset, moveData.cameraLocalSpaceOffsetVel, Vector(parent.ic.hbOffset + parent.icHBOffsetAdj, parent.ic.hbDepth + parent.icHBDepthAdj, parent.ic.hbHeight + parent.icHBHeightAdj), 0.6f, dt );
		}
		// ImmersiveCam--
	...
	event OnGameCameraPostTick( out moveData : SCameraMovementData, dt : float )
	{
		...
		if(Options().EnableImmersiveCam())
		{
			camera.SetAllowAutoRotation( thePlayer.GetAutoCameraCenter() || parent.IsCameraLockedToTarget() );
			DampFloatSpring( camera.fov, fovVel, parent.ic.hbFOV, 1.0, dt );
		}
		...
		else
		{
			if ( vehicleCombatMgr.IsInSwordAttackCombatAction() )
			{
				moveData.pivotDistanceController.SetDesiredDistance( parent.ic.hbCombatDis );
				currDesiredDist = parent.ic.hbCombatDis;
			}
			else if ( !horseComp.inCanter && !horseComp.inGallop && !horseComp.OnCheckHorseJump() )
			{
				moveData.pivotDistanceController.SetDesiredDistance( parent.ic.hbDistance );
				currDesiredDist = parent.ic.hbDistance;
			...
			{
				moveData.pivotDistanceController.SetDesiredDistance( parent.ic.hbGallopDis );
				currDesiredDist = parent.ic.hbGallopDis;
			}
			else if ( horseComp.inGallop && !horseComp.OnCheckHorseJump() )
			{
				moveData.pivotDistanceController.SetDesiredDistance( parent.ic.hbCanterDis );
				currDesiredDist = parent.ic.hbCanterDis;
			}
		}
