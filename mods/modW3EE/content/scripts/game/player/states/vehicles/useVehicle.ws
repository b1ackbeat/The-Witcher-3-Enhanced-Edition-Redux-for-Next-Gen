...
state UseGenericVehicle in CR4Player extends UseVehicle
{
	...
	event OnEnterState( prevStateName : name )
	{
		...
		else
		{
			...
			// -= WMK:modQuickSlots =-
			signSlotNames.PushBack( 'Slot1' ); // why this is missing?
			signSlotNames.PushBack( 'Crossbow' );
			signSlotNames.PushBack( 'Slot6' );
			// -= WMK:modQuickSlots =-
			...
			*/
			thePlayer.UnblockAction(EIAB_UsableItem, 'UsableItem');
			// W3EE - End
			...
			if ( (W3HorseComponent)vehicle )
			{
				...
				signSlotNames.PushBack( 'Slot4' );
				signSlotNames.PushBack( 'Slot3' ); // -= WMK:modQuickSlots =-
			...
			else if ( (CBoatComponent)vehicle )
			{
				...
				signSlotNames.PushBack( 'Slot3' ); // -= WMK:modQuickSlots =-
				signSlotNames.PushBack( 'Slot5' );
				signSlotNames.PushBack( 'Slot6' ); // -= WMK:modQuickSlots =-
				signSlotNames.PushBack('Yrden');
				signSlotNames.PushBack('Quen');
				signSlotNames.PushBack('Igni');
				signSlotNames.PushBack('Aard');
...
statemachine class W3VehicleCombatManager extends CEntity
{
	...
	public function Setup( player : CR4Player, _vehicle : CVehicleComponent )
	{
		...
		if( rider )
		{
			...
			theInput.RegisterListener( this, 'OnCastSign', 'SelectAxiiHorse' );
		}
	}
...
state Null in W3VehicleCombatManager
{
	...
	event OnEnterState( prevStateName : name )
	{
		...
		// W3EE - Begin
		rider.UnblockAction(EIAB_UsableItem, 'UsableItem');
		// W3EE - End
	...
	function StartItemAction()
	{
		...
		// W3EE - Begin
		else thePlayer.OnUseSelectedItem(true);
		// W3EE - End
	}
...
state ThrowProjectile in W3VehicleCombatManager extends RangedAttack 
{
	...
	event OnEnterState( prevStateName : name )
	{
		...
		// -= WMK:modQuickSlots =-
		rider.radialSlots.PushBack( 'Slot1' ); // why this is missing?
		rider.radialSlots.PushBack( 'Crossbow' );
		rider.radialSlots.PushBack( 'Slot6' );
		// -= WMK:modQuickSlots =-		
