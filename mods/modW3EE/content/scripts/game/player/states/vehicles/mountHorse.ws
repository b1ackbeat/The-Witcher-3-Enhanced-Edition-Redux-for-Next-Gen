...
state MountHorse in CR4Player extends MountTheVehicle
{
	...
	event OnGameCameraTick( out moveData : SCameraMovementData, dt : float )
	{
		...
		{
			DampVectorSpring( moveData.cameraLocalSpaceOffset, moveData.cameraLocalSpaceOffsetVel, Vector( thePlayer.ic.hbOffset, thePlayer.ic.hbDepth, thePlayer.ic.hbHeight), 0.6f, dt );
			return false;
		}
	}
	...
	event OnGameCameraPostTick( out moveData : SCameraMovementData, dt : float )
	{
		if(Options().EnableImmersiveCam())
		{
			...
			moveData.pivotDistanceController.SetDesiredDistance( thePlayer.ic.hbDistance );
