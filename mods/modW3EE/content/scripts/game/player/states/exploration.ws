...
state Exploration in CR4Player extends ExtendedMovable
{
	...
	event OnGameCameraTick( out moveData : SCameraMovementData, dt : float )
	{
		...
		{
			case PEA_Meditation 	: if(Options().EnableImmersiveCam())UpdateCameraMeditation( moveData, dt ); break;	
			case PEA_ExamineGround 	: if(Options().EnableImmersiveCam())UpdateCameraClueGround( moveData, dt ); doHeadTracking = false; break;
			...
			default:
			{
				...
				else if ( !thePlayer.interiorCamera && parent.IsInShallowWater() && !Options().EnableImmersiveCam())
				{
					return false;
				}
				else if( (parent.movementLockType == PMLT_NoSprint || parent.movementLockType == PMLT_NoRun) && !parent.GetExplCamera() && !Options().EnableImmersiveCam()) 
				{
