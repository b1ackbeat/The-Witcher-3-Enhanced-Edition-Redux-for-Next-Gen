...
state Combat in CR4Player extends ExtendedMovable 
{
	...
	entry function CombatInit()
	{
		...
		//Kolaris - Aeltoth Suggestion, Kolaris - Input Delay
		parent.AddTimer( 'CombatLoop', 0.f, true );
	...
	{	
		...
		if( thePlayer.IsFistFightMinigameEnabled() )
		{
			...
			if(parent.GetCmbtCamera() && !Options().EnableImmersiveCam())
			{
	...
	event OnPreAttackEvent(animEventName : name, animEventType : EAnimationEventType, data : CPreAttackEventData, animInfo : SAnimationEventAnimInfo )
	{
		...
		//Kolaris - Reflection
		if( (W3PlayerWitcher)parent && thePlayer.IsCounterAttack(data.attackName) && (parent.HasAbility( 'Runeword 25 _Stats', true ) || parent.HasAbility( 'Runeword 26 _Stats', true ) || parent.HasAbility( 'Runeword 27 _Stats', true ) ) )
		{
			data.rangeName = 'slash_long';		
		...
		else if( (W3PlayerWitcher)parent && parent.HasAbility( 'Runeword 39 _Stats', true ) )
		{
			if( thePlayer.IsHeavyAttack(data.attackName) )
			{
				weaponEntity = thePlayer.inv.GetItemEntityUnsafe(thePlayer.inv.GetItemFromSlot(data.weaponSlot));
			...
			else if( thePlayer.IsLightAttack(data.attackName) )
			{	
				weaponEntity = thePlayer.inv.GetItemEntityUnsafe(thePlayer.inv.GetItemFromSlot(data.weaponSlot));
				weaponEntity.PlayEffectSingle('light_trail_extended_fx');
				if( data.attackName == 'attack_light_special' )
		...
		else if( (parent.HasAbility( 'Runeword 7 _Stats', true ) || parent.HasAbility( 'Runeword 8 _Stats', true ) || parent.HasAbility( 'Runeword 9 _Stats', true )) && (W3PlayerWitcher)parent )
		{
			weaponEntity = thePlayer.inv.GetItemEntityUnsafe(thePlayer.inv.GetItemFromSlot(data.weaponSlot));
			weaponEntity.PlayEffectSingle('runeword1_fire_trail');
		}
		else if((parent.HasAbility( 'Runeword 40 _Stats', true ) || parent.HasAbility( 'Runeword 41 _Stats', true ) || parent.HasAbility( 'Runeword 42 _Stats', true )) && (W3PlayerWitcher)parent && GetWitcherPlayer().GetRunewordInfusionType() == ST_Igni)
		{
			weaponEntity = thePlayer.inv.GetItemEntityUnsafe(thePlayer.inv.GetItemFromSlot(data.weaponSlot));
			weaponEntity.PlayEffectSingle('runeword1_fire_trail');
		}
		...
		//Kolaris - Rend Hitbox
		if( data.attackName == 'attack_heavy_special' && (W3PlayerWitcher)parent && !parent.HasAbility( 'Runeword 39 _Stats', true ) )
			data.rangeName = 'thrust';
	...
	entry function ProcessAttack( playerAttackType : name, performApproachAttack : bool )
	{
		...
		{		
			...
			enableSoftLock = (Options().LockOn() != 0) || parent.IsHardLockEnabled();
	...
	function OnComboAttackCallback( out callbackInfo : SComboAttackCallbackInfo )
	{
		...
		else
		{
			...
			//W3EE - Begin
			// (Omnidirectional)
			if( Options().LockOnMode() && theInput.LastUsedGamepad() )
			{
		...
		if( Options().LockOn() == 1 )
			IAB_setAttackInfo(callbackInfo);
		...
		if( callbackInfo.inAspectName == 'AttackHeavy' || callbackInfo.inAspectName == 'AttackHeavyReal' || callbackInfo.inAspectName == 'AttackBattleAxeHeavy' )
		// W3EE - End
		...
		if( callbackInfo.outDistance == ADIST_Large && GetWitcherPlayer().IsSetBonusActive(EISB_Elven_2) )
		{
			Combat().SetElvenSetState(true);
			GetWitcherPlayer().AddTimer('DisableElvenSet', 3, false,,,,true);
		}
	...
	private function GetAssistHeading()
	{
		...
		if( !enableSoftLock && !parent.IsHardLockEnabled() )
		{
	...
	protected final function InteralCombatComboUpdate( timeDelta : float )
	{
		...
		// modStandaloneAttackBending - BEGIN
		if (Options().LockOn() == 1 && IAB_tryApplyingHeadingFromLeftStick(comboAttackA_Id, comboAttackA_Target, parent, comboPlayer, timeDelta))
			return;
		// modStandaloneAttackBending - END
