...
state Swimming in CR4Player extends ExtendedMovable
{
	...
	private function ToggleDiving( toggle : bool )
	{
		...
		if ( toggle && !diving )
		{
			...
			slotsToBlock.PushBack( 'Slot3' );
			// -= WMK:modQuickSlots =-
			// An workaround to help Script Merger to automatically solve a conflict with Immersive Cam v2.6.1 mod.
			slotsToBlock[slotsToBlock.Size() - 1] = 'Crossbow';
			// -= WMK:modQuickSlots =-
	...
	event OnGameCameraTick( out moveData : SCameraMovementData, dt : float )
	{
		...
		if(Options().EnableImmersiveCam())
			moveData.pivotRotationController.maxPitch = 15.0;
		if ( theGame.IsFocusModeActive() && !Options().EnableImmersiveCam())
		{	
			return false;
		}
	...
	event OnGameCameraPostTick( out moveData : SCameraMovementData, dt : float )
	{
		...
		if ( moveData.pivotPositionController.controllerName == 'Diving' )
		{
			...
			else
			{
				...
				moveData.pivotPositionController.offsetZ = 1.15f;
					...
					moveData.pivotPositionController.offsetZ = 0.75f;
			}
		}
		if ( theGame.IsFocusModeActive() && !Options().EnableImmersiveCam())
		{
		...
		else if ( moveData.pivotDistanceController.controllerName == 'Diving' )
		{
			...
			else if ( parent.rawPlayerSpeed == 0 && !Options().EnableImmersiveCam())
			{
				moveData.pivotDistanceController.SetDesiredDistance(2.25);
			}
			else if(!Options().EnableImmersiveCam())
			{
