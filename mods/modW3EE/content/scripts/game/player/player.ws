...
{		
	...
	const 				var DEATH_SCREEN_OPEN_DELAY		: float; 
		...
		//W3EE - Begin
		default DEATH_SCREEN_OPEN_DELAY = 2.f; 
		default fStartScreenFadeDuration = 0.5; 
		default fStartScreenFadeInDuration = 0.5; 
		//W3EE - End
	...
	// W3EE - Begin
	timer function ActivateDeathRagdoll( dt : float, id : int )
	{
		if( ((CMovingPhysicalAgentComponent)GetMovingAgentComponent()).HasRagdoll() )
			TurnOnRagdoll();
	}
	// W3EE - End
	...
	event OnDeath( damageAction : W3DamageAction  )
	{
		...
		if ( allowDeath )
		{
			...
			else
			{
				...
				AddTimer('ActivateDeathRagdoll', 0.5f, false);
				// W3EE - End
			}
	...
	{
		//Kolaris - Movement Efficiency, Kolaris - Footwork
		var attValue : SAbilityAttributeValue;
		attValue = GetAttributeValue('movement_stamina_efficiency');
		attValue.valueMultiplicative += 0.04f * ((W3PlayerWitcher)this).GetSkillLevel(S_Sword_s09);
		...
		if( !thePlayer.modifyPlayerSpeed )
		{
			...
			{
				DrainStamina(ESAT_Sprint, 0, 0, '', dt * (1.f - attValue.valueMultiplicative));
			}
		}
	}
	...
	protected function ShouldDrainStaminaWhileSprinting() : bool
	{
		...
		if ( Options().SprintStamUsage() == 0 )
		{
			if( currentStateName == 'Exploration' )
				if ( sprintingTime >= Options().SprintStamDelExpl() || ( GetStaminaPercents() < 1.0 && Options().SprintInstDrain() ) )
					return true;
			if( currentStateName == 'CombatSteel' || currentStateName == 'CombatSilver' || currentStateName == 'CombatFists' )
				//Kolaris - Fleet Footed
				if ( ( sprintingTime >= Options().SprintStamDelComb() || ( GetStaminaPercents() < 1.0 && Options().SprintInstDrain() ) ) && ( !((W3PlayerWitcher)this).CanUseSkill(S_Perk_13) || sprintingTime >= 2 ) )
					return true;
		}
		else if ( Options().SprintStamUsage() == 1 )
		{			
			if( currentStateName == 'CombatSteel' || currentStateName == 'CombatSilver' || currentStateName == 'CombatFists' )
				//Kolaris - Fleet Footed
				if ( ( sprintingTime >= Options().SprintStamDelComb() || ( GetStaminaPercents() < 1.0 && Options().SprintInstDrain() ) ) && ( !((W3PlayerWitcher)this).CanUseSkill(S_Perk_13) || sprintingTime >= 2 ) )
	...
	protected function ParryCounterCheck() : bool
	{
		...
		else if ( thePlayer.IsInCombatAction() && combatActionType == (int)CAT_Attack )
		{
			if ( thePlayer.IsThreatened() && !thePlayer.isInFinisher && !Options().AttackCommitment() )
			{
				return true;
			}
		}
