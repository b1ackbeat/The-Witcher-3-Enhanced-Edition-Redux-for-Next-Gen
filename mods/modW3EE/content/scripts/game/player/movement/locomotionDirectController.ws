	...
	function CalculateMoveSpeed() : float
	{
			...
			if ( speed > 0.0f )
			{
				if ( thePlayer.IsRunPressed() )
				{
					if( theInput.LastUsedGamepad() )
						...
						speed = thePlayer.GetInputHandler().GetMovementSpeed();
				}
				else
				{
					if( theInput.LastUsedGamepad() )
						...
						speed = ClampF( thePlayer.GetInputHandler().GetMovementSpeed(), 0.f, speedWalkingMax );
				}
			}
		}
		...
		{			
			...
			else if( thePlayer.IsSprintActionPressed() )
			{
				...
				else if ( speed > 0.8f )
				{
					...
					{
						speed = MinF( thePlayer.GetInputHandler().GetMovementSpeed(), speedRunning );
						player.playerMoveType = thePlayer.GetInputHandler().GetPlayerMoveTypeFromSpeed();
					}
				}
				else
				{
			...
			else
			{
				...
				{
					speed = MapF( MinF( thePlayer.GetInputHandler().GetMovementSpeed(), speedRunning ), 0.0f, speedRunning, 0.0f,  speedWalkingMax );
					player.playerMoveType = thePlayer.GetInputHandler().GetPlayerMoveTypeFromSpeed();
				}
			}			
		}
		...
		else
		{
			if ( theInput.LastUsedGamepad() )
			{
				...
				//Kolaris - NextGen Update, Kolaris - Gamepad Sprint Toggle
			...
			else if (!theGame.IsFading() && !theGame.IsBlackscreen())
			{
				...
				{	
					//Kolaris - Fleet Footed, Kolaris - New Moon Set, Kolaris - Movement Speed
					Combat().EvadeSpeedModule(, true);
					
					speed = speedSprintingWithPerk;
