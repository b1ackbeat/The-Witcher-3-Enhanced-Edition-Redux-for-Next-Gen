...
{ 
	...
	// ImmersiveCam++
	public 			var ic 						: icControl;
	private 		var slowMoActive			: bool;
	public			var motionSicknessFocusMode	: bool;
	public			var runningLanding			: float;
	public			var sprintSpeed				: float;
	
	//( interactive camera controls )
	private saved 	var icOffsetAdj 	: float;
	private saved 	var icDepthAdj 		: float;
	private saved 	var icHeightAdj 	: float;
	public saved 	var icHBDepthAdj 	: float;
	public saved 	var icHBOffsetAdj 	: float;
	public saved 	var icHBHeightAdj 	: float;
	private saved 	var icComOffsetAdj 	: float;
	private saved 	var icComDepthAdj 	: float;
	private saved 	var icComHeightAdj 	: float;	

	//(headtracking controls)
	private var lookTarget				: CActor;
	private var savedTarget				: CActor;
	private var nextlooktime			: float;
	private var nextchecktime			: float;
	private var potentiallookattargets 	: array<CActor>;

	// (camera control functions)
 	private function DecreaseOffset() 	{ icOffsetAdj -= 0.02; 		}
	private function IncreaseOffset() 	{ icOffsetAdj += 0.02; 		}
	private function DecreaseDepth() 	{ icDepthAdj -= 0.02; 		}
	private function IncreaseDepth()	{ icDepthAdj += 0.02; 		}
	private function DecreaseHeight() 	{ icHeightAdj -= 0.02; 		}
	private function IncreaseHeight() 	{ icHeightAdj += 0.02; 		}
	public function DecreaseHBOffset() 	{ icHBOffsetAdj -= 0.01; 	}
	public function IncreaseHBOffset() 	{ icHBOffsetAdj += 0.01; 	}
	public function DecreaseHBDepth() 	{ icHBDepthAdj -= 0.01; 	}
	public function IncreaseHBDepth() 	{ icHBDepthAdj += 0.01; 	}
	public function DecreaseHBHeight() 	{ icHBHeightAdj -= 0.01; 	}
	public function IncreaseHBHeight() 	{ icHBHeightAdj += 0.01; 	}
	private function DecreaseComOffset() { icComOffsetAdj -= 0.05; 	}
	private function IncreaseComOffset() { icComOffsetAdj += 0.05; 	}
	private function DecreaseComDepth() { icComDepthAdj -= 0.05; 	}
	private function IncreaseComDepth()	{ icComDepthAdj += 0.05; 	}
	private function DecreaseComHeight() { icComHeightAdj -= 0.05; 	}
	private function IncreaseComHeight() { icComHeightAdj += 0.05; 	}
	
	public function ResetExpCam() 		{ icOffsetAdj = 0; icDepthAdj = 0; icHeightAdj = 0;}
	public function ResetHBCam() 		{ icHBOffsetAdj = 0; icHBDepthAdj = 0; icHBHeightAdj = 0;}
	public function ResetComCam()		{ icComOffsetAdj = 0; icComDepthAdj = 0; icComHeightAdj = 0;}
	
	//plasticmetal - Perfect Parry Tweaks, plasticmetal - Boss Bar Fix
	public var parryspeedid : int;
	public var storedBossTag : name;
	
	public function SetExpCam()
	{
		theGame.GetInGameConfigWrapper().SetVarValue( 'ImmersiveCamPositionsExploration', 'expOffset', FloatToString( ic.expOffset + icOffsetAdj ) ); 
		theGame.GetInGameConfigWrapper().SetVarValue( 'ImmersiveCamPositionsExploration', 'expDepth', FloatToString( ic.expDepth + icDepthAdj ) ); 
		theGame.GetInGameConfigWrapper().SetVarValue( 'ImmersiveCamPositionsExploration', 'expHeight', FloatToString( ic.expHeight + icHeightAdj ) );
		theGame.GetInGameConfigWrapper().SetVarValue( 'ImmersiveCamPositionsWS', 'eWSOffset', FloatToString( ic.eWSOffset + icOffsetAdj ) ); 
		theGame.GetInGameConfigWrapper().SetVarValue( 'ImmersiveCamPositionsWS', 'eWSDepth', FloatToString( ic.eWSDepth + icDepthAdj ) ); 
		theGame.GetInGameConfigWrapper().SetVarValue( 'ImmersiveCamPositionsWS', 'eWSHeight', FloatToString( ic.eWSHeight + icHeightAdj ) );
		theGame.GetGuiManager().GetInGameConfigBufferedWrapper().SetVarValue( 'ImmersiveCamPositionsExploration', 'expOffset', FloatToString( ic.expOffset + icOffsetAdj ) ); 
		theGame.GetGuiManager().GetInGameConfigBufferedWrapper().SetVarValue( 'ImmersiveCamPositionsExploration', 'expDepth', FloatToString( ic.expDepth + icDepthAdj ) ); 
		theGame.GetGuiManager().GetInGameConfigBufferedWrapper().SetVarValue( 'ImmersiveCamPositionsExploration', 'expHeight', FloatToString( ic.expHeight + icHeightAdj ) );
		theGame.GetGuiManager().GetInGameConfigBufferedWrapper().SetVarValue( 'ImmersiveCamPositionsWS', 'eWSOffset', FloatToString( ic.eWSOffset + icOffsetAdj ) ); 
		theGame.GetGuiManager().GetInGameConfigBufferedWrapper().SetVarValue( 'ImmersiveCamPositionsWS', 'eWSDepth', FloatToString( ic.eWSDepth + icDepthAdj ) ); 
		theGame.GetGuiManager().GetInGameConfigBufferedWrapper().SetVarValue( 'ImmersiveCamPositionsWS', 'eWSHeight', FloatToString( ic.eWSHeight + icHeightAdj ) );

		ResetExpCam();
		ic.SetImmCamVars();
		thePlayer.DisplayHudMessage("default exploration camera position set");
	}
	
	public function SetHBCam()
	{
		theGame.GetInGameConfigWrapper().SetVarValue( 'ImmersiveCamPositionsHorse', 'hbOffset', FloatToString( ic.hbOffset + icHBOffsetAdj ) );
		theGame.GetInGameConfigWrapper().SetVarValue( 'ImmersiveCamPositionsHorse', 'hbDepth', FloatToString( ic.hbDepth + icHBDepthAdj ) );	
		theGame.GetInGameConfigWrapper().SetVarValue( 'ImmersiveCamPositionsHorse', 'hbHeight', FloatToString( ic.hbHeight + icHBHeightAdj ) );
		theGame.GetGuiManager().GetInGameConfigBufferedWrapper().SetVarValue( 'ImmersiveCamPositionsHorse', 'hbOffset', FloatToString( ic.hbOffset + icHBOffsetAdj ) );
		theGame.GetGuiManager().GetInGameConfigBufferedWrapper().SetVarValue( 'ImmersiveCamPositionsHorse', 'hbDepth', FloatToString( ic.hbDepth + icHBDepthAdj ) );	
		theGame.GetGuiManager().GetInGameConfigBufferedWrapper().SetVarValue( 'ImmersiveCamPositionsHorse', 'hbHeight', FloatToString( ic.hbHeight + icHBHeightAdj ) );

		ResetHBCam();
		ic.SetImmCamVars();
		thePlayer.DisplayHudMessage("default horseback camera position set");
	}
	
	public function SetComCam()
	{
		theGame.GetInGameConfigWrapper().SetVarValue( 'ImmersiveCamPositionsCombat', 'comOffset', FloatToString( ic.comOffset + icComOffsetAdj ) ); 
		theGame.GetInGameConfigWrapper().SetVarValue( 'ImmersiveCamPositionsCombat', 'comDepth', FloatToString( ic.comDepth + icComDepthAdj ) ); 
		theGame.GetInGameConfigWrapper().SetVarValue( 'ImmersiveCamPositionsCombat', 'comHeight', FloatToString( ic.comHeight + icComHeightAdj ) );
		theGame.GetGuiManager().GetInGameConfigBufferedWrapper().SetVarValue( 'ImmersiveCamPositionsCombat', 'comOffset', FloatToString( ic.comOffset + icComOffsetAdj ) ); 
		theGame.GetGuiManager().GetInGameConfigBufferedWrapper().SetVarValue( 'ImmersiveCamPositionsCombat', 'comDepth', FloatToString( ic.comDepth + icComDepthAdj ) ); 
		theGame.GetGuiManager().GetInGameConfigBufferedWrapper().SetVarValue( 'ImmersiveCamPositionsCombat', 'comHeight', FloatToString( ic.comHeight + icComHeightAdj ) );

		ResetComCam();
		ic.SetImmCamVars();
		thePlayer.DisplayHudMessage("default combat camera position set");
	}
	
	timer function SetImmCamVars(dt : float, id : int)
	{
		ic.SetImmCamVars();
	}
	
	public function storeBossTag(nm: name)
	{
		storedBossTag = nm;
	}
	
	timer function ShowBossFightIndicator(dt: float, id : int)
	{
		theGame.GetGuiManager().GetHudEventController().RunEvent_BossFocusModule_ShowBossIndicator( true, storedBossTag );
	}
	
	public function EnableMotionSicknessFocusMode( active : bool )
	{
		motionSicknessFocusMode = active;
	}
		
	// (headtracking functions)
	public function ProcessHeadTracking()
	{
		...
		if ( IsInInterior() )
			lookDistance = ic.intHTDis;
		else
			lookDistance = ic.extHTDis;
				
		if( curtime > nextchecktime )
		{
			potentiallookattargets = GetActorsInRange(thePlayer, lookDistance, , ,0);
			...
			if ( npccount )
			{
				if ( lookTarget != savedTarget )
				{
					...
					((CActor)thePlayer).EnableDynamicLookAt(lookTarget, 2.0);
					nextlooktime = theGame.GetEngineTimeAsSeconds() + 2.75;
					savedTarget = lookTarget;
				}
				else if( curtime >= nextlooktime) 
				{
					((CActor)thePlayer).DisableLookAt();
					((CActor)thePlayer).EnableDynamicLookAt(lookTarget, 10.0);
					nextlooktime = theGame.GetEngineTimeAsSeconds() + 12.0;
				}
			}
			else
				nextchecktime = theGame.GetEngineTimeAsSeconds() + 0.1; 
		}
				
	}
	
	public function DisableHeadTracking()
	{
		((CActor)thePlayer).DisableLookAt();
	}
	
	event OnStopTimeSlow(duration : float, id : Int32) 
	{
		theGame.RemoveTimeScale(theGame.GetTimescaleSource(7));
		slowMoActive = false;
	}
	
	// (immersive motion functions)
	private timer function DeactivateRunningLanding(dt: float, id: int)
	{
		runningLanding = 0;
	}
	
	public function EnableRunningLanding(rlSpeed : float, time : float)
	{
		if( runningLanding == 0)
		{
			runningLanding = rlSpeed;
			AddTimer('DeactivateRunningLanding', time, false);
		}
	}
	
	var slowStartID		: int;	default slowStartID = -1;
	var slowStartActive	: bool; 

	private timer function RemoveStartSlow( dt: float, id: int )
	{
		ResetAnimationSpeedMultiplier(slowStartID);
		slowStartID = SetAnimationSpeedMultiplier(0.9);
		AddTimer( 'RemoveStartSlow2', 1.0, false );
	}
	
	private timer function RemoveStartSlow2( dt: float, id: int )
	{
		ResetAnimationSpeedMultiplier(slowStartID);
	...
	public function StartSlow()
	{
		if( !slowStartActive )
		{
			...
			slowStartID = SetAnimationSpeedMultiplier(0.8);
			AddTimer( 'RemoveStartSlow', 1.0, false );
		}
	}
	
	public function SetSprintSpeed( speed : float )
	{
		sprintSpeed = speed;
	}
	// ImmersiveCam-- 
	...
	// W3EE - Begin
	timer function RefreshDamSett( dt : float, id : int )
	{	
		Options().FirstTimeOptionSetup();
		Damage().RefreshSettings();
		
		if( IsCiri() )
			theGame.GetHud().GetHudModule('WolfHeadModule').GetModuleFlash().GetChildFlashSprite("mcSwordIcon").SetVisible(false);
	}
	// W3EE - End
	...
	event OnSpawned( spawnData : SEntitySpawnData )
	{
		...
		// W3EE - Begin
		AddTimer('RefreshDamSett', 1.f, false);
		UnblockAction(EIAB_OpenMeditation, 'ExplorationMeditation');
		// W3EE - End
	...
	// W3EE - Begin
	public final function ReduceAllOilsAmmo( id : SItemUniqueId, isHeavyAttack : bool )
	{
		var i : int;
	...
	public function ApplyOil( oilId : SItemUniqueId, usedOnItem : SItemUniqueId ) : bool
	{
		...
		var appliedOilTags : array<name>;
		var min, max : SAbilityAttributeValue;
		var i, maxOils : int;
		var oils : array< W3Effect_Oil >;
		var existingOil : W3Effect_Oil;
		var hasViperSet, hasEmulation : bool;
		/*
		...
		*/
		...
		Experience().AwardAlchemyUsageXP(inv.IsItemMutagenPotion(oilId), inv.IsItemPotion(oilId), inv.IsItemOil(oilId), inv.IsItemBomb(oilId));
		...
		inv.GetItemTags(oilId, oilTags);
		for(i=0; i<oils.Size(); i+=1)
		{
			dm.GetAbilityTags(oils[i].GetAbilityName(), appliedOilTags);
			if( appliedOilTags[appliedOilTags.Size()-1] == oilTags[oilTags.Size()-1] )
			{
				existingOil = oils[i];
				break;
			}
		...
		if( existingOil )
			RemoveEffect(existingOil);
		else
		{
			hasViperSet = GetWitcherPlayer().IsSetBonusActive(EISB_Viper1);
			hasEmulation = CanUseSkill(S_Alchemy_s07);
			
			if( !GetWitcherPlayer() ||( !hasViperSet  && !hasEmulation ))
			{
			...
			{
				maxOils = 1;
				if(hasViperSet) maxOils += 1;
				if(hasEmulation) maxOils += 1;
				if( inv.GetActiveOilsAppliedOnItemCount( usedOnItem ) >= maxOils )
				{
		...
		if( Options().GetGlobalOilDuration() )
			ammo = Options().GetGlobalOilDuration();
		else
		if( Options().GetMinimumOilDuration() )
			ammo += Options().GetMinimumOilDuration();
			
		ammo *= Options().GetOilDurationMult();
		if( ((W3PlayerWitcher)this).IsMeditating() )
			ammo *= 1.2f;
		
		//Kolaris - Fixative
		if( CanUseSkill(S_Alchemy_s05) )
		{
			ammoBonus = 0.2f;
			ammo *= 1 + ammoBonus * GetSkillLevel(S_Alchemy_s05);
		}
		...
		inv.SingletonItemRemoveAmmo(oilId, 1);
		if( !inv.GetItemModifierInt(oilId, 'ammo_current') )
			inv.RemoveItem(oilId, 1);
		((CR4InventoryMenu)((CR4MenuBase)theGame.GetGuiManager().GetRootMenu() ).GetSubMenu()).updateCurrentTab(); 
	...
	//Kolaris - Poise Tweaks, Kolaris - Reflection
	public function CheckCounterSpamming(attacker : CActor, optional counterAttempt: bool) : bool
	{
		var counterWindowStartTime : EngineTime;		
		var i, spamCounter : int;
		var reflexAction : bool;
		var testEngineTime : EngineTime;
		var runeword27Time : EngineTime;
		var runeword27Check : bool;
		var playerPoise : W3Effect_Poise;
		var poisePercents : float;
		var weightedWindow : EngineTime;
		...
		playerPoise = (W3Effect_Poise)GetBuff(EET_Poise);
		poisePercents = playerPoise.GetPoisePercentage();
		if( counterAttempt && !ShouldProcessTutorial('TutorialCounter'))
		{
			weightedWindow = counterWindowStartTime + (theGame.GetEngineTime() - counterWindowStartTime) * (0.9f - poisePercents * 0.4f);
			//theGame.GetGuiManager().ShowNotification("Counter Window: " + EngineTimeToFloat((theGame.GetEngineTime() - weightedWindow)) + " seconds");
		}
		else
		{
			weightedWindow = counterWindowStartTime + (theGame.GetEngineTime() - counterWindowStartTime) * (0.8f - poisePercents * 0.8f);
			//theGame.GetGuiManager().ShowNotification("Parry Window: " + EngineTimeToFloat((theGame.GetEngineTime() - weightedWindow)) + " seconds");
		}
		
		runeword27Time = (weightedWindow + theGame.GetEngineTime()) / 2.f;
		runeword27Check = false;
		...
		for(i = counterTimestamps.Size() - 1; i>=0; i-=1)
		{
			...
			if(counterTimestamps[i] >= runeword27Time)
				runeword27Check = true;
			
			if(!reflexAction && (counterTimestamps[i] >= weightedWindow))
				reflexAction = true;
		}
		...
		{
			//Kolaris - Reflection
			if( ((W3PlayerWitcher)this).HasAbility('Runeword 27 _Stats', true) && runeword27Check )
			{
				theGame.GameplayFactsSet( "reflectionCounter", 1);
				PlayEffect('mutation_10_energy');
			}
			counterTimestamps.Clear();
			return true;
		}
	...
	protected function PerformCounterCheck(parryInfo: SParryInfo) : bool
	{
		...
		if( !HasStaminaToUseAction(ESAT_Counterattack, , mult) || VecDistance(GetWorldPosition(), parryInfo.attacker.GetWorldPosition()) >= 2.3f || parryInfo.attackActionName == 'stomp' || parryInfo.attackActionName == 'anchor_special_far' || parryInfo.attackActionName == 'achor_far' || parryInfo.attacker.HasTag('sq202_djinn') )
			return false;
			
		if( !(theInput.IsActionPressed('DistanceModifierMed') || (theInput.LastUsedGamepad() && theInput.GetActionValue('GI_AxisLeftY') >= Options().GetPadDistanceMedium())) && !(theInput.IsActionPressed('DistanceModifier') || (theInput.LastUsedGamepad() && theInput.GetActionValue('GI_AxisLeftY') >= Options().GetPadDistanceLong())) )
			return false;
			
		poiseEffect = (W3Effect_Poise)GetBuff(EET_Poise);
		poiseValue = poiseEffect.GetPoisePercentage();
		//W3EE - End
		
		fistFightCheck = FistFightCheck( parryInfo.target, parryInfo.attacker, fistFightCounter );
		//Kolaris - Injury Effects, Kolaris - Poise Tweaks
		if( !HasBuff(EET_Overexertion) && !GetInjuryManager().HasInjury(EFI_Chest) && !GetInjuryManager().HasInjury(EPI_Spine) )
		{
			if( ParryCounterCheck() && parryInfo.targetToAttackerAngleAbs < theGame.params.PARRY_HALF_ANGLE && fistFightCheck )
			{
				validCounter = CheckCounterSpamming(parryInfo.attacker, true);
				...
				if(validCounter)
				{
					...
					if ( !fistFightCounter )
					{
						...
						//W3EE - Begin
						Combat().PerformCounter( this, counterCollisionGroupNames, parryInfo, weaponTags, hitNormal, repelType, ragdollTarget, isMutation8, npc );
						//W3EE - End
	...
	// W3EE - Begin
	public function SetRagdollTarget( actor : CActor )
	{
		ragdollTarget = actor;
	}
	...
	event OnCombatStart()
	{
		...
		if( GetBehaviorVariable('playerDoubleHandSword') == 0 )
		{
			if( Options().GetUse2HSwordStance() || GetStatPercents(BCS_Vitality) <= 0.3f || ((W3PlayerWitcher)this).GetTotalActiveThreat() >= 95 )
				SetBehaviorVariable('playerDoubleHandSword',1.0f);
		}
	}
	...
	// W3EE - Begin
	function PlayerStartAnim( playerAction : EPlayerExplorationAction, animName : name, blendInValue, blendOutValue : float ) : bool
	{
		if( playerAction == PEA_SlotAnimation && !IsNameValid(animName) )
		{
			return false;
		...
		if( RaiseForceEvent('playerActionStart') )
		{
			...
			if( playerAction == PEA_SlotAnimation )
			{ 
				blendIn = blendInValue;
				blendOut = blendOutValue;
				playerActionSlotAnimName = animName;
				AddTimer('PlayActionAnimWorkaroundCustom', 0, false);
			}
			return true;
		}
		return false;
	...
	private var blendIn, blendOut : float;
	timer function PlayActionAnimWorkaroundCustom( dt : float , id : int)
	{
		this.ActionPlaySlotAnimationAsync('PLAYER_ACTION_SLOT',playerActionSlotAnimName, blendIn, blendOut, true);
	}
	// W3EE - End
	...
	event OnItemGiven(data : SItemChangedData)
	{
		...
		if(data.informGui && (MutSort().GetCurrentStateName()!='Sort'))//Plasticmetal - Mutagen Sorter
		{			
			hud = (CR4ScriptedHud)theGame.GetHud();
			if(hud)
			{
				//message = GetLocStringByKeyExt("panel_common_item_received") + ": " + inv.GetItemLocNameByID(data.ids[0]);
				message = GetLocStringByKeyExt("panel_common_item_received") + ": " +  GetInventory().AlchemyColor(data.ids[0]);//Plasticmetal - LootTweak
				if(data.quantity > 1)
					message += " x" + data.quantity;
				message += GetInventory().AlchemySecondary(data.ids[0]);//Plasticmetal - LootTweak
	...
	event OnHolsteredItem( category :  name, slotName : name )
	{
		...
		if ( slotName == 'r_weapon' && (category == 'steelsword' || category == 'silversword') )
		{
			...
			//Kolaris - Cat Set
			GetBuff( EET_SwordDancing ).Pause( 'drawing weapon' );
	...
	// W3EE - Begin
	public function GoToCombat( weaponType : EPlayerWeapon, optional initialAction : EInitialAction )
	// W3EE - End
	{			
	...
	event OnAnimEvent_SlideToTarget( animEventName : name, animEventType : EAnimationEventType, animInfo : SAnimationEventAnimInfo )
	{
		...
		//Kolaris - Slash Counter Slide
		distanceToTarget = VecDistance(GetWorldPosition(), slideNPC.GetWorldPosition(), );
		distanceToStay = ((CMovingPhysicalAgentComponent)this.GetMovingAgentComponent()).GetCapsuleRadius()+((CMovingPhysicalAgentComponent)slideNPC.GetMovingAgentComponent()).GetCapsuleRadius();
		
		if( (GetBehaviorVariable('repelType') != 4 || distanceToTarget >= 3) && (slideNPC.HasAbility('mon_ghoul_base') || slideNPC.HasAbility('mon_wolf_base') || slideNPC.HasAbility('mon_drowner_base') || slideNPC.HasAbility('mon_nekker_base')) )
			return false;
			
		if( GetBehaviorVariable('repelType') == 4 && distanceToTarget < 3 && animEventType == AET_DurationStart && slideNPC.GetGameplayVisibility() )
		{
			...
			movementAdjustor.MaxLocationAdjustmentSpeed( slideTicket, 10 );
			movementAdjustor.ScaleAnimation( slideTicket );
			movementAdjustor.AdjustmentDuration(slideTicket, 0.25);
			movementAdjustor.RotateTowards(slideTicket, slideNPC);
			movementAdjustor.SlideTowards(slideTicket, slideNPC, MaxF(1.5, distanceToStay), distanceToTarget);
			movementAdjustor.MaxLocationAdjustmentDistance(slideTicket, , distanceToStay, );
			//movementAdjustor.SlideTowards(slideTicket, slideNPC, 1.5);
			//movementAdjustor.MaxLocationAdjustmentDistance(slideTicket, , 0.5, );
		}
		else if( animEventType == AET_DurationStart && slideNPC.GetGameplayVisibility() )
		{
			movementAdjustor = GetMovingAgentComponent().GetMovementAdjustor();
			slideTicket = movementAdjustor.GetRequest( 'SlideToTarget' );
			movementAdjustor.CancelByName( 'SlideToTarget' );
			...
			movementAdjustor.ScaleAnimation( slideTicket );
			minSlideDistance = distanceToStay;
			if( IsInCombatFist() )
			{
	...
	function EquipGeraltFistfightWeapon( val : bool )
	{
		if ( val )
		{
			fistsItems = thePlayer.GetInventory().GetItemsByName('Geralt Fistfight Fists');
			if( !fistsItems.Size() )
	...
	event OnGameCameraTick( out moveData : SCameraMovementData, dt : float )
	{
		...
		{
		if( this.GetCurrentlyCastSign() == ST_Aard && GetBehaviorVariable( 'alternateSignCast' ) == 1 )
		{
			if( ic.critSloMoCam && thePlayer.IsInCombat() && ic.aardSlowMoFactor < 1.0  && !slowMoActive )
			{
				slowMoActive = true;
				theGame.SetTimeScale( ic.aardSlowMoFactor, theGame.GetTimescaleSource(7), theGame.GetTimescalePriority(7), true);
				AddTimer('OnStopTimeSlow',0.8, true);
			}
		}
		
		if( this.GetCurrentlyCastSign() == ST_Igni && GetBehaviorVariable( 'alternateSignCast' ) == 1 )
		{
			if( ic.critSloMoCam && thePlayer.IsInCombat() && ic.igniSlowMoFactor < 1.0  && !slowMoActive )
			{
				slowMoActive = true;
				theGame.SetTimeScale( ic.igniSlowMoFactor, theGame.GetTimescaleSource(7), theGame.GetTimescalePriority(7), true);
				AddTimer('OnStopTimeSlow',0.8, true);
			}
		}
		...
		if( thePlayer.IsInCombat() )
		{
			...
			if( theInput.IsActionPressed( 'CamOffsetIncrease' ) ) 	IncreaseComOffset();
			if( theInput.IsActionPressed( 'CamOffsetDecrease' ) ) 	DecreaseComOffset();
			if( theInput.IsActionPressed( 'CamDepthIncrease' ) ) 	IncreaseComDepth();
			if( theInput.IsActionPressed( 'CamDepthDecrease' ) )	DecreaseComDepth();
			if( theInput.IsActionPressed( 'CamHeightIncrease' ) )	IncreaseComHeight();
			if( theInput.IsActionPressed( 'CamHeightDecrease' ) )	DecreaseComHeight();
		}
		else
		{
			if( theInput.IsActionPressed( 'CamOffsetIncrease' ) ) 	IncreaseOffset();
			if( theInput.IsActionPressed( 'CamOffsetDecrease' ) ) 	DecreaseOffset();
			if( theInput.IsActionPressed( 'CamDepthIncrease' ) ) 	IncreaseDepth();
			if( theInput.IsActionPressed( 'CamDepthDecrease' ) )	DecreaseDepth();
			if( theInput.IsActionPressed( 'CamHeightIncrease' ) )	IncreaseHeight();
			if( theInput.IsActionPressed( 'CamHeightDecrease' ) )	DecreaseHeight();
		}
		// ImmersiveCam--
		...
		if( substateManager.UpdateCameraIfNeeded( moveData, dt ) )
		{
			//return true; //ImmersiveCam
		}
		*/
		
		// focusMode camera
		if ( theGame.IsFocusModeActive() && ic.witcherSenseZoom && GetCurrentStateName() == 'Exploration' )
		{
			//restricted movement
			if( ( movementLockType == PMLT_NoSprint || movementLockType == PMLT_NoRun ) && !ic.noInteriorCamChange )
				DampVectorSpring( moveData.cameraLocalSpaceOffset, moveData.cameraLocalSpaceOffsetVel, Vector( ic.iWSOffset, ic.iWSDepth, ic.iWSHeight ), 0.5f, dt );
			//normal
			else 
				DampVectorSpring( moveData.cameraLocalSpaceOffset, moveData.cameraLocalSpaceOffsetVel, Vector( ic.eWSOffset + icOffsetAdj, ic.eWSDepth + icDepthAdj, ic.eWSHeight + icHeightAdj ), 0.5f, dt );
		}
		// combat camera
		else if( GetCurrentStateName() == 'CombatSteel' || GetCurrentStateName() == 'CombatSilver' || GetCurrentStateName() == 'CombatFists' )
		{	
			if ( IsCameraLockedToTarget() )
				DampVectorSpring( moveData.cameraLocalSpaceOffset, moveData.cameraLocalSpaceOffsetVel, Vector( ic.hlOffset, ic.hlDepth, ic.hlHeight ), 0.60f, dt );
			else
				DampVectorSpring( moveData.cameraLocalSpaceOffset, moveData.cameraLocalSpaceOffsetVel, Vector( ic.comOffset + icComOffsetAdj, ic.comDepth + icComDepthAdj, ic.comHeight + icComHeightAdj ), 0.60f, dt );
		}
		// clue camera
		else if( GetPlayerAction() == PEA_ExamineGround )
		{
			DampVectorSpring( moveData.cameraLocalSpaceOffset, moveData.cameraLocalSpaceOffsetVel, Vector( ic.clueOffset + icOffsetAdj, ic.clueDepth + icDepthAdj, ic.clueHeight + icHeightAdj ), 1.f, dt ); 
		}
		// exploration camera
		else
		{
			//vanilla sprint came
			if( GetIsSprinting() && ic.sprintMode == 2)
			{
				//do nothing - will use vanilla cam for sprint
			}
			//custom sprint cam
			else if ( GetIsSprinting() && ic.sprintMode == 3 )
			{
				DampVectorSpring( moveData.cameraLocalSpaceOffset, moveData.cameraLocalSpaceOffsetVel, Vector( ic.sprintOffset, ic.sprintDepth, ic.sprintHeight ), 0.60f, dt );
			}
			//restricted movement
			else if( ( movementLockType == PMLT_NoSprint || movementLockType == PMLT_NoRun ) && !ic.noInteriorCamChange )
			{
				DampVectorSpring( moveData.cameraLocalSpaceOffset, moveData.cameraLocalSpaceOffsetVel, Vector( ic.intOffset, ic.intDepth, ic.intHeight), 0.60f, dt );
			}
			//normal
			else
			{
				DampVectorSpring( moveData.cameraLocalSpaceOffset, moveData.cameraLocalSpaceOffsetVel, Vector( ic.expOffset + icOffsetAdj, ic.expDepth + icDepthAdj, ic.expHeight + icHeightAdj ), 0.60f, dt );
			}
		}
		// ImmersiveCam--
		}
	...
	event OnGameCameraPostTick( out moveData : SCameraMovementData, dt : float )
	{
		...
		if(GetExplCamera() && substateManager.GetStateCur() == 'Interaction' && !Options().EnableImmersiveCam())
		{
	...
	{	
		...
		if(!Options().EnableImmersiveCam())
		moveData.pivotDistanceController.SetDesiredDistance( 1.5f ); //ImmersiveCam
	...
	{	
		...
		else
		{
			if( this.IsCurrentSignChanneled() && this.GetCurrentlyCastSign() != ST_Quen && this.GetCurrentlyCastSign() != ST_Yrden )
			{
				...
				{
					// ImmersiveCam++
					screenSpaceOffsetFwd = ic.fsDepth;
					screenSpaceOffsetUp = ic.fsHeight;
					// ImmersiveCam--
				}
				else if ( moveData.pivotRotationValue.Pitch > 0 )
				{
					// ImmersiveCam++
					screenSpaceOffsetFwd = moveData.pivotRotationValue.Pitch*0.00727 + 1.275f;
					screenSpaceOffsetFwd = ClampF( screenSpaceOffsetFwd, ic.fsDepth - 0.3, ic.fsDepth + 0.4 );
					
					screenSpaceOffsetUp = -moveData.pivotRotationValue.Pitch*0.00727 + 0.4363f;
					screenSpaceOffsetUp = ClampF( screenSpaceOffsetUp, ic.fsHeight - 0.4, ic.fsHeight - 0.1 );
					// ImmersiveCam--	
				}
				else	
				{
					...
					{
						screenSpaceOffsetFwd = -moveData.pivotRotationValue.Pitch*0.0425 + 0.8625f;		
						screenSpaceOffsetFwd = ClampF( screenSpaceOffsetFwd, ic.fsDepth - 0.3, ic.fsDepth + 0.5 );
					}
					else
					{
						screenSpaceOffsetFwd = -moveData.pivotRotationValue.Pitch*0.035 + 0.75f;
						screenSpaceOffsetFwd = ClampF( screenSpaceOffsetFwd, ic.fsDepth - 0.3, ic.fsDepth + 0.8 );
					}
					screenSpaceOffsetUp = -moveData.pivotRotationValue.Pitch*0.005 + 0.325f;
					screenSpaceOffsetUp = ClampF( screenSpaceOffsetUp, ic.fsHeight, ic.fsHeight + 0.1 );
					// ImmersiveCam--				
				}
	...
	protected function UpdateCameraSprint( out moveData : SCameraMovementData, timeDelta : float )
	{
		...
		if((GetExplCamera() || GetCmbtCamera()) && !Options().EnableImmersiveCam())
		{
			return;
		}		
		...
		if( camera )
		{
			if ( sprintingCamera )
			{
				...
				DampFloatSpring( camera.fov, fovVel, 70.f, 1.0, timeDelta );
					...
					DampFloatSpring( camera.fov, fovVel, ic.expFOV, 1.0, timeDelta ); //ImmersiveCam
			}
			else
			{
	...
	{	
		...
		if( (!theGame.IsUberMovementEnabled() || Options().EnableImmersiveCam()) && !useSprintingCameraAnim )
		{
			return;
		}
		//ImmersiveCam--
	...
	{	
		...
		if ( !Options().EnableImmersiveCam() && !GetExplCamera() && !GetCmbtCamera() &&   ( !IsCurrentSignChanneled() || GetCurrentlyCastSign() == ST_Quen || GetCurrentlyCastSign() == ST_Yrden ) && !specialAttackCamera && !IsInCombatActionFriendly() )
		{
	...
	{
		if( Options().LockOn() == 0 && IsInCombatAction_SpecialAttack() )
		{
	...
	// W3EE - Begin
	public function ClearCustomOrientationInfoStack()
	{
	...
	{
		if( !Options().LockOnMode() && Options().LockOn() == 0 && !IsHardLockEnabled() )
		{
			if( GetCombatAction() == EBAT_ItemUse )
		...
		{
			if( GetCombatAction() == EBAT_CastSign && theGame.GetInGameConfigWrapper().GetVarValue('EnhancedTargeting', 'ETSignsTowardsCamera') )
				return VecHeading( theCamera.GetCameraDirection() ) - oTCameraOffset;
	...
	public function CanSprint( speed : float ) : bool
	{
		...
		{
			// W3EE - Begin
			if ( ShouldUseStaminaWhileSprinting() && !GetIsSprinting() && !IsInCombat() && GetStatPercents(BCS_Stamina) <= 0.1 )
			// W3EE - End
			{
				return false;
			}
	...
	protected function FindTargetSimple()
	{
		...
		targetingInfo.knockDownCheck = false;
		targetingInfo.knockDownCheckDist = 1.0f;
		targetingInfo.rsHeadingCheck = false;
		targetingInfo.rsHeadingLimitCos = 1.0f;
		...
		for( i = targets.Size() - 1; i >= 0; i -= 1 )
		{
			...
			if ( !IsEntityTargetable( targetingInfo, false ) )
				targets.EraseFast( i );
				
			//Kolaris - posession
			if ( !IsThreat( targets[i], false ) && !targets[i].IsAnimal() && !targets[i].HasBuff(EET_AxiiGuardMe) )
			{
		...
		{
			//Kolaris - posession
			if ( !IsThreat( currentTarget, false ) && !targets[i].IsAnimal() && !targets[i].HasBuff(EET_AxiiGuardMe) )
			{
				currentTarget = NULL;
			}
			...
			for( i = 0; i < targets.Size(); i += 1 )
			{
				...
				rank = inverseDist * (distanceWeight + 1);
		...
		else
		{
			...
			if (potentialFriendlyTargets.Size() > 0)
			{
	...
	public var aimAssist : bool;
	public var aimAssistDist : float;
	public var aimAssistAngle : float;
	public var aimAssistAngleFacing : float;
	
	public function SetAimAssist( b : bool )
	{
		aimAssist = b;
	}
	
	public function SetAssistDistance( f : float )
	{
		aimAssistDist = f;
	}
	
	public function SetAssistAngle( f : float )
	{
		aimAssistAngle = f;
	}
	
	public function SetAssistAngleFacing( f : float )
	{
		aimAssistAngleFacing = f;
	}
	// W3EE - End
	...
	// W3EE - Begin
	public function SetTarget( targetActor : CActor, optional forceSetTarget : bool )
	// W3EE - End
	{
	...
	public function LockToTarget( flag : bool )
	{
		...
		// W3EE - Begin
		if (theGame.GetInGameConfigWrapper().GetVarValue('EnhancedTargeting', 'ETDisableCameraLock'))
		{
			if (IsCameraLockedToTarget())
	...
	public function GetCriticalHitChance( isLightAttack : bool, isHeavyAttack : bool, target : CActor, victimMonsterCategory : EMonsterCategory, isBolt : bool ) : float
	{
		...
		*/
		...
		critVal = ((W3PlayerWitcher)this).GetOilCritChanceBonus(victimMonsterCategory);
		critChance += critVal.valueMultiplicative;
		// W3EE - End
	...
	public function GetCriticalHitDamageBonus(weaponId : SItemUniqueId, victimMonsterCategory : EMonsterCategory, isStrikeAtBack : bool) : SAbilityAttributeValue
	{
		...
		*/
		...
		bonus += ((W3PlayerWitcher)this).GetOilCritDamageBonus(victimMonsterCategory);
	...
	public function ReactToBeingHit(damageAction : W3DamageAction, optional buffNotApplied : bool) : bool
	{
		...
		if(damageAction.DealsAnyDamage())
		{
			...
			if(shakeCam)
			{
				animType = damageAction.GetHitReactionType();
	...
	// W3EE - Begin
	public function HasStaminaToParry( attackName : name ) : bool
	{
		return (IsHeavyAttack(attackName) && HasStaminaToUseAction(ESAT_Parry, , 1.2f)) || (IsLightAttack(attackName) && HasStaminaToUseAction(ESAT_Parry));
	}
	
	function PerformPerfectParryCheck( parryInfo : SParryInfo ) : bool
	{
		var mult 					: float;
		var parryType 				: EParryType;
		...
		var npc : CNewNPC;
		var staggerEffect : W3DamageAction;
		var animName : name;
		
		poiseEffect = (W3Effect_Poise)GetBuff(EET_Poise);
		poiseValue = poiseEffect.GetPoisePercentage();
		//Kolaris - Poise Tweaks
		if( ParryCounterCheck() && parryInfo.targetToAttackerAngleAbs < theGame.params.PARRY_HALF_ANGLE && FistFightCheck(parryInfo.target, parryInfo.attacker, fistFightCounter) && CheckCounterSpamming(parryInfo.attacker) && parryInfo.targetToAttackerDist <= 3.f )
		{
			...
			SetBehaviorVariable('combatActionType', (int)CAT_Parry);
			SetBehaviorVariable('repelType', (int)PRT_Slash);
			
			//plasticmetal - Perfect Parry Tweaks
			//if( RaiseForceEvent('PerformParry') )
			parryspeedid = SetAnimationSpeedMultiplier(1.2f, parryspeedid);
			AddTimer( 'ResetParrySpeed', 0.55f/1.2f );
			if(IsWeaponHeld('fist'))
			{
				switch(parryDir)
				{						
					case PPD_Left:
						thePlayer.GetRootAnimatedComponent().PlaySlotAnimationAsync( 'man_fistfight_parry_side_left', 'PLAYER_SLOT', SAnimatedComponentSlotAnimationSettings(0, 0.55));
						break;
					
					case PPD_Forward:
						thePlayer.GetRootAnimatedComponent().PlaySlotAnimationAsync( 'man_fistfight_parry_left', 'PLAYER_SLOT', SAnimatedComponentSlotAnimationSettings(0, 0.55));
						break;
					
					case PPD_Right:
						thePlayer.GetRootAnimatedComponent().PlaySlotAnimationAsync( 'man_fistfight_parry_side_right', 'PLAYER_SLOT', SAnimatedComponentSlotAnimationSettings(0, 0.55));
						break;
					default: 
			...
			else
			{
				...
				{
					case PPD_Back:
						if(thePlayer.GetCombatIdleStance() == 0)
							thePlayer.GetRootAnimatedComponent().PlaySlotAnimationAsync( 'man_geralt_sword_parry_b_left_lp', 'PLAYER_SLOT', SAnimatedComponentSlotAnimationSettings(0, 0.55));
						else
							thePlayer.GetRootAnimatedComponent().PlaySlotAnimationAsync( 'man_geralt_sword_parry_b_right_rp', 'PLAYER_SLOT', SAnimatedComponentSlotAnimationSettings(0, 0.55));
						break;
					
					case PPD_Left:
						if(thePlayer.GetCombatIdleStance() == 0)
							thePlayer.GetRootAnimatedComponent().PlaySlotAnimationAsync( 'man_geralt_sword_parry_l_left_lp', 'PLAYER_SLOT', SAnimatedComponentSlotAnimationSettings(0, 0.55));
						else
							thePlayer.GetRootAnimatedComponent().PlaySlotAnimationAsync( 'man_geralt_sword_parry_l_left_rp', 'PLAYER_SLOT', SAnimatedComponentSlotAnimationSettings(0, 0.55));
						break;
					
					case PPD_Forward:
						if(thePlayer.GetCombatIdleStance() == 0)
							thePlayer.GetRootAnimatedComponent().PlaySlotAnimationAsync( 'man_geralt_sword_parry_f_leftup_lp', 'PLAYER_SLOT', SAnimatedComponentSlotAnimationSettings(0, 0.55));
						else
							thePlayer.GetRootAnimatedComponent().PlaySlotAnimationAsync( 'man_geralt_sword_parry_f_leftup_rp', 'PLAYER_SLOT', SAnimatedComponentSlotAnimationSettings(0, 0.55));
						break;
					
					case PPD_Right:
						if(thePlayer.GetCombatIdleStance() == 0)
							thePlayer.GetRootAnimatedComponent().PlaySlotAnimationAsync( 'man_geralt_sword_parry_r_right_lp', 'PLAYER_SLOT', SAnimatedComponentSlotAnimationSettings(0, 0.55));
						else
							thePlayer.GetRootAnimatedComponent().PlaySlotAnimationAsync( 'man_geralt_sword_parry_r_right_rp', 'PLAYER_SLOT', SAnimatedComponentSlotAnimationSettings(0, 0.55));
						break;
					default: break;
				}
			}
			...
			if( npc.IsImmuneToBuff(EET_Stagger) || npc.IsImmuneToBuff(EET_LongStagger) )
			{
				staggerEffect = new W3DamageAction in theGame;
				staggerEffect.Initialize(this, parryInfo.attacker, this, '', EHRT_Heavy, CPS_Undefined, true, false, false, false);
				staggerEffect.SetCannotReturnDamage(true);
				staggerEffect.SetCanPlayHitParticle(false);
				staggerEffect.SetHitAnimationPlayType(EAHA_ForceYes);
				
				theGame.damageMgr.ProcessAction(staggerEffect);
			}
			else
			{
				//Kolaris - Bear School Techniques
				if( !(npc.IsImmuneToBuff(EET_Knockdown)) && ((W3PlayerWitcher)this).CanUseSkill(S_Perk_07) && RandRange(100, 0) < CalculateAttributeValue(((W3PlayerWitcher)this).GetAttributeValue('parryKnockdownChance')) )
					npc.AddEffectDefault(EET_Knockdown, this, "PerfectParry");
				else
					npc.AddEffectDefault(EET_Stagger, this, "PerfectParry");
			}
			
			GCameraShake(0.35f, false, thePlayer.GetWorldPosition(), 10,,, 1.5f);
			
			//Kolaris - Immolation
			if( poiseValue >= 0.99f && (HasAbility('Glyphword 8 _Stats', true) || HasAbility('Glyphword 9 _Stats', true)))
			{
				theGame.GameplayFactsSet( "ImmolationCast", 1);
				((W3PlayerWitcher)this).CastDesiredSign( ST_Igni, true, false, false, thePlayer.GetWorldPosition(), thePlayer.GetWorldRotation() );
				theGame.GameplayFactsSet( "ImmolationCast", 0);
			}
			
			//Kolaris - Glaciation
			if( HasAbility('Runeword 1 _Stats', true) || HasAbility('Runeword 2 _Stats', true) || HasAbility('Runeword 3 _Stats', true) )
			{
				npc.AddEffectDefault(EET_SlowdownFrost, this, "Runeword 1");
				npc.CreateFXEntityAndPlayEffect('mutation1_hit', 'mutation_1_hit_aard');
			}
	...
	function PerformParryCheck( parryInfo : SParryInfo ) : bool
	{
		...
		var counter 				: int;
		var onHitCounter 			: SAbilityAttributeValue;
		var shieldItem : W3DLCShield;
		var poiseEffect : W3Effect_Poise;
		
		poiseEffect = (W3Effect_Poise)GetBuff(EET_Poise);
		
		shieldItem = Combat().IsUsingShield();
		if( shieldItem )
			return shieldItem.ProcessShieldParry(this, parryInfo);
		...
		{	
			...
			/*if( parryDir == PPD_Back && (RandRange(100) > (GetSkillLevel(S_Sword_s10) * 20)) )
				return false;*/
			
			this.SetBehaviorVariable( 'combatActionType', (int)CAT_Parry );
			//Kolaris - Projectile Blocking Sprint Fix
			/*if ( parryInfo.targetToAttackerDist > 3.f && !bLAxisReleased && !thePlayer.IsCiri() )
			{
				...
				{
					//Kolaris - Heightened Senses
					Combat().StaminaLoss(ESAT_Parry, 1.f - GetSkillLevel(S_Sword_s10) * 0.2f);
					ClearCustomOrientationInfoStack();
					IncDefendCounter();
				}
			}
			...
			{*/
				...
				{
					if( parryInfo.targetToAttackerDist > 3.f )
					{
						//Kolaris - Heightened Senses
						Combat().StaminaLoss(ESAT_Parry, 1.f - GetSkillLevel(S_Sword_s10) * 0.2f);
					}
	...
	public function GetParryHeading( parryInfo : SParryInfo, out parryDir : EPlayerParryDirection ) : float
	// W3EE - End
	{
	...
	protected function DoAttack(animData : CPreAttackEventData, weaponId : SItemUniqueId, parried : bool, countered : bool, parriedBy : array<CActor>, attackAnimationName : name, hitTime : float)
	{
		...
		// W3EE - Begin
		var attackDistance : EAttackDistance;
		var attackNameString : string;
		
		Combat().AttackCameraShake(animData.attackName, hitTargets.Size() > 0);
		...
		*/
		...
		// W3EE - Begin
		//Kolaris - Cat Set
		/*if( IsLightAttack(animData.attackName) && !IsCounterAttack(animData.attackName) )
		{
			Combat().ActivateSwordDance();
			((W3Effect_SwordSignDancer)GetBuff(EET_SwordSignDancer)).CountActionType(ESA_FastAttack);
		}
		else
		if( IsHeavyAttack(animData.attackName) )
			((W3Effect_SwordSignDancer)GetBuff(EET_SwordSignDancer)).CountActionType(ESA_StrongAttack);*/
		if( ((W3PlayerWitcher)this).IsSetBonusActive(EISB_Lynx_1) && (IsLightAttack(animData.attackName) || IsHeavyAttack(animData.attackName)) && !IsCounterAttack(animData.attackName) )
			AddEffectDefault(EET_LynxSetDodge, this, "LynxSet");
			
		//Kolaris - Attack Stamina Cost Timing
		/*if( IsInCombatAction_SpecialAttack() || IsSpecialAttack(animData.attackName) || IsCounterAttack(animData.attackName) || IsPerformingFinisher() )
			return;
			
		attackNameString = NameToString(attackAnimationName);
		if( StrContains(attackNameString, "_far_") )
		{
			if( StrContains(attackNameString, "_mod") )
				attackDistance = ADIST_Large;
			else
				attackDistance = ADIST_Medium;
		}
		else attackDistance = ADIST_Small;
		
		switch(attackDistance)
		{
			case ADIST_Large: 
				if( IsHeavyAttack(animData.attackName) )
					Combat().StaminaLoss(ESAT_HeavyAttack, 1.f);
				else
					Combat().StaminaLoss(ESAT_LightAttack, 1.f);
			break;
	...
	public function ProcessCombatActionBuffer() : bool
	{
		...
		//Kolaris - 2H Sword Stance
		if( GetBehaviorVariable('playerDoubleHandSword') == 0 && action != EBAT_ItemUse )
		{
			if( Options().GetUse2HSwordStance() || GetStatPercents(BCS_Vitality) <= 0.3f || ((W3PlayerWitcher)this).GetTotalActiveThreat() >= 95 )
				SetBehaviorVariable('playerDoubleHandSword',1.0f);
		}
		...
		switch ( action )
		{
				...
				switch ( stage )
				{
					case BS_Pressed :
					{
						...
						else
						{
							...
							//Kolaris - Attack Stamina Drain Timing
							Combat().StaminaLoss(ESAT_LightAttack, 1.f);
				...
				switch ( stage )
				{
					case BS_Pressed :
					{
							...
							
						{
							...
							else if(inv.IsItemBomb(selectedItemId) && this.inv.SingletonItemGetAmmo(selectedItemId) > 0 || inv.ItemHasTag(selectedItemId, 'ThrowingKnife') )
							{
			...
			case EBAT_Dodge :
			{
				switch ( stage )
				{
					...
					{
						//W3EE - Begin
						if( HasStaminaToUseAction(ESAT_Dodge) )
						{
							Combat().EvadeSpeedModule();
							Combat().StaminaLoss(ESAT_Dodge, 1.f);
							//Kolaris - Dodge Exhaustion
							GetPoiseEffect().ReducePoise(0, 2.f, 'Dodge');
							...
							GetInjuryManager().DodgeStumbles();
							//Kolaris - Cat Set
							if( ((W3PlayerWitcher)this).IsSetBonusActive(EISB_Lynx_1) )
								AddEffectDefault(EET_LynxSetAttack, this, "LynxSet");
							//Kolaris - Acceleration
							if( HasAbility('Glyphword 4 _Stats', true) || HasAbility('Glyphword 5 _Stats', true) || HasAbility('Glyphword 6 _Stats', true) )
							{
								if( !HasBuff(EET_ReflexBlast) )
									AddEffectDefault(EET_ReflexBlast, this, "AardReflexBlast");
								else
									((W3Effect_ReflexBlast)GetBuff(EET_ReflexBlast)).StackEffectDuration();
							}
						}
			...
			case EBAT_Roll :
			{
				...
				switch ( stage )
				{
					...
					{
						//W3EE - Begin
						//Kolaris - Roll Stamina
						if( HasStaminaToUseAction(ESAT_Roll, , 1.5f) )
						{
							Combat().EvadeSpeedModule();
							Combat().StaminaLoss(ESAT_Roll, 1.5f);
							//Kolaris - Dodge Exhaustion
							GetPoiseEffect().ReducePoise(0, 3.f, 'Dodge');
							Experience().AwardDodgingXP(this);	
							theGame.GetBehTreeReactionManager().CreateReactionEvent( this, 'PlayerEvade', 1.0f, 10.0f, -1.0f, -1 );
							thePlayer.BreakPheromoneEffect();
		...
		if (actionResult)
		{
			//Kolaris - Stamina Regen Delay
			if( action == EBAT_LightAttack || action == EBAT_HeavyAttack || action == EBAT_SpecialAttack_Light || action == EBAT_SpecialAttack_Heavy || action == EBAT_Dodge || action == EBAT_Roll || action == EBAT_Parry )
				PauseStaminaRegen( 'InsideCombatAction' );
			
			//Kolaris - Exhilaration
			if( ((W3PlayerWitcher)this).HasAbility('Runeword 32 _Stats', true) || ((W3PlayerWitcher)this).HasAbility('Runeword 33 _Stats', true) )
			{
				if( action == EBAT_LightAttack || action == EBAT_HeavyAttack )
					((W3PlayerWitcher)this).AddAbility('Runeword 32 Ability', true);
				else if( action == EBAT_Parry || action == EBAT_Roll || action == EBAT_Dodge || action == EBAT_CastSign || action == EBAT_ItemUse )
					((W3PlayerWitcher)this).RemoveAbilityAll('Runeword 32 Ability');
			}
	...
	{
		return super.CanPerformCounter();
	}
	...
	public function DisplayHudMessage( value : string ) : void
	{
		...
		// -= WMK:modQuickSlots =-
		if (WmkGetQuickSlotsInstance().OnDisplayHudMessage(value)) {
			return;
		}
		// -= WMK:modQuickSlots =-
	...
	private final function DisallowedActionDontShowHack(action : EInputActionBlock, isTimeLock : bool) : bool
	{
		...
		// W3EE - Begin
		if( GetWitcherPlayer() && GetWitcherPlayer().IsMeditating() )
			return true;		
		// W3EE - End
	...
	public final function HasRequiredLevelToEquipItem(item : SItemUniqueId) : bool
	{
		...
		return Equipment().LevelRequirements(item, inv);
	...
	public function ConsumeItem( itemId : SItemUniqueId ) : bool
	{
		...
		if(category == 'edibles' || inv.ItemHasTag(itemId, 'Drinks') || ( category == 'alchemy_ingredient' && inv.ItemHasTag(itemId, 'Alcohol')) )
		{
			// W3EE - Begin
			//Kolaris - Bandage Fix
			if( inv.ItemHasTag(itemId, 'Drinks') && !inv.ItemHasTag(itemId, 'Bandage' ) )
				inv.AddAnItem('Empty bottle', 1);
				
			if( inv.ItemHasTag(itemId, 'Jar') )
				inv.AddAnItem('Jar', 1);
				
			if( inv.ItemHasAbility(itemId, 'FoodPoisoning') )
				ApplyPoisoning(1, this, "Poisoning");
			// W3EE - End
	...
	public function HasStaminaToUseAction(action : EStaminaActionType, optional abilityName : name, optional dt :float, optional multiplier : float) : bool
	{
		...
		witcher = (W3PlayerWitcher)this;
		cost = Combat().GetActionStaminaCost(action, temp, multiplier, dt, abilityName, true);
		//Kolaris - Stamina Options
		if( Options().StamCostGlobal() > 0 )
			cost *= Options().StamCostGlobal();
		ret = GetStat(BCS_Stamina) >= cost; // super.HasStaminaToUseAction(action, abilityName, dt, multiplier);
		
		//Kolaris - Conservation
		if(!ret && GetStat(BCS_Focus) >= 1 && (witcher.HasAbility('Glyphword 52 _Stats', true) || witcher.HasAbility('Glyphword 53 _Stats', true) || witcher.HasAbility('Glyphword 54 _Stats', true)) )
		{
			DrainFocus(1);
			GainStat(BCS_Stamina, 50 * Options().StamCostGlobal());
			PlayEffect('mutation_energy');
		}
		//Kolaris - Desperation
		else if(!ret && witcher.GetAdrenalineEffect().GetFullValue() >= cost / Options().StamCostGlobal() && (witcher.HasAbility('Runeword 22 _Stats', true) || witcher.HasAbility('Runeword 23 _Stats', true) || witcher.HasAbility('Runeword 24 _Stats', true)) )
		{
			GainStat(BCS_Stamina, cost);
			witcher.GetAdrenalineEffect().RemoveAdrenaline(cost / Options().StamCostGlobal());
		}
		else if(!ret)
		{
			SetCombatActionHeading( GetHeading() );
			AddEffectDefault(EET_Overexertion, this, "Overexertion", false);
	...
	// W3EE - Begin
    public function GetVigorActionCost( abilityName : name , dt : float, costMult : float ) : float
    {
        var temp, temp2 : float;
        
		if( abilityManager && abilityManager.IsInitialized() && IsAlive() )
			return abilityManager.GetVigorActionCost(temp, temp2, abilityName, dt, costMult);
			
		return 0;
    }
    
    public function DrainVigor( abilityName : name , dt : float, costMult : float, optional overrideValue : float, optional forceDrain : bool ) : bool
    {
		if( abilityManager && abilityManager.IsInitialized() && IsAlive() )
			return abilityManager.DrainVigor(abilityName, dt, costMult, overrideValue, forceDrain);
	...
	{
		if( abilityManager && abilityManager.IsInitialized() && IsAlive() )
			abilityManager.DrainFocus(amount, dt);
	}
	// W3EE - End
	...
	public function IsHeavyAttack(attackName : name) : bool
	{
		...
		if ( attackName == 'attack_heavy_special' || attackName == 'geralt_heavy_special1' || attackName == 'geralt_heavy_special2' )
			return true;
	...
	public function IsLightAttack(attackName : name) : bool
	{
		...
		if ( attackName == 'attack_light_special' || IsCounterAttack(attackName) )
			return true;
	...
	public function IsSpecialAttack( attackName : name ) : bool
	{
		return attackName == 'geralt_kick_special' || attackName == 'geralt_heavy_special1' || attackName == 'geralt_heavy_special2';
	}
	
	public function IsCounterAttack( attackName : name ) : bool
	{
		return StrContains(NameToString(attackName), "counter");
	}
	
	public function IsFinisher( attackName : name ) : bool
	{
		return StrContains(NameToString(attackName), "finisher");
	}
	// W3EE - End
	...
	public function IsThreat( actor : CActor, optional usePrecalcs : bool ) : bool
	{
		...
		//Kolaris - posession
		if ( ((CNewNPC)actor).HasBuff(EET_AxiiGuardMe) )
			return false;
	...
	// W3EE - Begin
	private var StaggeredDuringAnim : bool; default StaggeredDuringAnim = false;
	private var weapon : EPlayerWeapon;
	private var IsCurrentlyStaggered : bool; default IsCurrentlyStaggered = false;
	private var AnimatedState : bool; default AnimatedState = false;
	
	public function GetStaggeredDuringAnim() : bool
	{
		return StaggeredDuringAnim;
	}
	
	public function SetStaggeredDuringAnim( hit : bool )
	{
		StaggeredDuringAnim = hit;
	}
	
	public function GetWep() : EPlayerWeapon
	{
		return weapon;
	}
	
	public function SetWep( weps : EPlayerWeapon )
	{
		weapon = weps;
	}
	
	public function GetIsStaggered() : bool
	{
		return IsCurrentlyStaggered;
	}
	
	public function SetIsStaggered( i : bool )
	{
		IsCurrentlyStaggered = i;
	}
	
	timer function SetNotStaggered( dt : float, id : int )
	{
		IsCurrentlyStaggered = false;
	}
	
	//plasticmetal - Perfect Parry Tweaks
	timer function ResetParrySpeed( dt : float, id : int )
	{
		ResetAnimationSpeedMultiplier(parryspeedid);
	}
	
	public function GetIsInAnimatedState() : bool
	{
		return AnimatedState;
	}
	
	public function SetAnimatedState( i : bool )
	{
		AnimatedState = i;
		
		if( i == true )
		{
			BlockAction(EIAB_DrawWeapon, 			'ConsumableAnims');
			BlockAction(EIAB_CallHorse,				'ConsumableAnims');
			BlockAction(EIAB_Signs,					'ConsumableAnims');
			BlockAction(EIAB_Crossbow,				'ConsumableAnims');
			BlockAction(EIAB_UsableItem,			'ConsumableAnims');
			BlockAction(EIAB_ThrowBomb,				'ConsumableAnims');
			BlockAction(EIAB_SwordAttack,			'ConsumableAnims');
			BlockAction(EIAB_Jump,					'ConsumableAnims');
			BlockAction(EIAB_Dodge,					'ConsumableAnims');
			BlockAction(EIAB_Roll,					'ConsumableAnims');
			BlockAction(EIAB_LightAttacks,			'ConsumableAnims');
			BlockAction(EIAB_HeavyAttacks,			'ConsumableAnims');
			BlockAction(EIAB_SpecialAttackLight,	'ConsumableAnims');
			BlockAction(EIAB_SpecialAttackHeavy,	'ConsumableAnims');
		}
		else
		{
			UnblockAction(EIAB_DrawWeapon, 			'ConsumableAnims');
			UnblockAction(EIAB_CallHorse,			'ConsumableAnims');
			UnblockAction(EIAB_Signs,				'ConsumableAnims');
			UnblockAction(EIAB_Crossbow,			'ConsumableAnims');
			UnblockAction(EIAB_UsableItem,			'ConsumableAnims');
			UnblockAction(EIAB_ThrowBomb,			'ConsumableAnims');
			UnblockAction(EIAB_SwordAttack,			'ConsumableAnims');
			UnblockAction(EIAB_Jump,				'ConsumableAnims');
			UnblockAction(EIAB_Dodge,				'ConsumableAnims');
			UnblockAction(EIAB_Roll,				'ConsumableAnims');
			UnblockAction(EIAB_LightAttacks,		'ConsumableAnims');
			UnblockAction(EIAB_HeavyAttacks,		'ConsumableAnims');
			UnblockAction(EIAB_SpecialAttackLight,	'ConsumableAnims');
			UnblockAction(EIAB_SpecialAttackHeavy,	'ConsumableAnims');		
		}
	}
	// W3EE - End
	...
	event OnFinisherStart()
	{
		...
		//W3EE - Begin
		if( Options().FinisherCinematicCamera() )
		...
		Combat().ReaperFinisher();
		((W3PlayerWitcher)this).GetAdrenalineEffect().AdrenalineGrantFinisher();
		//SetImmortalityMode( AIM_Invulnerable, AIC_SyncedAnim );
		//W3EE - End
		
		//Kolaris - Mutilation, Kolaris - Morale Loss
		moveTargets = GetMoveTargets();
		
		for ( i = 0; i < moveTargets.Size(); i += 1 )
		{
			if ( moveTargets[i].IsHuman() && IsRequiredAttitudeBetween(this, moveTargets[i], true))
			{
				moveTargets[i].DrainMorale(50.f * (1.6f - Options().AggressionBehavior() * 0.2f));
				if( ((W3PlayerWitcher)this).HasAbility('Runeword 56 _Stats', true) || ((W3PlayerWitcher)this).HasAbility('Runeword 57 _Stats', true) )
					moveTargets[i].DrainMorale(50.f * (1.6f - Options().AggressionBehavior() * 0.2f));
			}
		}
	}
	...
	//Kolaris - Player Stagger Duration
	...
	{
		var params : SCustomEffectParams;
		var effAtt, speedAtt : SAbilityAttributeValue;
		var mult : float;
		...
		if(effectType == EET_Stagger || effectType == EET_LongStagger || effectType == EET_Knockdown || effectType == EET_HeavyKnockdown)
		{
			...
			effAtt = GetAttributeValue('movement_stamina_efficiency');
			speedAtt = GetAttributeValue('evade_speed');
			mult = 1.f * (1.f - Combat().CalcArmorPenalty((W3PlayerWitcher)this, false)) / (1.f + speedAtt.valueMultiplicative) / (1.f + effAtt.valueMultiplicative);
			
			if ( effectType == EET_Stagger )
				params.duration = 1.5 * mult;
			else if ( effectType == EET_LongStagger )
				params.duration = 3 * mult;
			else if ( effectType == EET_Knockdown ) 
				params.duration = 2.5;
			else if ( effectType == EET_HeavyKnockdown ) 
				params.duration = 4;
			//theGame.GetGuiManager().ShowNotification(params.duration);
	...
	public function CheatResurrect()
	{
		...
		for ( i = 0; i < size; i += 1 )
		{
			itemPrice = inv.GetItemBasePrice(items[i]);
	...
	public function SetSelectedItemId( itemId : SItemUniqueId )		{selectedItemId = itemId;}
	...
	function BlockSlotsOnLItemUse ()
	{
		...
		// -= WMK:modQuickSlots =-
		slotsToBlock.PushBack( 'Crossbow' );
		slotsToBlock.PushBack( 'Slot6' );
		// -= WMK:modQuickSlots =-
	...
	public function CanSetupCombatAction_Throw() : bool
	{
		...
		if(!inv.IsItemSingletonItem(selectedItemId) && !inv.ItemHasTag(selectedItemId, 'ThrowingKnife'))
			return false;
			...
			if(attackAction.IsActionMelee())
			{
				...
				*/
				...
				if( npc )
				{
					...
					//Kolaris - Absorption
					lifeLeech += CalculateAttributeValue(GetAttributeValue('lifesteal'));
					
					//Kolaris - Desperation
					if( ((W3PlayerWitcher)this).HasAbility('Runeword 24 _Stats', true) )
						lifeLeech += ((W3PlayerWitcher)this).GetAdrenalineEffect().GetFullValue() / 2000 + (1.f - GetStatPercents(BCS_Vitality)) / 10;
					
					//Kolaris - Resolution
					if( ((W3PlayerWitcher)this).IsInCombatAction_SpecialAttackHeavy() && GetSpecialAttackTimeRatio() > 0.75f && ((W3PlayerWitcher)this).HasAbility('Runeword 45 _Stats', true) )
					{
						PlayEffect('runeword_20_adrenaline');
						lifeLeech += GetStat(BCS_Focus) / 10;
					...
					{
						//Kolaris - Perfection
						if( GetStatPercents(BCS_Vitality) >= 0.999f && (((W3PlayerWitcher)this).HasAbility('Glyphword 41 _Stats', true) || ((W3PlayerWitcher)this).HasAbility('Glyphword 42 _Stats', true)) )
						{
							GainStat(BCS_Stamina, lifeLeech * 0.02f * Options().StamCostGlobal());
							GainStat(BCS_Focus, lifeLeech * 0.0005f);
						}
	...
	timer function FixHorseCamera( dt : float, id : int )
	{
		if( (CR4PlayerStateHorseRiding)GetCurrentState() && Options().EnableImmersiveCam())//NextGen Cam
		{
			theGame.ActivateHorseCamera(true, 0.f);
		}
	}
	...
	// W3EE - Begin
	timer function RemoveCameraEffect( dt : float, id : int )
	{
	...
	timer function RemoveBloodEffects( dt : float, id : int )
	{
		StopEffect('covered_blood');
	}
	// W3EE - End
	...
	// W3EE - Begin
	private var cancelReload : bool;
	private var isReloading : bool;
	public function SetCancelReload( b : bool )
	{
		cancelReload = b;
	}
	
	public function ShouldCancelReload() : bool
	{
		return cancelReload;
	}
	
	public function SetIsReloading( b : bool )
	{
		isReloading = b;
	}
	...
	public function IsImmuneToBuff( effect : EEffectType ) : bool
	{
		...
		GetResistValue(theGame.effectMgr.GetBuffResistStat(effect), resPt, resPrc);
		
		return super.IsImmuneToBuff(effect) || resPrc >= 1.f;
	}

	timer function ForceTaunt( dt : float, id : int )
	{
		var chance : float = RandF();
		if( RandF() > 0.66f )
			thePlayer.PlayBattleCry('BattleCryHumansHit', 1.f, true, true);
		else
		if( RandF() > 0.33f )
			thePlayer.PlayBattleCry('BattleCryHumansEnd', 1.f, true, true);
		else
