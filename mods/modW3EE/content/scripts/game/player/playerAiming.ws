...
statemachine class PlayerAiming
{
	...
	protected var aimStartTime  : float;
	protected var aimEndTime    : float;
	...
	public function GetAimTime( optional checkOnly : bool ) : float
	{
		var aimTime : float;
		
		if( aimStartTime != 0 && aimType == AT_Bomb )
		{
			if( !checkOnly )
				aimTime = aimEndTime - aimStartTime;
			else
				aimTime = theGame.GetEngineTimeAsSeconds() - aimStartTime;
			
			aimTime /= 0.6f - 0.1f * GetWitcherPlayer().GetSkillLevel(S_Alchemy_s09);
			aimTime *= 1.f + 0.2f * GetWitcherPlayer().GetSkillLevel(S_Alchemy_s10);
...
state Waiting in PlayerAiming
{
	event OnStartAiming( t : CThrowable )
	{
		...
		if ( (W3BoltProjectile)( parent.throwable ) || (W3ThrowingKnife)( parent.throwable ) )
...
state Aiming in PlayerAiming
{
	...
	function UpdateThrowPositionBolt()
	{
		...
		thePlayer.SetCancelReload(false);
	...
	function UpdateThrowPosition()
	{
		...
		*/
		...
		// CalculateAimPosition( parent.tracePosFrom, throwPosInitial, parent.throwPos, throwPosNormal, aimEnt );
		//Kolaris - Steady Aim
		if( parent.owner.CanUseSkill(S_Alchemy_s09) )
			maxRangePos = VecNormalize(theCamera.GetCameraDirection()) * theGame.params.MAX_THROW_RANGE * (1.f + 0.2f * parent.owner.GetSkillLevel(S_Alchemy_s09) - 0.05f * parent.owner.GetSkillLevel(S_Alchemy_s10) - 0.05f * parent.owner.GetSkillLevel(S_Alchemy_s11));
		else
		...
		//Kolaris - Bomb Fuse Rework
		if( parent.GetAimTime(true) >= 5.f + GetWitcherPlayer().GetSkillLevel(S_Alchemy_s10) && !petard.IsEffectActive('sparks') && theGame.GetTimeScale() < 0.61f )
			petard.PlayEffectSingle('sparks');
		/*else if( parent.GetAimTime(true) < 5.f && petard.IsEffectActive('sparks') )
			petard.StopEffect('sparks');*/
	...
	function SloMo()
	{
		...
		//Kolaris - Bomb Fuse Rework
		parent.aimStartTime = theGame.GetEngineTimeAsSeconds();
	...
	{
		//Kolaris - Bomb Fuse Rework
		parent.aimEndTime = theGame.GetEngineTimeAsSeconds();
