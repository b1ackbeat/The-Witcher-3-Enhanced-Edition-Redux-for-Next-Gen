...
class CBTTaskRidingManagerVehicleDismount extends IBehTreeTask
{
    ...
    function FindDismountDirection( riderData : CAIStorageRiderData, vehicleComponent : CVehicleComponent, out dismountDirection : float )
	{
		...
		{
			if( horseComponent.GetHorseSpeed() >= 2.f )
			{
				thePlayer.SetBehaviorVariable('dismountType', 0.f);
				dismountDirection = 4;
				return;
			...
			cameraAngle = AngleDistance(theCamera.GetCameraHeading(), horseComponent.GetHeading());
			if( cameraAngle <= 25.f && cameraAngle >= -25.f )
			{
				thePlayer.SetBehaviorVariable('dismountType', 1.f);
					dismountDirection = 4;
			}
			else
			if( cameraAngle < -25.f && cameraAngle > -120.f )
				dismountDirection = 1;
			else
			if( cameraAngle > 25.f && cameraAngle <= 120.f )
				dismountDirection = 0;
			else
			if( cameraAngle <= -120.f && cameraAngle >= -180.f )
			{
				dismountDirection = 3;
			}
			else
