...
{	
	function IsAvailable() : bool
	{
		...
		// W3EE - Begin
		return (horseComp.GetLastRider() == thePlayer || thePlayer.GetHorseWithInventory() == GetActor()) && GetWitcherPlayer().GetHorseManager().ShouldRest();
		// W3EE - End
	}
}
...
class CBTTaskHorseSummon extends IBehTreeTask
{
	...
	function OnActivate() : EBTNodeStatus
	{
		...
		if ( GetActionTarget() )
		{
			witcher.SetFollowTask(this);
			witcher.AddTimer('SummonDistanceCheck', 0.2, true);
		// W3EE - End
			return BTNS_Active;
		}
		else
	...
	{
		// W3EE - Begin
		GetWitcherPlayer().ClearFollowTask();
		// W3EE - End
