...
class CBehTreeTaskCSEffect extends IBehTreeTask
{
	...
	latent function Main() : EBTNodeStatus
	{
		...
		if ( canCounter && CheckGuardOrCounter(data))
		{
	...
	function CheckGuardOrCounter( data : CDamageData ) : bool
	{
		var npc 		: CNewNPC = GetNPC();
		var hitCounter 	: int;
		...
		if( ((W3DamageAction)data).IsCounterAttack() || (((W3DamageAction)data).GetHitAnimationPlayType() == EAHA_ForceYes && ((W3DamageAction)data).CanPlayHitParticle() == false) )
			return false;
	...
	function OnAnimEvent( animEventName : name, animEventType : EAnimationEventType, animInfo : SAnimationEventAnimInfo ) : bool
	{
		...
		//Kolaris - Premature Confusion End
		if ( animEventName == 'AllowBlend' && animEventType == AET_DurationStart && !(CSType == ECST_Confusion && (npc.HasAbility('mon_drowner_base') || npc.HasAbility('mon_harpy_base'))) )
		{
			npc.RequestCriticalAnimStop(); 
			Complete( true );
			return true;
		...
		else if ( animEventName == 'DisableHitAnim' )
		{
			...
			//Kolaris - Remove BurnNoAnim
			else if ( CSType == ECST_BurnCritical && npc.HasAbility('BurnNoAnim') )
			{
				npc.SetCanPlayHitAnim( true );
				hitAnim = false;
			}
