...
class BTTaskBlockPlayerActions extends IBehTreeTask
{
	...
	function OnActivate() : EBTNodeStatus
	{
		if( onActivate )
		{
			...
			exceptions.PushBack(EIAB_Jump);
			exceptions.PushBack(EIAB_Roll);
			exceptions.PushBack(EIAB_RunAndSprint);
			exceptions.PushBack(EIAB_Dodge);
			exceptions.PushBack(EIAB_Sprint);
			thePlayer.BlockAllActions( 'BTTaskBlockPlayerActions', block, exceptions );
		}
