...
class BTTaskFrostSnakeAttack extends CBTTaskAttack
{
	...
	final function OnActivate() : EBTNodeStatus
	{
		...
		if( canTriggerFrenzySlowmo  )
		{
			...
			if( ( (W3PlayerWitcher) GetCombatTarget() ) && ( thePlayer.IsActionAllowed(EIAB_Dodge) || thePlayer.IsActionAllowed(EIAB_Roll) ) && witcher.GetStat(BCS_Toxicity, false) > theGame.params.TOXICITY_DAMAGE_THRESHOLD && thePlayer.CanUseSkill(S_Alchemy_s16))
