...
class CBTTaskDodge extends CBTTaskPlayAnimationEventDecorator
{
	...
	function IsAvailable() : bool
	{
		...
		{
			// W3EE - Begin
			if ( dodgeEventTime + 0.25 < GetLocalTime() )
			// W3EE - End
			{
				Time2Dodge = false;
			}
	...
	function Dodge() : bool
	{
		...
		*/
		...
		if( combatDataStorage.GetIsAttacking() && Options().AttackCommitment() )
		{
			return false;
		}
	...
	function GetDodgeStats()
	{
		...
		*/
		...
		dodgeChanceAttackLight = Enemies().SetSkillValue(npc, ENST_DodgeChanceLightAttack);
		dodgeChanceAttackHeavy = Enemies().SetSkillValue(npc, ENST_DodgeChanceHeavyAttack);
		dodgeChanceAard = Enemies().SetSkillValue(npc, ENST_DodgeChanceSigns);
		dodgeChanceIgni = Enemies().SetSkillValue(npc, ENST_DodgeChanceSigns);
		dodgeChanceBomb = Enemies().SetSkillValue(npc, ENST_DodgeChanceProjectiles);
		dodgeChanceProjectile = Enemies().SetSkillValue(npc, ENST_DodgeChanceProjectiles);
		hitsToCounter = Enemies().SetSkillValue(npc, ENST_HitsToRollCounter);
		counterChance = Enemies().SetSkillValue(npc, ENST_ChanceToCounter);
	...
	function ChooseAndCheckDodge() : bool
	{
		...
		if( npc.GetInjuryManager().HasInjury(EFI_Legs) || npc.GetInjuryManager().HasInjury(EPI_Legs) )
			return false;
...
class CBTTaskDodgeDef extends CBTTaskPlayAnimationEventDecoratorDef
{
	...
	// W3EE - Begin
	default maxDistanceFromTarget					= 3;
	// W3EE - End
	default disableIsDodgingFlagAfter 				= 0.4;
	// W3EE - Begin
	default allowDodgeWhileAttacking				= true;
	// W3EE - End
	...
	// W3EE - Begin
	default allowDodgeOverlap 						= false;
	// W3EE - End
