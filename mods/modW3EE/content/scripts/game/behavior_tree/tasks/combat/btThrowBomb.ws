...
class CBTTaskThrowBomb extends CBTTaskAttack
{
	...
	function InitializeBombs() : bool
	{
		...
		throwingKnives = inventory.GetItemsByTag('ThrowingKnife');
		if( throwingKnives.Size() <= 0 )
			thrownEntity = (W3Petard)inventory.GetDeploymentItemEntity(selectedBomb);
		else
			thrownEntity = (W3ThrowingKnife)inventory.GetDeploymentItemEntity(selectedBomb);
	...
	function CheckIfFriendlyIsInAoe() : bool
	{
		...
		owner = GetActor();
		radius = ((W3Petard)thrownEntity).GetImpactRange();
