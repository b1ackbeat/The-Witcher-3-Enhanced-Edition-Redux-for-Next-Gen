...
class CTicketAttackAlgorithm extends CTicketBaseAlgorithm
{
    ...
    private function ShouldAskForTicket() : bool
    {
        var owner : CActor = GetActor();
        
        if( owner.GetStatPercents(BCS_Stamina) <= 0.25f && !((CNewNPC)owner).IsHuge() )
            return false;
	...
	public function CalculateTicketImportance() : float
	{
		...
		else if ( npc && GetCombatTarget() == thePlayer && RandRange(100, 1) < Combat().GetEnemyAggressionChance() )
		{
			overrideTicketsCount = 0;
			return 10000;
		}
