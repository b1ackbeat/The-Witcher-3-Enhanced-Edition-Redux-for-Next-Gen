...
class CBTTaskBiesHypnotize extends CBTTask3StateAttack
{
	...
	//Kolaris - Bies Hypnosis
	function IsAvailable() : bool
	{
		var npc : CNewNPC = GetNPC();

		if( npc.HasAbility('qmh206_bies') && !npc.IsInvulnerable() && npc.GetStatPercents(BCS_Essence) >= 0.5f )
			return false;
		else
			return true;
	}
	...
	function GetTargets(out targets : array<CActor>) : bool
	{
		...
		//Kolaris - Morvudd Fix
		if( owner.HasAbility('qmh206_bies') )
			targets = owner.GetAttackableNPCsAndPlayersInCone(20,owner.GetHeading(),360,0);
		else
