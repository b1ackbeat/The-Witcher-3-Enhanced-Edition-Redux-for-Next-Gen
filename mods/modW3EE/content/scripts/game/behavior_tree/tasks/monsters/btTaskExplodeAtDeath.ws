			...
			if( l_actors.Size() > 0 )
			{
				...
				if( IsNameValid( weaponSlot ) )
				{
					...
					*/
					...
					if ( theGame.GetDifficultyMode() == EDM_Easy )     damageValue = 4250  /*+ ( ( l_damageAttr.valueBase * l_damageAttr.valueMultiplicative + l_damageAttr.valueAdditive ) * 2 )*/; else
					if ( theGame.GetDifficultyMode() == EDM_Hard )     damageValue = 5250 /*+ ( ( l_damageAttr.valueBase * l_damageAttr.valueMultiplicative + l_damageAttr.valueAdditive ) * 4 )*/; else
					if ( theGame.GetDifficultyMode() == EDM_Hardcore ) damageValue = 5550 /*+ ( ( l_damageAttr.valueBase * l_damageAttr.valueMultiplicative + l_damageAttr.valueAdditive ) * 5 )*/; else
																	   damageValue = 4750 /*+ ( ( l_damageAttr.valueBase * l_damageAttr.valueMultiplicative + l_damageAttr.valueAdditive ) * 3 )*/;
					// W3EE - End
				}
				...
				for	( i = 0; i < l_actors.Size(); i += 1 )
				{
					...
					l_damageAction.Initialize( NULL, l_actors[i], NULL, "Explosion", EHRT_Heavy, CPS_Undefined, false, false, false, true );
					l_damageAction.SetCannotReturnDamage( true );
					
					// W3EE - Begin
					l_damageAction.AddDamage(  theGame.params.DAMAGE_NAME_BLUDGEONING, damageValue );
