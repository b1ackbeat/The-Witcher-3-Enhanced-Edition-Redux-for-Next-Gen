...
class W3PlayerAbilityManager extends W3AbilityManager
{
	...
	// W3EE - Begin
	private 		var totalSkillSlotsCount, orgTotalSkillSlotsCount : int;
	public    saved var skillSlots : array<SSkillSlot>;
	// W3EE - End
	...
	public final function Init(ownr : CActor, cStats : CCharacterStats, isFromLoad : bool, diff : EDifficultyMode) : bool
	{
		...
		// W3EE - Begin
		orgTotalSkillSlotsCount = 16;
		
		if  ( (!isMutationSystemEnabled) && ( totalSkillSlotsCount > 12) )
			for(i=12; i < orgTotalSkillSlotsCount; i+=1)
			{
				UnequipSkill(skillSlots[i].id);
				skillSlots[i].unlockedOnLevel = 0;
				skillSlots[i].groupID = MUTATION_SKILL_GROUP_ID;
				skillSlots[i].unlocked = false;	
			}
		...
		for(i=0; i<skills.Size(); i+=1)
		{
			if( MustEquipSkill(skills[i].skillType) )
				...
				UnequipSkill(GetSkillSlotID(skills[i].skillType), true);
		}
		// W3EE - End
	...
	{		
		...
		{
			playerLevel = Experience().GetTotalSkillPoints();
			for(i=0; i<skillSlots.Size(); i+=1)
			{
				if( skillSlots[ i ].groupID != MUTATION_SKILL_GROUP_ID )
				{
					// W3EE - Begin
					skillSlots[i].unlocked = ( playerLevel >= skillSlots[i].unlockedOnLevel) || Options().AllSkillsUnlocked();
					// W3EE - End
				}
			}
		}
	...
	public final function AddTempNonAlchemySkills() : array<SSimpleSkill>
	{
		...
		skillIDs.PushBack(45);
		skillIDs.PushBack(47);
		skillIDs.PushBack(48);
		skillIDs.PushBack(51);
		skillIDs.PushBack(54);
		for(i=0; i<skillIDs.Size(); i+=1)
		{
			if( skills[i].level < skills[skillIDs[i]].maxLevel )
			{
				temp.skillType = skills[skillIDs[i]].skillType;
				temp.level = skills[skillIDs[i]].level;
				ret.PushBack(temp);
				
				tempSkills.PushBack(skills[skillIDs[i]].skillType);
				
				cnt = skills[skillIDs[i]].maxLevel - skills[skillIDs[i]].level;
				for(j=0; j<cnt; j+=1)
					AddSkill(skills[skillIDs[i]].skillType, true);
			}
		}
		*/
		return ret;
	...
	// W3EE - Begin
	private var prevPoints : float;	default prevPoints = -1;
	...
	// W3EE - End
	{
		...
		// W3EE - Begin
		if( prevPoints == -1 )
			prevPoints = FactsQueryLatestValue("StoredVigor") / 1000;
		...
		if(points < 1 && owner.HasBuff(EET_BattleTrance) /*&& !(((W3PlayerWitcher)owner).IsMutationActive( EPMT_Mutation1 ))*/)
		{
	...
	protected final function OnVitalityChanged()
	{
		...
		if(vitPerc < 1.f)
			ResetOverhealBonus();
		// W3EE - Begin, Kolaris - Injury Effects
		//else ((CR4Player)owner).GetInjuryManager().ClearInjuries();
		//witcher = ((W3PlayerWitcher)owner);
		((W3PlayerWitcher)owner).UpdateWoundedState();
	...
	protected final function OnToxicityChanged()
	{
		...
		tox = GetStat(BCS_Toxicity, false);
	
		//Kolaris - Toxicity Rework
		if( tox == 0.f && owner.HasBuff( EET_Toxicity ) && ((W3Effect_Toxicity)owner.GetBuff(EET_Toxicity)).GetToxicityEntryCount() == 0 )
		{
		...
		else if( (W3PlayerWitcher)owner && GetWitcherPlayer().IsMutationActive( EPMT_Mutation10 ) && !owner.HasBuff( EET_Mutation10 ) )
		{
	...
	public final function IsSkillMutagenSlotUnlocked( eqSlot : EEquipmentSlots ) : bool
	{
		...
		// W3EE - Begin
		//Kolaris - Mutagen Slots, Kolaris - Acclimation
		return GetSkillLevel(S_Alchemy_s19) >= mutagenSlots[ i ].unlockedAtLevel || Options().AllMutagensUnlocked();
		// W3EE - End
	}
	...
	public function Glyphword48MutagenUpdate(toxEntries : int)
	{
		MutagensSyngergyBonusUpdate( -1, toxEntries );
	}
	...
	private final function MutagensSyngergyBonusUpdateSingle( skillGroupID : int, skillLevel : int )
	{
		...
		{			
			...
			// W3EE - Begin
			owner.GetInventory().GetItemStats(mutagenItemID, mutagenStats);
			...
			if( skillLevel > 0 )
			{
				...
				{
					if( mutagenStats[i].value < 1)
					{
						attributeValue = mutagenStats[i].value * 100 * skillLevel;
						current.count = RoundMath(attributeValue);
					}
					else
					{
						attributeValue = mutagenStats[i].value * skillLevel;
						current.count = RoundMath(attributeValue);
					}
				}
			}
	...
	public final function GetSkillStaminaUseCost(skill : ESkill, optional isPerSec : bool) : float
	{
		...
		reductionCounter = GetSkillLevel(skill);
	...
	public final function AddSkill(skill : ESkill, isTemporary : bool)
	{
		...
		if( MustEquipSkill(skill) )
			...
			UnequipSkill(GetSkillSlotID(skill), true);
		// W3EE - End
	}
	...
	protected final function AddSkillInternal(skill : ESkill, spendPoints : bool, isTemporary : bool, optional skipTutorialMessages : bool) : bool
	{
		...
		// W3EE - Begin
		Alchemy().OnSkillUpdated(skill, skills[skill].level);
		// W3EE - End
		...
		if(!isTemporary)
		{
			...
			// W3EE - Begin
			if(spendPoints)
				Experience().SpendSkillPoints(skill, skills[skill].cost);
	...
	public final function CanLearnSkill(skill : ESkill) : bool
	{
		...
		*/
		...
		if(Experience().GetCurrentPathPoints(GetSkillSubPathType(skill)) < skills[skill].cost)
			return false;
		// W3EE - End
	...
	event OnLevelGained(currentLevel : int)
	{
		...
		{
			// W3EE - Begin
			/*if(Experience().GetTotalSkillPoints() >= skillSlots[i].unlockedOnLevel || Options().AllSkillsUnlocked())
	...
	// W3EE - Begin
	public final function UnequipSkill(slotID : int, optional noForceEquip : bool) : bool
	{
		var idx : int;
		var prevColor : ESkillColor;
		var skill : ESkill;
	...
	// W3EE - End
		...
		{
			if( MustEquipSkill(skill, noForceEquip) )
				ForceEquipSkill(skill);
		}
		// W3EE - End
	...
	private final function OnSkillEquip(skill : ESkill)
	{
		...
		//Kolaris - Affinity
		/*
		if( skills[S_Alchemy_s18].level && Alchemy().GetHasToxicityChanged() )
			Alchemy().OnSkillUpdated(S_Alchemy_s18, skills[S_Alchemy_s18].level);
		*/
		/*if( MustEquipSkill(skill)) )
		{*/
			if ( CanUseSkill(S_Alchemy_s19) )
			{
				//MutagensSyngergyBonusUpdate( GetSkillGroupIdFromSkill( skill ), GetSkillLevel(S_Alchemy_s19) ); //Kolaris - Acclimation
			}
			...
			else if(skill == S_Sword_s03)		
			{
				charStats.AddAbilityMultiple(GetSkillAbilityName(skill), GetSkillLevel(skill));
			}
			//Kolaris - Footwork
			else if(skill == S_Sword_s09)		
			{
				charStats.AddAbilityMultiple(GetSkillAbilityName(skill), GetSkillLevel(skill));
			}
			else if(skill == S_Magic_s06)		
			{
				charStats.AddAbilityMultiple(GetSkillAbilityName(skill), GetSkillLevel(skill));
			}
			else if(skill == S_Magic_s09)		
			{
				charStats.AddAbilityMultiple(GetSkillAbilityName(skill), GetSkillLevel(skill));
			}
			else if(skill == S_Magic_s12)		
			{
				charStats.AddAbilityMultiple(GetSkillAbilityName(skill), GetSkillLevel(skill));
			}
			...
			else if(skill == S_Perk_13)		
			{
				charStats.AddAbility(GetSkillAbilityName(skill));
			}
			// W3EE - End
			...
			{
				//MutagensSyngergyBonusUpdate( -1, GetSkillLevel(S_Alchemy_s19) ); //Kolaris - Acclimation
			}
			...
			*/
			...
			{
				charStats.AddAbility(GetSkillAbilityName(skill), false);
		...
		*/
		...
		{*/
			if(skill == S_Magic_s11)
			{
				...
				// W3EE - Begin
				owner.RemoveAbilityMultiple(GetSkillAbilityName(skill), GetSkillLevel(skill));
				// W3EE - End
			}
			else if(skill == S_Magic_s07)
			{
			...
			{			
				//MutagensSyngergyBonusUpdate( -1, 0 ); //Kolaris - Acclimation
			}
			else if(skill == S_Perk_01)
			{
			...
			/*else if( skill == S_Perk_15 )
			{
				...
				((W3Effect_WellHydrated)owner.GetBuff(EET_WellHydrated)).OnPerk15Unequipped();
			}*/
			else if(skill == S_Perk_22)
			{
				charStats.RemoveAbility(GetSkillAbilityName(skill));
	...
	public final function ResetCharacterDev()
	{
		...
		{			
			...
			// if(IsSkillEquipped(skillType))
				UnequipSkill(GetSkillSlotID(skillType), true);
		...
		Alchemy().ResetSkills();
		Experience().ResetCharacterSkills();
		// W3EE - End
	...
	private final function OnMutationUnequippedPre( mutationType : EPlayerMutationType )
	{
		...
		{
			owner.RemoveBuff(EET_HarmonyAard);
			owner.RemoveBuff(EET_HarmonyAxii);
			owner.RemoveBuff(EET_HarmonyQuen);
			owner.RemoveBuff(EET_HarmonyIgni);
			owner.RemoveBuff(EET_HarmonyYrden);
		}
	...
	public final function MutationResearchWithSkillPoints( mutation : EPlayerMutationType, skillPoints : int ) : bool
	{
		...
		*/
		...
		mutReq = Experience().GetMutationPathPointTypes(mutation);
		for(i=0; i<mutReq.skillPaths.Size(); i+=1)
		{
			Experience().SpendSkillPointsMutation(mutReq.skillPaths[i], mutReq.requiredPoints[i]);
		}
		// W3EE - End
	...
	public final function MutationResearchWithItem( mutation : EPlayerMutationType, item : SItemUniqueId, optional quantity : int ) : bool
	{
		...
		baseMutagen = Equipment().GetBaseMutagenForMutation(mutation); //Kolaris - Mutation Rework
		mutationCostMult = Equipment().GetMutationCostMult(mutation, baseMutagen); //Kolaris - Mutation Rework
		...
		missingRed = RoundMath(mutations[ idx ].progress.redRequired * mutationCostMult) - mutations[ idx ].progress.redUsed; //Kolaris - Mutation Rework
		missingGreen = RoundMath(mutations[ idx ].progress.greenRequired * mutationCostMult) - mutations[ idx ].progress.greenUsed; //Kolaris - Mutation Rework
		missingBlue = RoundMath(mutations[ idx ].progress.blueRequired * mutationCostMult) - mutations[ idx ].progress.blueUsed; //Kolaris - Mutation Rework
	...
	// W3EE - Begin
	public final function MustEquipSkill(skill : ESkill, optional noForceEquip : bool ) : bool
	{
		if( IsCoreSkill(skill) || !HasLearnedSkill(skill) || noForceEquip )
			return false;
		else
			return true;
	}
	...
	public final function GetFreeSkillSlotIndex() : int
	{
		...
		Diff = orgTotalSkillSlotsCount - totalSkillSlotsCount;
		
		//Plastic - Skill Slot Bug Fix
		//Removed as it inflated slot id by increments much larger than needed
		/*
		for(i=0; i<Diff; i+=1)
		{
			...
			slot.unlockedOnLevel = 0;
			slot.groupID = MUTATION_SKILL_GROUP_ID;
			slot.unlocked = false;	
	...
	public final function ForceEquipSkill(skill : ESkill)
	{
		...
		//Plastic - Skill Slot Bug Fix
		//Added fix for slow loading char dev
		if(!IsSkillEquipped(skill))
		{
