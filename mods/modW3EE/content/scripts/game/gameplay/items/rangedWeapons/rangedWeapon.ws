...
{	
	...
	{
		if( runCheck && ownerPlayer.GetBehaviorVariable('isShootingWeapon') || ownerPlayer.IsReloading() )
		{
			ownerPlayer.SetCancelReload(true);
			runCheck = false;
		}
	...
	event OnWeaponReloadStart() 
	{
		...
		ownerPlayer.SetIsReloading(true);
		if ( !ownerPlayer.ShouldCancelReload() && GetWitcherPlayer().GetItemEquippedOnSlot(EES_Bolt, id) )
		{
	...
	event OnWeaponReload()
	{
		...
		if ( GetWitcherPlayer().GetItemEquippedOnSlot(EES_Bolt, id) )
		{
			ReloadWeapon( id );
		}
		else
		...
		ownerPlayer.SetIsReloading(false);
	}
	...
	event OnWeaponHolsterStart()
	{
		...
		ownerPlayer.SetCancelReload(false);
		ownerPlayer.SetIsReloading(false);
	}
...
class Crossbow extends RangedWeapon
{
	...
	{	
		...
		//setFullWeight = theInput.GetActionValue('GI_AxisLeftY') <= 0.f && ownerPlayer.playerAiming.GetCurrentStateName() == 'Aiming';
