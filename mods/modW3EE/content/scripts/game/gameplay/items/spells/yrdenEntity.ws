...
statemachine class W3YrdenEntity extends W3SignEntity
{
	...
	protected function GetSignStats()
	{
		...
		charges = 3;
		//Kolaris - Sustained Glyphs
		trapDuration = 40.f;
		if (owner.GetSkillLevel(S_Magic_s10, this) >= 1)
			trapDuration += owner.GetSkillLevel(S_Magic_s10, this) * 4;
		trapDuration *= sp.valueMultiplicative;
		//Kolaris - Prolongation
		if( ((W3PlayerWitcher)owner.GetPlayer()) )
			trapDuration *= ((W3PlayerWitcher)owner.GetPlayer()).GetPlayerSignDurationMod();
		// W3EE - End
	}
	...
	event OnStarted()
	{
		...
		else if( IsAlternateCast() )
		{
			CacheActionBuffsFromSkill();
			GotoState( 'YrdenShock' );
		}
		else
		{
			GotoState( 'YrdenCast' );
	...
	protected latent function Place(trapPos : Vector)
	{
		...
		{
			trigger = GetComponent( "Slowdown" );
			//Kolaris - Yrden Scaling
			scale = trigger.GetLocalScale() * (1.f * creatorPowerStatVal.valueMultiplicative + 0.1f * owner.GetSkillLevel(S_Magic_s10, this));
	...
	protected function UpdateGryphonSetBonusYrdenBuff()
	{
		...
		{*/
			...
			{
				if( player.IsSetBonusActive(EISB_Gryphon_1 ) )
					player.AddEffectDefault( EET_GryphonSetBonus, NULL, "GryphonSetBonus" );
			...
			else if( !isPlayerInYrden && hasBuff )
			{
				...
				player.RemoveBuff( EET_GryphonSetBonus, false, "GryphonSetBonus" );
			}
		/*}
		else if( hasBuff )
		{
...
state YrdenShock in W3YrdenEntity extends Active
{
	...
	event OnEnterState( prevStateName : name )
	{
		...
		//Kolaris - Yrden Scaling
		areaScale = triggerArea.GetLocalScale() * (1 * parent.creatorPowerStatVal.valueMultiplicative + 0.05f * caster.GetSkillLevel(S_Magic_s10, (W3SignEntity)parent));
		triggerArea.SetScale(areaScale);	
		ActivateShock();
		parent.NotifyGameplayEntitiesInArea( usedShockAreaName );
		
		//Kolaris - Invocation
		actors = GetActorsInRange(parent, VecLength2D(areaScale), 10,, true);
		for(i=0; i<actors.Size(); i+=1)
		{
			target = (CNewNPC)actors[i];
	...
	public function SetWardingGlyphTargets( target : CNewNPC )
	{
		if( target.CanGlyphHit() )
		{
			...
			target.AddTimer('StartGlyphCooldown', 6.5f - caster.GetSkillLevel(S_Magic_s03, (W3SignEntity)parent) / 2.f, false);
		}
	}
		...
		if ( results.Size() > 0 )
		{
			...
			while(results.Size() > 0)
			{
				...
				if(results[ind].component)
				{
					...
					// W3EE - Begin
					if( (targetActor && targetActor.GetHealth() > 0.f && targetActor.IsAlive() && (parent.validTargetsInArea.Contains(targetActor) || ((CNewNPC)targetActor).IsFlying())) || (!targetActor && entity) )
					// W3EE - End
					{
	...
	private final function YrdenTrapHitEnemy(entity : CEntity, hitPosition : Vector)
	{
		...
		if ( casterActor && (CGameplayEntity)entity)
		{
			...
			//Kolaris - Alternate Signs
			if(player /*&& skillLevel >= 1*/)
			{
				sp = parent.GetTotalSignIntensity();
				//Kolaris - Enervation
				if( player.HasAbility('Glyphword 17 _Stats', true) || player.HasAbility('Glyphword 18 _Stats', true) )
					damages.PushBack(SRawDamage(theGame.params.DAMAGE_NAME_ELEMENTAL, (250 + targetActor.GetStatMax(BCS_Vitality) * 0.02f + targetActor.GetStatMax(BCS_Essence) * 0.02f + 50 * caster.GetSkillLevel(S_Magic_s03, (W3SignEntity)parent)) * sp.valueMultiplicative));
				else
			...
			action.AddEffectInfo(EET_GlyphDebuff, 5 * player.GetPlayerSignDurationMod());
...
state YrdenSlowdown in W3YrdenEntity extends Active
{
	...
	private function CreateTrap()
	{
		...
		if( thePlayer.CanUseSkill(S_Magic_s10) )*/
			yrdenRange *= 1.f * parent.creatorPowerStatVal.valueMultiplicative + 0.1f * thePlayer.GetSkillLevel(S_Magic_s10);
