...
class W3Petard extends CThrowable
{
	...
	//Kolaris - Bomb Fuse Rework
	private var hasExploded							: bool;
	private var hasFuseTimer						: bool;
	private var delayedChargeStart					: float;
	private var delayedChargeTime					: float;
		...
		default hasFuseTimer = false;
		default delayedChargeStart = 0;
		default delayedChargeTime = 0;
	...
	protected function LoadDataFromItemXMLStats()
	{
		...
		for( i = 0; i < iSize; i += 1 )
		{
			...
			if(isLoopAbility && loopParams.damages.Size() > 0)
			{
				...
				dm.GetAbilityAttributeValue(abs[i], 'armorignored', min, max);
	...
	public function GetBombArmorPierce( isDoT : bool ) : float
	{
		if( isDoT )
	...
	{		
		...
		//Kolaris - Bomb Fuse Rework
		if( GetOwner() == thePlayer && !thePlayer.inv.ItemHasTag(itemId, 'Snowball') && !thePlayer.inv.ItemHasTag(itemId, 'Paintball') && !isCluster )
		{
			isProximity = true;
			//theGame.GetGuiManager().ShowNotification("True");
		}
		else
		{
			isProximity = false;
			//theGame.GetGuiManager().ShowNotification("False");
		}
		...
		{
			// W3EE - Begin
			if( !thePlayer.inv.ItemHasTag(itemId, 'Snowball') && !thePlayer.inv.ItemHasTag(itemId, 'Paintball') )
				Experience().AwardAlchemyUsageXP(thePlayer.inv.IsItemMutagenPotion(itemId), thePlayer.inv.IsItemPotion(itemId), thePlayer.inv.IsItemOil(itemId), thePlayer.inv.IsItemBomb(itemId));
				
			//Kolaris - Manticore Set
			/*if( GetWitcherPlayer().IsSetBonusActive(EISB_RedWolf_1) && GetWitcherPlayer().IsFreeBomb() )
			{
				GetWitcherPlayer().SetIsFreeBomb(false);
				GetWitcherPlayer().AddTimer('ManticoreBombCooldown', 120, false);
			}
			else*/ thePlayer.inv.SingletonItemRemoveAmmo(itemId, 1);
			
			if( !thePlayer.inv.GetItemModifierInt(itemId, 'ammo_current') )
			{
				if (GetWitcherPlayer().GetItemSlot(itemId) != EES_InvalidSlot)
					GetWitcherPlayer().UnequipItem(itemId);
				AddTimer('removeBombFromInventory', 0.1f, 0);
			}
			// W3EE - End
	...
	// W3EE - Begin
	timer function removeBombFromInventory (time : float, id : int)
	{
		thePlayer.inv.RemoveItem(itemId, 1);
		if( thePlayer.inv.GetItemQuantity(itemId) > 0 )
			AddTimer('removeBombFromInventory', 0.1f, false);
	}
	// W3EE - End
	...
	timer function ReleaseProjectile( time : float , id : int)
	{
		...
		if (isFromAimThrow)
		{
			angleInDegrees = thePlayer.playerAiming.GetThrowAngle();
			speed = thePlayer.playerAiming.GetThrowSpeed();
			actualTargetPos = thePlayer.playerAiming.GetThrowParabolicEnd();
		}
		else
		{
			angleInDegrees = 20.0f;
			speed = 15.0f;
		...
		if( target.HasTag('AddRagdollCollision'))
		{
			...
			// W3EE - Begin
			//Kolaris - Steady Aim
			if( GetOwner() == thePlayer && GetWitcherPlayer().CanUseSkill(S_Alchemy_s09) )
				ShootProjectileAtPosition( angleInDegrees, speed, actualTargetPos, (theGame.params.MAX_THROW_RANGE * (1 + 0.2f * GetWitcherPlayer().GetSkillLevel(S_Alchemy_s09) - 0.05f * GetWitcherPlayer().GetSkillLevel(S_Alchemy_s10) - 0.05f * GetWitcherPlayer().GetSkillLevel(S_Alchemy_s11))), collisionGroups);
			else
		...
		else
		{
			// W3EE - Begin
			if( GetOwner() == thePlayer && GetWitcherPlayer().CanUseSkill(S_Alchemy_s09) )
				ShootProjectileAtPosition( angleInDegrees, speed, actualTargetPos, (theGame.params.MAX_THROW_RANGE * (1 + 0.2f * GetWitcherPlayer().GetSkillLevel(S_Alchemy_s09) - 0.05f * GetWitcherPlayer().GetSkillLevel(S_Alchemy_s10) - 0.05f * GetWitcherPlayer().GetSkillLevel(S_Alchemy_s11))) );
			else
	...
	event OnProjectileCollision( pos, normal : Vector, collidingComponent : CComponent, hitCollisionsGroups : array< name >, actorIndex : int, shapeIndex : int )
	{
		...
		var npc : CNewNPC;
		var victim, lastVictim : CActor;
		var entity : CEntity;
		var rot : EulerAngles;
		var rotMatrix, m1, m2, m3 : Matrix;
		var reboundCollisionGroups : array<name>;
		var fuseTimer, randAngleOffset : float;
		var waterEntity : CEntity; 
		
		//theGame.witcherLog.AddMessage("Hit " + NameToString(hitCollisionsGroups[0]));
		
		if(stopCollisions && !isProximity)
			return true;
		//Kolaris - Bomb Fuse Rework
		else if(stopCollisions && (hitCollisionsGroups.Contains( 'Terrain' ) || hitCollisionsGroups.Contains( 'Static' )) )
		{
		...
		if(isProximity)
		{
			...
			if( !hasFuseTimer )
			{
				if( npc && npc.HasBuff(EET_Burning) )
					fuseTimer = 0.1f;
				else
					fuseTimer = 5.f + thePlayer.GetSkillLevel(S_Alchemy_s10) - MinF(thePlayer.playerAiming.GetAimTime(), 4.9f + thePlayer.GetSkillLevel(S_Alchemy_s10));
				
				if( fuseTimer >= 1.f && ((((W3PlayerWitcher)GetOwner()).GetRunewordInfusionType() == ST_Igni) || theInput.IsActionPressed('DistanceModifier')) && ((W3PlayerWitcher)GetOwner()).GetStat(BCS_Focus) > 0.5f )
				{
					fuseTimer = 0.1f;
					((W3PlayerWitcher)GetOwner()).DrainFocus(0.5f);
				}
				
				AddTimer( 'DetonationTimer', fuseTimer, , , , false );
				delayedChargeStart = theGame.GetEngineTimeAsSeconds();
			...
			{
				stopCollisions = true;
				AddTimer('DelayedRestoreCollisions', 0.1f);	
				
				if( npc )
				{
					npc.AddEffectDefault(EET_Stagger, thePlayer);
					reboundCollisionGroups.PushBack('Terrain');
					reboundCollisionGroups.PushBack('Static');
					reboundCollisionGroups.PushBack('Water');
					randAngleOffset = RandRangeF( 10, -10 );
					ShootProjectileAtPosition( randAngleOffset, 5, npc.GetWorldPosition(), 5, reboundCollisionGroups);
				}
				else
				{
					bounceOfVelocityPreserve = 0.4;
					BounceOff( normal, pos );
				}
	...
	{
		if(isProximity && !hasExploded && interactionComponentName == "ProximityActivationArea" && IsRequiredAttitudeBetween(GetOwner(), activator, true))
			ProcessEffect();	
	}
	...
	public function ProcessEffect( optional explosionPosition : Vector, optional collidedTarget : CGameplayEntity )
	{
		...
		if(!isCluster && !ignoreBombSkills && (W3PlayerWitcher)GetOwner() && GetWitcherPlayer().CanUseSkill(S_Alchemy_s11) && !thePlayer.inv.ItemHasTag(itemId, 'Snowball') && !thePlayer.inv.ItemHasTag(itemId, 'Paintball'))
		{
			ProcessClusterBombs();
			// W3EE - Begin
			//return;
	...
	protected function ProcessLoopEffect()
	{
		...
		if( GetOwner() == thePlayer && GetWitcherPlayer().CanUseSkill(S_Alchemy_s10) )
			loopDuration *= (1 + delayedChargeTime * 0.1f);
		AddTimer('OnTimeEnded', loopDuration, false, , , true);
		// W3EE - Begin
		AddTimer('Loop', 1.f, true, , , true);	
		// W3EE - End
	}
	...
	{			
		var i, index, j, k, perk8Level: int;
		...
		// W3EE - Begin
		var damageMult : float = 1.f;
		var reductionValue : float;
		var steelBonusAdded, silverBonusAdded : bool;
		var effectName : name;
		// W3EE - End
		//Kolaris - Delayed Charge
		var durationMult : float = 1.f;
		var poiseEffect : W3Effect_NPCPoise; //Kolaris - Mutation 6
		
		perk8Level = thePlayer.GetSkillLevel(S_Alchemy_s08);
		
		for(i=targets.Size()-1; i>=0; i-=1)
		{
			//Kolaris - Bomb Fuse Rework
			if( (CActionPoint)targets[i] || ((W3Petard)targets[i]).IsCluster() )
			{
				targets.Erase(i);
				continue;
			}
		...
		if( isImpact )
			delayedChargeTime = MinF(10.f, theGame.GetEngineTimeAsSeconds() - delayedChargeStart);
		...
		/*if(isImpact)
		{
			if( !ignoreBombSkills && (W3PlayerWitcher)GetOwner() && GetWitcherPlayer().CanUseSkill(S_Alchemy_s10) && !HasTag('Snowball') && !HasTag('Paintball') && !isCluster)
			{
				if( params.damages.Size() )
				{
					steelBonusAdded = false;
					silverBonusAdded = false;
					for(j=0; j<params.damages.Size(); j+=1)
					{
						if( IsDamageTypeNameValid(params.damages[j].dmgType) )
						{
							if( DamageHitsVitality(params.damages[j].dmgType) && !DamageHitsEssence(params.damages[j].dmgType) )
							{
								if( steelBonusAdded )
									continue;
								params.damages[j].dmgVal *= 1.f + 0.1f * delayedChargeTime;
								steelBonusAdded = true;
							}
							else
							if( DamageHitsEssence(params.damages[j].dmgType) && !DamageHitsVitality(params.damages[j].dmgType) )
							{
								if( silverBonusAdded )
									continue;
								params.damages[j].dmgVal *= 1.f + 0.1f * delayedChargeTime;
				...
				for(j=0; j<atts.Size(); j+=1)
				{
					if(IsDamageTypeNameValid(atts[j]))
					{
						...
						if(index != -1)
						{
							params.damages[index].dmgVal += 100 * thePlayer.GetSkillLevel(S_Alchemy_s10);
						}
						else
						{
							newDamage.dmgType = atts[j];
							newDamage.dmgVal = 100 * thePlayer.GetSkillLevel(S_Alchemy_s10);
							params.damages.PushBack(newDamage);
						}
					}
				}
		...
		//Kolaris - Delayed Charge
		if( (W3PlayerWitcher)GetOwner() && GetWitcherPlayer().CanUseSkill(S_Alchemy_s10) )
		{
			damageMult *= 1.0f + 0.1f * delayedChargeTime;
			durationMult *= 1.0f + 0.1f * delayedChargeTime;
		}
		
		//Kolaris - Manticore Set
		if( (W3PlayerWitcher)GetOwner() && GetWitcherPlayer().IsSetBonusActive(EISB_RedWolf_1) )
			damageMult *= 1.f + 0.06f * GetWitcherPlayer().GetSetPartsEquipped(EIST_RedWolf);
		...
		*/
		...
		{	
			...
			npc = (CNewNPC)actorTarget; //W3EE
			
			//Kolaris - Bomb Fuse Rework
			npc.SignalGameplayEvent( 'HitByBomb' );	
			
			if( itemName == 'Grapeshot 1' || itemName == 'Grapeshot 2' || itemName == 'Grapeshot 3' )
			{
				if( npc && npc.IsShielded(this) )
					npc.ProcessShieldDestruction();
			}
			
			//Kolaris - Mutation 6
			if( ((W3PlayerWitcher)GetOwner()).IsMutationActive(EPMT_Mutation6) )
			{
				poiseEffect = (W3Effect_NPCPoise)npc.GetBuff(EET_NPCPoise);
				damageMult *= 1.f + 0.5f * (1.f - poiseEffect.GetPoisePercentage());
			}
			...
			for(j=0; j<params.damages.Size(); j+=1)
			{
				// W3EE - Begin
				if(dt > 0)
					action.AddDamage(params.damages[j].dmgType, params.damages[j].dmgVal * damageMult * dt);
				else
					action.AddDamage(params.damages[j].dmgType, params.damages[j].dmgVal * damageMult);
				//Kolaris - Tiger Set
				if( actorTarget && perk8Level && (!npc.HasBeenHitByBomb() || ((GetWitcherPlayer().IsSetBonusActive(EISB_Tiger_2) && !isCluster))) )
				{
					reductionValue = (0.06f * perk8Level * (1.f + 0.1f * delayedChargeTime));
					npc.MultiplyTypeResistBy(params.damages[j].dmgType, reductionValue);
				}
			...
			for(j=0; j<params.buffs.Size(); j+=1)
			{
				...
				//W3EE - Begin
				if( !isCluster )
					action.AddEffectInfo(params.buffs[j].effectType, params.buffs[j].effectDuration * durationMult, params.buffs[j].effectCustomValue, params.buffs[j].effectAbilityName, params.buffs[j].effectCustomParam, params.buffs[j].applyChance);
				//Kolaris - Tiger Set
				if( actorTarget && perk8Level && (!npc.HasBeenHitByBomb() || ((GetWitcherPlayer().IsSetBonusActive(EISB_Tiger_2) && !isCluster))) )
				{
					reductionValue = (0.06f * perk8Level * (1.f + 0.1f * delayedChargeTime));
					effectName = EffectTypeToName(params.buffs[j].effectType);
					npc.MultiplyTypeResistBy( effectName, reductionValue);
				}
				//W3EE - End
			}
	...
	private function ProcessClusterBombs()
	{
		...
		// W3EE - Begin
		// Kolaris - Cluster Bombs, Kolaris - Advanced Maintenance
	...
	//Kolaris - Bomb Injuries
	public function GetBombName() : name
	{
		return itemName;
	}
