...
EES_Unused,
	...
	EES_Potion4,
	// -= WMK:modQuickSlots =-
	// Same order as the version for patch 1.12.
	EES_Potion5,
	EES_Potion6,
	EES_Potion7,
	EES_Potion8,
...
function SlotEnumToName(slot : EEquipmentSlots) : name
{
	...
	// -= WMK:modQuickSlots =- (unused?)
	else if(slot == EES_Potion5) return 'EES_Potion5';
	else if(slot == EES_Potion6) return 'EES_Potion6';
	else if(slot == EES_Potion7) return 'EES_Potion7';
	else if(slot == EES_Potion8) return 'EES_Potion8';
	else if(slot == EES_Petard3) return 'EES_Petard3';
	else if(slot == EES_Petard4) return 'EES_Petard4';
	// -= WMK:modQuickSlots =-
...
{
	return IsSlotPetardslot(slot) || slot == EES_Quickslot1 || slot == EES_Quickslot2 || IsSlotPotionSlot(slot) // -= WMK:modQuickSlots =-
...
{
	import var uniqueId 			: SItemUniqueId;
	import var craftedAbilities 	: array<name>;
}

import struct SItemUniqueId
{
	import var value : Uint32;
}
// W3EE - End
...
function GetSlotForItem(category : name, tags : array<name>, isPlayer : bool) : EEquipmentSlots
{
	...
	else if(tags.Contains('Decoction'))				return EES_PotionMutagen1;
...
{
	return slot == EES_Potion1 || slot == EES_Potion2 || slot == EES_Potion3 || slot == EES_Potion4 || slot == EES_Potion5 || slot == EES_Potion6 || slot == EES_Potion7 || slot == EES_Potion8; // -= WMK:modQuickSlots =-
}


function IsSlotPetardslot(slot : EEquipmentSlots) : bool
{
	return slot == EES_Petard1 || slot == EES_Petard2 || slot == EES_Petard3 || slot == EES_Petard4; // -= WMK:modQuickSlots =-
}
