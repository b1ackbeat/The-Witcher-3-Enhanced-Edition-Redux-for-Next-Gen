...
class CBaseGameplayEffect extends CObject
{
	...
	// W3EE - Begin
	protected saved var resistStat : ECharacterDefenseStats;
	protected saved var resistance : float;
	protected saved var resistancePts : float;
	protected saved var isDecoctionEffect : bool;
	
	default isDecoctionEffect = false;
	// W3EE - End 						
	...
	// W3EE - Begin
	public var creator : CGameplayEntity;
	// W3EE - End
	public function Init(params : SEffectInitInfo)
	{
		...
		// W3EE - Begin
		if( !EntityHandleGet(creatorHandle) )
		...
		if( IsSignEffect() )
		{
			resistance = MinF(0, resistance);
			resistancePts = MinF(0, resistancePts);
		}
		// W3EE - End
	...
	protected function CalculateDuration(optional setInitialDuration : bool)
	{
		...
		if(isNegative)
		{
			...
			// W3EE - Begin
			initialDuration *= 1 + (creatorPowerStat.valueMultiplicative - 1)/2 - resistancePts/100;
			duration = MaxF(0, MaxF(0, initialDuration) * (1 - durationResistance));
	...
	event OnEffectAddedPost()
	{
		...
		if(target == thePlayer.GetTarget())
		{
			...
			if(localizationKey != "")
				target.ShowFloatingValue(EFVT_Buff, 0.f, false, localizationKey, , GetEffectType());
		}
	}
	...
	event OnEffectAdded(optional customParams : W3BuffCustomParams)
	{
		...
		if(isOnPlayer && switchCameraEffect && isCameraEffectNameValid  && !(cameraEffectName == 'critical_toxicity') )
		{
		...
		if(isPotionEffect)
		{
			...
			// W3EE - Begin
			if( !thePlayer.HasBuff(EET_ToxicityFever) )
			{
				customEffect.effectType = EET_Drunkenness;
				customEffect.sourceName = potionItemName;
				customEffect.duration = Options().GetPotionDrunkDuration();
				
				if( customEffect.duration > 0 )
					thePlayer.AddEffectCustom(customEffect);			
			}
			// W3EE - End
		}
	...
	{
 		if(isCameraEffectNameValid && isOnPlayer && switchCameraEffect && !(cameraEffectName == 'critical_toxicity') )
		{
	...
	// W3EE - Begin
	public function IsDecoctionEffect() : bool
	{
		return isDecoctionEffect;
	}
	// W3EE - End
	...
	public final function RecalcPotionDuration()
	{
		...
		newDuration = GetWitcherPlayer().CalculatePotionDuration(GetInvalidUniqueId(), isDecoctionEffect, potionItemName);
