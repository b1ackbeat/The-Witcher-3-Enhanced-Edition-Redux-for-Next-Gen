...
class W3Effect_Aerondight extends CBaseGameplayEffect
{
	...
	private function LoadParams()
	{
		...
		target.GetInventory().SetItemModifierFloat( target.GetInventory().GetCurrentlyHeldSword(), 'PermDamageBoost', 0.f );
	}
	...
	public function IncreaseAerondightCharges( attackName : name )
	{
		...
		if( m_currCount < m_maxCount && !m_wasDischarged )
		{
			...
			if( !Options().DisableSpecialSwordFx() )
				UpdateAerondightFX();
		}
	}
	...
	{	
		...
		l_levelDiff = 15; //RoundMath(target.GetLevel() / 1.5);
		// W3EE - End
