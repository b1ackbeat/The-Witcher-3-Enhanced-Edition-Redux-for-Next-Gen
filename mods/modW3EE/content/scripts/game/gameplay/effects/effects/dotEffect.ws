...
{	
	...
	// W3EE - Begin
	private var sleepTime : float;	default sleepTime = 0;
	...
	{	
		...
		{
			dmg = CalculateDamage(i, maxVit, maxEss, 1.f);
	...
	{
		if( sleepTime > 0 )
			DealPartialDamage(sleepTime);
		
		super.OnEffectRemoved();
	}
	
	private function DealPartialDamage( remainingDT : float )
	{
		var dmg, maxVit, maxEss : float;
		var i : int;
		...
		for(i=0; i<damages.Size(); i+=1)
		{
			dmg = CalculateDamage(i, maxVit, maxEss, remainingDT);
			if( (!damages[i].hitsVitality && !damages[i].hitsEssence) || dmg <= 0)
			{
				LogAssert(false, "W3DamageOverTimeEffect: effect " + this + " on " + target.GetReadableName() + " is dealing no damage");
			}
			...
			{
				effectManager.CacheDamage(damages[i].damageTypeName, dmg, GetCreator(), this, remainingDT, true, powerStatType, isEnvironment);		
			}
		}
	}
	// W3EE - End
	...
	protected function IsImmuneToAllDamage(dt : float) : bool
	{
		...
		for(i=0; i<damages.Size(); i+=1)
		{
			...
			if( target == thePlayer )
				...
				percents = ((CNewNPC)target).GetNPCCustomStat(damages[i].damageTypeName);
			// W3EE - End
