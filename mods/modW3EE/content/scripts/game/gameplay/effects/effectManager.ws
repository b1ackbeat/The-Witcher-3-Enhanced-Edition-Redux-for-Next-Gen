...
class W3EffectManager
{
	...
	public final function PerformUpdate(deltaTime : float)
	{
		...
		if( size > 0 )
		{
			...
			for(i=0; i<size; i+=1)
			{
				...
				//Kolaris - Viper Set
				if(carrier && carrier.GetEffectType() == EET_Poison)
				{
					((W3Effect_Poison)carrier).OnDamageDealt( action.DealsAnyDamage() );
				}
				//Kolaris - Griffin Set
				if(carrier && carrier.GetEffectType() == EET_YrdenHealthDrain)
				{
					((W3Effect_YrdenHealthDrain)carrier).OnDamageDealt( action.DealsAnyDamage() );
				}
			}
	...
	private final function ApplyEffect( effect : CBaseGameplayEffect, overridenEffectsIdxs : array<int>, cumulateIdx : int, customParams : W3BuffCustomParams) : EEffectInteract
	{
		...
		else
		{
			...
			//Kolaris - Conjunction
			if( Combat().DoesAxiiLinkContainActor(owner) )
			{
				Combat().ProcessConjunctionEffects(effect, owner);
			}
	...
	private final function InternalAddEffect(effectType : EEffectType, creat : CGameplayEntity, srcName : string, optional inDuration : float, optional customVal : SAbilityAttributeValue, optional customAbilityName : name, optional customFXName : name, optional signEffect : bool, optional powerStatValue : SAbilityAttributeValue, optional customParams : W3BuffCustomParams, optional vibratePadLowFreq : float, optional vibratePadHighFreq : float) : EEffectInteract
	{
		...
		if(effect)
		{
			...
			if( (hasQuen || (((W3PlayerWitcher)owner) && FactsQuerySum("player_had_quen") > 0)) && IsDoTEffect(effect))
			{
				...
				//GetWitcherPlayer().FinishQuen(false); //Kolaris - DoTs Cancel Quen
				// W3EE - End
				return EI_Deny;
			}
			...
			// W3EE - Begin
			//Kolaris - Lethargy
			if(signEffect && (effectType == EET_Confusion || effectType == EET_Hypnotized) && creat == thePlayer && effect.GetDurationLeft() < 0.9f)
			// W3EE - End
			{				
				LogEffects("EffectManager.InternalAddEffect: Axii effect is blocked, stagger instead");
				return EI_Deny;
			}
	...
	public final function GetDrunkDecoctions( optional sourceName : string ) : array<CBaseGameplayEffect>
	{
		var i : int;
		var ret : array<CBaseGameplayEffect>;
		var decoction : W3Decoction_Effect;
		
		for(i=0; i<effects.Size(); i+=1)
		{
			decoction = (W3Decoction_Effect)effects[i];
			if(decoction)
			{
				if( sourceName == "" || decoction.GetSourceName() == sourceName )
				{
					ret.PushBack(decoction);
				}
			}
		}
	...
	public final function RemoveEffect(effect : CBaseGameplayEffect, optional csForcedRemove : bool)
	{
		...
		/*
		if( effect.IsPotionEffect() || effect.IsDecoctionEffect() )
			((W3Effect_Toxicity)GetWitcherPlayer().GetBuff(EET_Toxicity)).ClearEffectOfType(effect.GetEffectType(), (effect.GetDurationLeft() <= 0) );
		
		if( effect == W3EffectToxicity )
			((W3Effect_Toxicity)GetWitcherPlayer().GetBuff(EET_Toxicity)).ClearToxicityEffect(effects);
		*/	
	...
	// W3EE - Begin
	public final function CountEffectsOfType( type : EEffectType ) : int
	{
		var i, count : int;
	...
	public final function HasDecoctionEffect() : bool
	{
		var i : int;
		for(i=0; i<effects.Size(); i+=1)
		{
			if( effects[i].IsDecoctionEffect() )
				return true;
		}
	...
	public final function RemoveAllNonAutoEffects( optional removeOils : bool, optional skipPerk14 : bool )
	{
		...
		if(!autos.Contains(EET_CombatAdr))
			autos.PushBack(EET_CombatAdr);
		if(!autos.Contains(EET_Poise))
			autos.PushBack(EET_Poise);
	...
	public final function AddEffectsFromAction( action : W3DamageAction ) : bool
	{
		...
		// W3EE - Begin
		var attackerPowerStatValue, effectPowerStatMin, effectPowerStatMax : SAbilityAttributeValue;
		var retB, applyBuff, forceEffect : bool;
		...
		if( size && action.attacker.HasAbility('mon_basilisk') && !action.DealsAnyDamage() )
			return false;
		...
		{		
			...
			if( (W3Petard)action.causer || action.GetBuffSourceName() == "petard" )
				forceEffect = true;
			...
			else
			{
				...
				applyBuff = forceEffect || GetNonSignApplyBuffTest(effectInfos[i].applyChance, effectInfos[i].effectType, (CActor)action.attacker);
				// W3EE - End
			}
			...
			{
				// W3EE - Begin
				if( effectInfos[i].effectType == EET_Poison )
				{
					theGame.GetDefinitionsManager().GetAbilityAttributeValue(effectInfos[i].effectAbilityName, 'PoisonDamage', effectPowerStatMin, effectPowerStatMax);
					owner.ApplyPoisoning(RoundMath(MaxF(1, RandRangeF(effectPowerStatMax.valueMultiplicative, effectPowerStatMin.valueMultiplicative))), action.attacker, "Poisoning", true);
					continue;
				}
				
				if( effectInfos[i].effectType == EET_Bleeding )
				{
					theGame.GetDefinitionsManager().GetAbilityAttributeValue(effectInfos[i].effectAbilityName, 'DirectDamage', effectPowerStatMin, effectPowerStatMax);
					owner.ApplyBleeding(RoundMath(MaxF(1, RandRangeF(effectPowerStatMax.valueMultiplicative, effectPowerStatMin.valueMultiplicative))), action.attacker, "Bleeding", true);
	...
	private final function GetNonSignApplyBuffTest(applyChance : float, effectType : EEffectType, actorAttacker : CActor) : bool
	{
		...
		*/
		...
		if( owner == thePlayer )
			...
			resPrc = ((CNewNPC)owner).GetNPCCustomStat(((CNewNPC)owner).ResistStatToName(theGame.effectMgr.GetBuffResistStat(effectType)));
	...
	{
		// W3EE - Begin
		var sp, resPt, res, chance, tempF, rawChance, bonusChance : float;
		var chanceBonus : SAbilityAttributeValue;
		var witcher : W3PlayerWitcher;
		
		witcher = (W3PlayerWitcher)caster;
		
		if(signType == ST_Yrden || signType == ST_Aard)
		{
			return true;
		}
		...
		{
			if(witcher.GetSignOwner().CanUseSkill(S_Magic_s07, witcher.GetSignEntity(ST_Igni)))
			{
				// chanceBonus = witcher.GetSkillAttributeValue(S_Magic_s09, 'chance_bonus', false, true);
				bonusChance = 0.05f * witcher.GetSignOwner().GetSkillLevel(S_Magic_s07, witcher.GetSignEntity(ST_Igni)) * sp;
			}
		...
		{
			rawChance += 0.3f;
			chance = MaxF(0, rawChance + bonusChance - resPt/100) * (1 - res) * (1 - (owner.GetBurnCounter() * 0.1f));
		}
		/*else
		...
		if( signType == ST_Igni )
			chance = MaxF(0, rawChance + bonusChance - resPt/100) * (1 - res) * (1 - (owner.GetBurnCounter() * 0.1f) ) / 5;
		else
			chance = MaxF(0, (rawChance - resPt/100)) * (1 - res);
		
		if(witcher && (witcher.GetPotionBuffLevel(EET_PetriPhiltre) == 3 || ((W3Effect_NPCPoise)owner.GetBuff(EET_NPCPoise)).IsPoiseBroken()) && res < 1)
		{
			chance = 1;
		}
