...
import statemachine class W3ToxicCloud extends CGameplayEntity
{
	...
	event OnAreaExit( area : CTriggerAreaComponent, activator : CComponent )
	{
		...
		if( (CR4Player)ent )
		{
			theGame.GetTutorialSystem().MarkMessageAsSeen('TutorialLeaveGas');
			theGame.GetTutorialSystem().HideTutorialHint('TutorialLeaveGas');
		}
	...
	private var poisonTimer : float;	default poisonTimer = 0.f;
	timer function PoisonTimer(dt : float, id : int)
	{
		var i : int;
		
		if( buffParams.effectType == EET_Poison )
		{
			poisonTimer += dt;
			if( poisonTimer <= 2.f )
				return;
		...
		for(i=0; i<entitiesInPoisonRange.Size(); i+=1)
		{
			...
			((CActor)entitiesInPoisonRange[i]).ApplyPoisoning(1, this, "Poisoning");
		}	
	}
...
state Armed in W3ToxicCloud
{
	...
	event OnEnterState( prevStateName : name )
	{
		...
		// W3EE - Begin
		fireSource = FindOpenFireSources();
		if( fireSource )
			Explode(fireSource);
		// W3EE - End
	...
	// W3EE - Begin
	private function FindOpenFireSources() : CEntity
	{
		var entities : array<CGameplayEntity>;
		var lightComponent : CGameplayLightComponent;
		var i : int;
		
		FindGameplayEntitiesInRange(entities, parent, 3.f, 50,, FLAG_ExcludePlayer,, 'CGameplayEntity');
		for(i=0; i<entities.Size(); i+=1)
		{
			lightComponent = (CGameplayLightComponent)entities[i].GetComponentByClassName('CGameplayLightComponent');
			if( lightComponent && lightComponent.IsLightOn() )
				return entities[i];
		}
