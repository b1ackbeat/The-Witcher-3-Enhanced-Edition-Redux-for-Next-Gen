...
statemachine class CMonsterNestEntity extends CInteractiveEntity
{
	...
	editable var expOnNestDestroyed							: int; default expOnNestDestroyed = 20;
															// W3EE - Begin
	editable var bonusExpOnBossKilled						: int; default expOnNestDestroyed = 20;
															// W3EE - End
	...
	event OnFireHit(source : CGameplayEntity)
	{
		if( !wasExploded )
		{
			...
			if ( IsBossProtectingNest() )
				AddTimer('SpawnBoss', nestUpdateDefintion.bossSpawnDelay, false, , , true );
	...
	function PlayerHasBombActivator() : bool
	{
		...
		if( GetWitcherPlayer().GetItemEquippedOnSlot(EES_Petard1, tempItem) && bombActivators.Contains(playerInventory.GetItemName(tempItem)) && playerInventory.SingletonItemGetAmmo(tempItem) > 0 )
		{
			usedBomb = tempItem;
			return true;
		}
		
		if( GetWitcherPlayer().GetItemEquippedOnSlot(EES_Petard2, tempItem) && bombActivators.Contains(playerInventory.GetItemName(tempItem)) && playerInventory.SingletonItemGetAmmo(tempItem) > 0 )
		{
			usedBomb = tempItem;
			return true;
		}
		
		if( GetWitcherPlayer().GetItemEquippedOnSlot(EES_Petard3, tempItem) && bombActivators.Contains(playerInventory.GetItemName(tempItem)) && playerInventory.SingletonItemGetAmmo(tempItem) > 0 )
		{
			usedBomb = tempItem;
			return true;
		}
		
		if( GetWitcherPlayer().GetItemEquippedOnSlot(EES_Petard4, tempItem) && bombActivators.Contains(playerInventory.GetItemName(tempItem)) && playerInventory.SingletonItemGetAmmo(tempItem) > 0 )
		{
			usedBomb = tempItem;
			return true;
		}
...
state SettingExplosives in CMonsterNestEntity
{
	...
	{	
		...
		// W3EE - Begin	
		Experience().AwardAlchemyUsageXP(thePlayer.inv.IsItemMutagenPotion(parent.usedBomb), thePlayer.inv.IsItemPotion(parent.usedBomb), thePlayer.inv.IsItemOil(parent.usedBomb), thePlayer.inv.IsItemBomb(parent.usedBomb));
		...
		else parent.playerInventory.SingletonItemRemoveAmmo(parent.usedBomb, 1);
		
		if (!parent.playerInventory.GetItemModifierInt(parent.usedBomb, 'ammo_current') )
		{
			if (GetWitcherPlayer().GetItemSlot(parent.usedBomb) != EES_InvalidSlot)
				GetWitcherPlayer().UnequipItem(parent.usedBomb);			
			
			parent.playerInventory.RemoveItem(parent.usedBomb, 1);
			while( parent.playerInventory.GetItemQuantity(parent.usedBomb) > 0 )
			{
				parent.playerInventory.RemoveItem(parent.usedBomb, 1);
				Sleep(0.25f);
			}			
