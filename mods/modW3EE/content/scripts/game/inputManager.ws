...
import class CInputManager
{
	...
	public final function GetLastUsedGamepadType() : EInputDeviceType
	{
		...
		//Kolaris - Control Hints Override
		if(Options().ControlHintOverride() == 1)
			return IDT_KeyboardMouse;
		else if (Options().ControlHintOverride() == 2)
			return IDT_Steam;
		else if (Options().ControlHintOverride() == 3)
			return IDT_Xbox1;
		else if (Options().ControlHintOverride() == 4)
			return IDT_PS4;
	...
	event OnInputDeviceChanged()
	{
		...
		{			
			...
			if (WmkGetQuickSlotsInstance()) {
				WmkGetQuickSlotsInstance().UpdateInputDevice();
			}
			// -= WMK:modQuickSlots =-
		}
	}
