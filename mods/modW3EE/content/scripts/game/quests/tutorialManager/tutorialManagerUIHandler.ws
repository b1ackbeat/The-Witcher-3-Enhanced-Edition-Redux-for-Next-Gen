...
statemachine class W3TutorialManagerUIHandler
{
	...
	public final function AddStashTutorial()
	{
		var tutorial : SUITutorial;
		
		tutorial.menuName = 'InventoryMenu';
		tutorial.tutorialStateName = 'Stash';
		tutorial.triggerCondition = EUITTC_OnMenuOpen;
		tutorial.priority = 20;
		tutorial.abortOnMenuClose = true;
		RegisterUIHint(tutorial);
	}
	
	
	private function HandleListeners(menuName : name, isOpened : bool) : bool
	{
		var i, j, factVal, chosenIndex, minPriority 	: int;
		...
		for(i=0; i<listeners.Size(); i+=1)
		{
			...
			if(menuName == 'InventoryMenu' && listeners[i].tutorialStateName != 'Stash' && theGame.GameplayFactsQuerySum("stashMode") > 0)
				continue;
				
			if(menuName == 'InventoryMenu' && listeners[i].tutorialStateName == 'Stash' && theGame.GameplayFactsQuerySum("stashMode") < 1)
				continue;
			...
			{
				if( !GetWitcherPlayer().IsMeditating() || GetWitcherPlayer().GetNumMutagenSlotsUnlocked() < 1 )
					continue;
					
				arr = thePlayer.inv.GetItemsByTag('MutagenIngredient');
				for(j=0; j<arr.Size(); j+=1)
				{
					if( !thePlayer.inv.ItemHasTag(arr[i], 'InertMutagen') )
					{
						found = true;
						break;
					}
			...
			if( listeners[i].tutorialStateName == 'CharacterDevelopment' && !GetWitcherPlayer().IsMeditating() )
				continue;
	...
	event OnClosingMenu(menuName : name)
	{
		...
		if( menuName == 'InventoryMenu' && theGame.GameplayFactsQuerySum("stashMode") < 1 )
		{
