...
state CharacterDevelopment in W3TutorialManagerUIHandler extends TutHandlerBaseState
{
	...
	event OnLeaveState( nextStateName : name )
	{
		...
		theGame.GetTutorialSystem().MarkMessageAsSeen(OPEN_CHAR_DEV);
		theGame.GetTutorialSystem().MarkMessageAsSeen(LEVELING);
		theGame.GetTutorialSystem().MarkMessageAsSeen(SKILLS);
		theGame.GetTutorialSystem().MarkMessageAsSeen(BUY_SKILL);
		theGame.GetTutorialSystem().MarkMessageAsSeen(SKILL_EQUIPPING);
		theGame.GetTutorialSystem().MarkMessageAsSeen(EQUIP_SKILL);
		theGame.GetTutorialSystem().MarkMessageAsSeen(GROUPS);
