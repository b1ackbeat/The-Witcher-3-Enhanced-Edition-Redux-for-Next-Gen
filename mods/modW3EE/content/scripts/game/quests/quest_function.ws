...
{
	// W3EE - Begin
	GetWitcherPlayer().StartW3EEMeditation();
	// W3EE - End
}
		...
		if(entity)
		{
			...
			if(inv)
			{
				...
				if(!skip)
				{
					...
					// W3EE - Begin
					if( itemName == 'Tutorial Bomb' || itemName == 'Snow Ball' )
						quantity = 99;
					// W3EE - End
					
					//Kolaris - Paintball Fix
					if( itemName == 'q703_paint_bomb_red' || itemName == 'q703_paint_bomb_yellow' || itemName == 'q703_paint_bomb_green' || itemName == 'q703_paint_bomb_blue' || itemName == 'q703_paint_bomb_purple' )
						quantity = 3;
...
{
	var i : int;
	var hasWeapon, disableAutoSheathe : bool;
	var inv : CInventoryComponent;
	var items : array<SItemUniqueId>;
	//Kolaris - Manual Sword Drawing
	var item : SItemUniqueId;
	...
	else if((W3PlayerWitcher)thePlayer)
	{
		if(weapon == EDWQT_Silver)
		{
			...
			if( disableAutoSheathe && !(inv.ItemHasTag(item, 'Quest')) )
				return;
...
function OutOfMemoryHack_Level0Items(out items : array<SGwentIngDef>)
{
	...
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 5;
	itemDef.quantityMax = 8;
	...
	itemDef.itemName = 'Twine';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 3;
	itemDef.quantityMax = 5;
	items.PushBack(itemDef);
	
	itemDef.itemName = 'Rope';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 1;
	itemDef.quantityMax = 1;
	items.PushBack(itemDef);
	...
	itemDef.itemName = 'Copper mineral';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 5;
	itemDef.quantityMax = 8;
	items.PushBack(itemDef);
	
	itemDef.itemName = 'Copper ore';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 1;
	itemDef.quantityMax = 2;
	items.PushBack(itemDef);
...
function OutOfMemoryHack_Level10Items(out items : array<SGwentIngDef>)
{
	...
	itemDef.itemName = 'Cherry Cordial';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 2;
	itemDef.quantityMax = 4;
	items.PushBack(itemDef);
	
	itemDef.itemName = 'Dwarven spirit';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 1;
	itemDef.quantityMax = 1;
	items.PushBack(itemDef);
	
	itemDef.itemName = 'Mahakam Spirit';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 2;
	itemDef.quantityMax = 4;
	items.PushBack(itemDef);
	
	itemDef.itemName = 'Nilfgaardian Lemon';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 2;
	itemDef.quantityMax = 4;
	items.PushBack(itemDef);
	
	itemDef.itemName = 'Local pepper vodka';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 1;
	itemDef.quantityMax = 1;
	items.PushBack(itemDef);
	
	itemDef.itemName = 'Redanian Herbal';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 2;
	itemDef.quantityMax = 4;
	items.PushBack(itemDef);
	
	itemDef.itemName = 'Temerian Rye';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 1;
	itemDef.quantityMax = 1;
	items.PushBack(itemDef);
	
	itemDef.itemName = 'Bread';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 2;
	itemDef.quantityMax = 4;
	items.PushBack(itemDef);
	
	itemDef.itemName = 'Cheese';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 2;
	itemDef.quantityMax = 4;
	items.PushBack(itemDef);
	
	itemDef.itemName = 'Roasted chicken leg';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 2;
	itemDef.quantityMax = 4;
	items.PushBack(itemDef);
	
	itemDef.itemName = 'Chips';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 2;
	itemDef.quantityMax = 4;
	items.PushBack(itemDef);
	
	itemDef.itemName = 'Bottled water';
	itemDef.reqLevel = 0;
	itemDef.quantityMin = 2;
	itemDef.quantityMax = 4;
	items.PushBack(itemDef);
...
quest function RecoverGeralt(dontUpdateUI : bool)
{
	...
	thePlayer.ForceSetStat(BCS_Stamina, thePlayer.GetStatMax(BCS_Stamina));
	// thePlayer.SimulateBuffTimePassing(0);
...
quest function ApplyAlchemyTableBuff()
{
	...
	GetWitcherPlayer().ManageAlchemyTableBonus();
	// W3EE - End
}
