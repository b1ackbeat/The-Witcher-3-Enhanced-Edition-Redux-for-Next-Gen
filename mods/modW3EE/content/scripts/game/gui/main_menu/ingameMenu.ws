...
class CR4IngameMenu extends CR4MenuBase
{
	...
	// W3EE - Begin
	protected var primer_settings_changed : bool;
	// W3EE - End
	...
	event  OnConfigUI()
	{
		...
		if (commonIngameMenu)
		{
			...
			WmkGetQuickSlotsInstance().OnIngameMenuOpen(this); // -= WMK:modQuickSlots =-
		}
	...
	event  OnClosingMenu()
	{
		...
		// -= WMK:modQuickSlots =-
		if (WmkGetQuickSlotsInstance() && (CR4CommonIngameMenu)(GetParent())) {
			WmkGetQuickSlotsInstance().OnClosingIngameMenu(this);
		}
		// -= WMK:modQuickSlots =-
	...
	event  OnItemActivated( actionType:int, menuTag:int ) : void
	{
		...
		else
		{
			...
			case IGMActionType_PurchaseEP2:
				...
				break;
			case IGMActionType_Quicksave:
				if( !theGame.AreSavesLocked() )
				{
					executeSave(SGT_QuickSave, -1);
					CloseMenu();
				}
				else
				{
	...
	event  OnOptionValueChanged(groupId:int, optionName:name, optionValue:string)
	{
		...
		// -= WMK:modQuickSlots =-
		if (WmkGetQuickSlotsInstance()) {
			WmkGetQuickSlotsInstance().OnOptionValueChanged(groupName, optionName, optionValue);
		}
		// -= WMK:modQuickSlots =-	
