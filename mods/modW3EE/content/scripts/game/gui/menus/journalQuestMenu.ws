...
{	
	...
	private var UNREAD_COLOR : string; default UNREAD_COLOR = "<font color=\"#be8728\">";
	private var UPDATED_COLOR : string; default UPDATED_COLOR = "<font color=\"#7f807b\">";
	private function IsDescriptionRead( questEntry : CJournalQuest ) : bool
	{
		...
		{
			descriptionGroup = (CJournalQuestDescriptionGroup)questEntry.GetChild(i);
			if( descriptionGroup )
				break;
		}
		
		for(i=0; i<descriptionGroup.GetNumChildren(); i+=1)
		{
			descriptionEntry = (CJournalQuestDescriptionEntry)descriptionGroup.GetChild(i);
			if( m_journalManager.GetEntryStatus(descriptionEntry) != JS_Inactive )
			{
				if( m_journalManager.IsEntryUnread(descriptionEntry) )
					return false;
			}
		}
	...
	private function UpdateQuestDescription( questEntry : CJournalQuest, wasRead : bool )
	{
		var descriptionGroup : CJournalQuestDescriptionGroup;
		var descriptionEntry : CJournalQuestDescriptionEntry;
		var i : int;
		...
		for( i = 0; i < descriptionGroup.GetNumChildren(); i += 1 )
		{
			descriptionEntry = (CJournalQuestDescriptionEntry)descriptionGroup.GetChild(i);
			if( m_journalManager.GetEntryStatus(descriptionEntry) != JS_Inactive )
				m_journalManager.SetEntryUnread(descriptionEntry, !wasRead);
		}
	}
	
	event OnEntrySelected( tag : name )
	{
		var newEntry, currEntry : CJournalQuest;
		
		if( tag )
		{
			super.OnEntrySelected(tag);
			
			newEntry = (CJournalQuest)m_journalManager.GetEntryByTag(tag);
			if( newEntry && tag != lastSelectedQuestTag )
			{
				if( lastSelectedQuestTag )
				{
					currEntry = (CJournalQuest)m_journalManager.GetEntryByTag(lastSelectedQuestTag);
					UpdateQuestDescription(currEntry, true);
					UpdateTwoQuests(currEntry, newEntry);
				}
				
				UpdateQuestDescription(newEntry, true);
	...
	private function generateFlashObjectForQuest( targetQuest : CJournalQuest, optional initSelection : bool  ) : CScriptedFlashObject
	{
		...
		wasRead = IsDescriptionRead(targetQuest);
		if( !wasRead )
			l_questLevel = UPDATED_COLOR + "•" + "</font>";
		...
		l_questsDataFlashObject.SetMemberFlashString( "reqdifficulty", "" );
		// W3EE - End
		l_questsDataFlashObject.SetMemberFlashBool( "isdeadlydifficulty", false );
		...
		for( i = 0; i < questEntry.GetNumChildren(); i += 1 )
		{
			...
			{				
				for( j = 0; j < l_questPhase.GetNumChildren(); j += 1 )
				{
					l_objective =( CJournalQuestObjective ) l_questPhase.GetChild(j);
					l_objectiveStatus 	= ( m_journalManager.GetEntryStatus( l_objective ) );
					if( l_objectiveStatus != JS_Inactive && l_objectiveStatus != JS_Failed)
					{						
	...
	function GetDescription( currentQuest : CJournalQuest ) : string
	{
		...
		for( i = 0; i < descriptionsGroup.GetNumChildren(); i += 1 )
		{
			...
			if( m_journalManager.GetEntryStatus(description) != JS_Inactive )
			{
				...
				currentJournalDescriptionText.wasRead = !m_journalManager.IsEntryUnread(description);
		...
		{
			if( !journalDescriptionArray[i].wasRead )
				str += UNREAD_COLOR + GetLocStringById(journalDescriptionArray[i].stringKey) + "</font><br>";
			else
