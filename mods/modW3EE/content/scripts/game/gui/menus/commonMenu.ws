...
class CR4CommonMenu extends CR4MenuBase
{
	...
	// W3EE - Begin
	protected var stashHotkey : EInputKey;
	...
	{	
		...
		theGame.GameplayFactsRemove("inFixedStash");
	...
	{	
		...
		if( MenuName == 'InventoryMenu' && !menuInitData && (MenuState == "" || MenuState == "None") )
			MenuState = "StashInventory";
			
		if( currentSubMenu && currentSubMenu.GetMenuName() == MenuName && MenuName != 'InventoryMenu' )
		{
	...
	private function DefineMenuStructure() : void
	{
		...
		if (!isCiri)
		{
			...
			DefineMenuItem('CraftingParent', "panel_title_crafting",,, 'AlchemyMenu');
				DefineMenuItem('CraftingMenu', "panel_title_crafting", 'CraftingParent');
				DefineMenuItem('AlchemyMenu', "panel_title_alchemy", 'CraftingParent');
				
			DefineMenuItem('InventoryParent', "panel_inventory",,, 'InventoryMenu');
				DefineMenuItem('InventoryMenu', "gwint_geralt", 'InventoryParent', 'CharacterInventory');
				if( Equipment().GetHorseDistance() <= 81 || Options().InvEverywhere() )
					DefineMenuItem('InventoryMenu', "panel_title_roach", 'InventoryParent', 'StashInventory');
		...
		if (!isCiri)
		{
			...
			if( isPlayerMeditatingInBed || GetWitcherPlayer().IsMeditating() )
	...
	private function DisableNotAllowedTabs()
	{
		...
		if( !thePlayer.IsActionAllowed( EIAB_OpenInventory ))
		{
			...
			SetMenuTabeEnable( 'InventoryParent', false );				
			SetMenuTabeEnable( 'InventoryMenu',	false, 'CharacterInventory' );				
			SetMenuTabeEnable( 'InventoryMenu',	false, 'StashInventory' );				
		}	
		if( !thePlayer.IsActionAllowed( EIAB_OpenAlchemy ))
		{
			SetMenuTabeEnable('CraftingParent', false);
	...
	public function ActionBlockStateChange(action:EInputActionBlock, blocked:bool) : void
	{
		...
		{
			SetMenuTabeEnable( 'InventoryParent', blocked );
			SetMenuTabeEnable( 'InventoryMenu',	blocked, 'CharacterInventory' );
			SetMenuTabeEnable( 'InventoryMenu',	blocked, 'StashInventory' );
			tabName = 'InventoryMenu';
			subTabName = 'CharacterInventory';
		}	
		if( action == EIAB_OpenAlchemy )
		{
			SetMenuTabeEnable( 'CraftingParent', blocked );
	...
	private function DefineMenuItem(itemName:name, itemLabel:string, optional parentMenuItem:name, optional menuState:name, optional iconOverride:name) : void
	{
		...
		newMenuItem.IconOverride = iconOverride;
	...
	private function GetGFxMenuItem(MenuItemData:SMenuTab, out GFxObjectData:CScriptedFlashObject):void
	{
		...
		if( MenuItemData.IconOverride != '' )
			GFxObjectData.SetMemberFlashString("icon", NameToString(MenuItemData.IconOverride)); 
		else
	...
	private function GetNewItemsGFxMenuStruct( out StructGFx : CScriptedFlashArray ) : void
	{
		...
		{	
			...
			if( _inv.IsIdValid(item) && !( _inv.ItemHasTag( item, 'NoShow' )) )
			{
				...
				l_flashObject.SetMemberFlashString( "itemName", _inv.GetItemLocNameByID( item ) );
				StructGFx.PushBackFlashObject(l_flashObject);
			}
		}
	...
	protected function fetchCurrentHotkeys() : void
	{
		...
		// W3EE - Begin
		outKeys.Clear();
		theInput.GetPCKeysForAction('OpenStash', outKeys);
		if (outKeys.Size() > 0)
		{
	...
	event  OnHotkeyTriggered(keyCode:EInputKey)
	{
		...
		if (childMenu && !IsLockedInHub() && !IsLockedInMenu())
		{
			...
			if (keyCode == inventoryHotkey && theGame.GameplayFactsQuerySum("shopMode") == 0)
			{
				...
				if( HasMenuWithStateDefined('InventoryMenu', 'CharacterInventory') && thePlayer.IsActionAllowed(EIAB_OpenInventory) )
				{
			...
			if( keyCode == stashHotkey && theGame.GameplayFactsQuerySum("shopMode") == 0 )
			{
				invMenu = (CR4InventoryMenu)childMenu;
				...
				if( HasMenuWithStateDefined('InventoryMenu', 'StashInventory') && thePlayer.IsActionAllowed(EIAB_OpenInventory) )
				{
			...
			*/
			...
			// -= WMK:modQuickSlots =-
			else if ((CR4InventoryMenu)childMenu) {
				WmkGetQuickSlotsInstance().OnInventoryMenuHotkeyTriggered((CR4InventoryMenu)childMenu, keyCode);
			}
			// -= WMK:modQuickSlots =-
		}
	}
