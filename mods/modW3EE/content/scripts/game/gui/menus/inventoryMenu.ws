...
class WmkCR4InventoryMenu extends CR4MenuBase // -= WMK:modQuickSlots =-
{
	...
	// W3EE - Begin
	private var _invStashContext	: W3StashGridContext;
	// W3EE - End
	...
	// W3EE - Begin
	public var _disableStashContext : bool; default _disableStashContext = false; 
	private var m_fxSetWeight			 : CScriptedFlashFunction;
	private var m_fxSetStashText	     : CScriptedFlashFunction;
	
	public function IsStashMode() : bool
	{
		return _defaultInventoryState == IMS_Stash;
	}
	// W3EE - End
	...
	event  OnConfigUI()
	{
		...
		else if( _shopNpc )
		{
			...
			ModNoDuplicatesHideInventoryComponentDuplicates(_shopInv.GetInventoryComponent());//modNoDuplicates
		...
		// W3EE - Begin
		else if ( theGame.GameplayFactsQuerySum("stashMode") == 1 || m_menuState == 'StashInventory' )
		{
			_defaultInventoryState = IMS_Stash;
			_disableStashContext = true;
			
			m_fxSetWeight = m_flashModule.GetMemberFlashFunction("setWeight");
			m_fxSetStashText = m_flashModule.GetMemberFlashFunction("setStashText");
			//Kolaris - Fixed Stashes
			if( GameplayFactsQuerySum("inFixedStash") > 0 )
				m_fxSetStashText.InvokeSelfOneArg(FlashArgString(GetLocStringByKeyExt("panel_title_stash")));
			else
				m_fxSetStashText.InvokeSelfOneArg(FlashArgString(GetLocStringById(1082500)));
		}
		else
		{
		...
		// W3EE - Begin
		if( _defaultInventoryState == IMS_Stash )
		{
	...
	event  OnTabChanged(tabIndex:int)
	{
		...
		// W3EE - Begin
		if( _currentState == IMS_Stash && _horseInv.syncWithPlayer )
		{
	...
	private function SetInventoryState(targetMode : int) : void
	{
		...
		switch (targetMode)
		{
			...
			case IMS_Stash:
				// W3EE - Begin
				_playerInv.stashMode = !_disableStashContext;
				// W3EE - End
	...
	function StashUpdateItem( item : SItemUniqueId )
	{
		...
		if( !_inv.ItemHasTag(item, 'NoShow') || GameplayFactsQuerySum("inFixedStash") > 0 )
		{
			tempFlashObject = m_flashValueStorage.CreateTempFlashObject();
			itemDataObject = tempFlashObject.CreateFlashObject("red.game.witcher3.menus.common.ItemDataStub");
	...
	function StashUpdateEncumbrance()
	{
		var currWeight 			: int;
		var maxWeight			: int;
		
		maxWeight = (int)CalculateAttributeValue( GetWitcherPlayer().GetHorseManager().GetHorseAttributeValue('encumbrance', false) ) + Options().BaseCWRoach();
		currWeight = (int)GetCurrentWeight( _horseInv.GetInventoryComponent() );
		if( GameplayFactsQuerySum("inFixedStash") > 0 )
			m_fxSetWeight.InvokeSelfOneArg(FlashArgString(GetLocStringByKeyExt("panel_stash_unlimited")));
		else
			m_fxSetWeight.InvokeSelfOneArg(FlashArgString(currWeight + " / " + maxWeight));
	}
	...
	private function AddEquippedPotionsToList(out itemsList  : array < SItemUniqueId > ) : void
	{
		...
		// -= WMK:modQuickSlots =-
		GetWitcherPlayer().GetItemEquippedOnSlot(EES_Potion5,itemOnSlot);
		if ( _inv.IsIdValid(itemOnSlot) )
		{
			itemsList.PushBack(itemOnSlot);
		}
		GetWitcherPlayer().GetItemEquippedOnSlot(EES_Potion6,itemOnSlot);
		if ( _inv.IsIdValid(itemOnSlot) )
		{
			itemsList.PushBack(itemOnSlot);
		}
		GetWitcherPlayer().GetItemEquippedOnSlot(EES_Potion7,itemOnSlot);
		if ( _inv.IsIdValid(itemOnSlot) )
		{
			itemsList.PushBack(itemOnSlot);
		}
		GetWitcherPlayer().GetItemEquippedOnSlot(EES_Potion8,itemOnSlot);
		if ( _inv.IsIdValid(itemOnSlot) )
		{
			itemsList.PushBack(itemOnSlot);
		}
	...
	function UpdateHorseInventory()
	{
		...
		maxWeight = (int)CalculateAttributeValue( GetWitcherPlayer().GetHorseManager().GetHorseAttributeValue('encumbrance', false) ) + Options().BaseCWRoach();
		currWeight = (int)GetCurrentWeight( _horseInv.GetInventoryComponent() );        
		// W3EE MQS merge - Begin
		//showNotification("Saddlebag: " + currWeight + "/" + maxWeight);
		//Kolaris - Fixed Stashes
		if( GameplayFactsQuerySum("inFixedStash") > 0 )
			m_fxSetWeight.InvokeSelfOneArg(FlashArgString(GetLocStringByKeyExt("panel_stash_unlimited")));
		else
			m_fxSetWeight.InvokeSelfOneArg(FlashArgString(currWeight + " / " + maxWeight));
		//m_fxSetStashText.InvokeSelfOneArg(FlashArgString("ok"));
		// W3EE MQS merge - Begin
		// W3EE - End
	}
	...
	{		
		...
		{
			// W3EE - Begin
			if( m_currentContext == _invStashContext )
			{
				return GetWitcherPlayer().GetHorseManager().GetInventoryComponent();
			}
			else
			{
				return _inv;
	...
	private function GetDefenseTooltipData(out GFxData: CScriptedFlashArray):void
	{
		...
		/*PushStatItem(GFxData, "rending_resistance", GetStatValue('rending_resistance_perc') + "%");*/ //W3EE
	...
	event  OnPutInSocket( item : SItemUniqueId )
	{
		...
		else if( !CampfireManager().CanPerformUpgrade() && !Options().GetUpgradeAnywhere() )
		{
	...
	public function TryEquipToPockets(item : SItemUniqueId, slot : int ) : bool
	{
		...
		else if( IsSlotPetardslot(slot) ) // -= WMK:modQuickSlots =-
		{
			targetList.PushBack(EES_Petard1);
			targetList.PushBack(EES_Petard2);
			// -= WMK:modQuickSlots =-
			targetList.PushBack(EES_Petard3);
			targetList.PushBack(EES_Petard4);
			// -= WMK:modQuickSlots =-
		}	
		else if( IsSlotPotionSlot(slot) )
		{
			...
			// -= WMK:modQuickSlots =-
			targetList.PushBack(EES_Potion5);
			targetList.PushBack(EES_Potion6);
			targetList.PushBack(EES_Potion7);
			targetList.PushBack(EES_Potion8);
			// -= WMK:modQuickSlots =-
		}
	...
	event  OnEquipItem( item : SItemUniqueId, slot : int, quantity : int )
	{
		...
		else
		{
			...
			else
			{
				...
				// -= WMK:modQuickSlots =-
				if (WmkIsNewPotionSlot(slot) && (theGame.m_quickSlotsConfig.MAX_EQUIPPED_CONSUMABLES == MAXC_Vanilla)) {
					showNotification(GetLocStringByKeyExt("mqs_message_panel_inventory_new_quick_slots_disabled"));
					OnPlaySoundEvent("gui_global_denied");
					return false;
				}
				// -= WMK:modQuickSlots =-
		...
		else
		{
			...
			*/
			...
			if (_inv.IsItemSetItem(item))
			{
				...
				// -= WMK:modQuickSlots =-
				PushIfItemEquipped(paperdollItemsToUpdate, EES_Petard3);
				PushIfItemEquipped(paperdollItemsToUpdate, EES_Petard4);
				// -= WMK:modQuickSlots =-
				...
				// -= WMK:modQuickSlots =-
				PushIfItemEquipped(paperdollItemsToUpdate, EES_Potion5);
				PushIfItemEquipped(paperdollItemsToUpdate, EES_Potion6);
				PushIfItemEquipped(paperdollItemsToUpdate, EES_Potion7);
				PushIfItemEquipped(paperdollItemsToUpdate, EES_Potion8);
				// -= WMK:modQuickSlots =-
	...
	public function UnequipItem( item : SItemUniqueId, moveToIndex : int ) : bool
	{
		...
		else if (_inv.IsIdValid(item))
		{
			...
			if (_inv.IsItemSetItem(item) && isSetBonusActive)
			{
				...
				// -= WMK:modQuickSlots =-
				PushIfItemEquipped(itemsList, EES_Petard3);
				PushIfItemEquipped(itemsList, EES_Petard4);
				// -= WMK:modQuickSlots =-
				...
				// -= WMK:modQuickSlots =-
				PushIfItemEquipped(itemsList, EES_Potion5);
				PushIfItemEquipped(itemsList, EES_Potion6);
				PushIfItemEquipped(itemsList, EES_Potion7);
				PushIfItemEquipped(itemsList, EES_Potion8);
				// -= WMK:modQuickSlots =-
	...
	event  OnDropItem( item : SItemUniqueId, quantity : int ) 
	{
		if( _inv.IsItemSingletonItem(item) )
			quantity = _inv.SingletonItemGetAmmo(item);
	...
	event  OnSellItem( itemId : SItemUniqueId, quantity : int )
	{
		...
		// W3EE - Begin
		itemPrice = _shopInv.GetInventoryComponent().GetInventoryItemBasePriceModified( invItem, _playerInv.GetInventoryComponent(), true, _shopInv.GetInventoryComponent().GetEntity() );
	...
	event  OnBuyItem( item : SItemUniqueId, quantity : int, moveToIdx : int )
	{
		...
		{
			// W3EE - Begin
			price = _shopInv.GetInventoryComponent().GetItemSellPrice(item);			
			if(price > thePlayer.GetMoney())
			// W3EE - End
			{			
	...
	private function OpenQuantityPopup( itemId : SItemUniqueId, action : EQuantityTransferFunction, maxValue : int )
	{
		...
		switch( action )
		{
			case QTF_Sell:
				invItem = _playerInv.GetInventoryComponent().GetItem( itemId );
				// W3EE - Begin
				_quantityPopupData.itemCost = _shopInv.GetInventoryComponent().GetInventoryItemBasePriceModified( invItem, _playerInv.GetInventoryComponent(), true, _shopInv.GetInventoryComponent().GetEntity() );
				// W3EE - End
			...
			case QTF_Buy:
				// W3EE - Begin
				_quantityPopupData.itemCost = _shopInv.GetInventoryComponent().GetItemSellPrice( itemId );
				// W3EE - End
				_quantityPopupData.showPrice = true;
		...
		_quantityPopupData.inventoryRef = (CR4InventoryMenu) this; // -= WMK:modQuickSlots =-
	...
	function TakeItem( item : SItemUniqueId, quantity : int  )
	{
		...
		if( _currentState == IMS_Stash )
		{
			...
			if( _horseInv.GetInventoryComponent().GetItemQuantity(item) <= 0 )
			{
				ShopRemoveItem(item);
				_horseInv.clearGridPosition(item);
			}
			else StashUpdateItem(item);
	...
	function SellItem( item : SItemUniqueId, quantity : int ) : SItemUniqueId
	{
		...
		if( _shopNpc )
		{
			...
			// W3EE - Begin
			itemCost = shopInvComponent.GetInventoryItemBasePriceModified( invItem, _playerInv.GetInventoryComponent(), true, shopInvComponent.GetEntity() ) * quantity;
			// W3EE - End
	...
	public function HorseDropItem( inv : CInventoryComponent, item : SItemUniqueId, quantity : int ) 
	{
		if( inv.IsItemSingletonItem(item) )
			quantity = inv.SingletonItemGetAmmo(item);
		...
		if( !inv.ItemHasTag(item, 'NoDrop') && !inv.ItemHasTag(item, 'Quest') )
		{
			if( quantity <= 1 )
			{
				inv.HorseDropItemInBag(item, quantity);
				OnPlaySoundEvent("gui_inventory_drop");
				ShopRemoveItem(item);
			}
			else
			{
				OpenQuantityPopup(item, QTF_DropHorse, quantity);
			}
		}
		else
		{
	...
	// W3EE - Begin
	public function SwitchShowStashItems()
	{
		if( _currentState == IMS_Stash )
		{
		...
		else if( _currentState == IMS_Shop )
		{
			if( _playerInv.IsOwner() )
			{
				...
				_tooltipDataProvider.SwitchToHorse();
				_disableStashContext = true;
				ResetContext();
				createInventoryStashContext();
			}
			else
			{
				_playerInv.SwitchToOwner(false);
				_tooltipDataProvider.SwitchToOwner();
				_disableStashContext = false;
				ResetContext();
	...
	// W3EE - Begin
	protected function createInventoryStashContext():void
	{
		if (_invStashContext)
		{
			delete _invStashContext;
		...
		_invStashContext = new W3StashGridContext in this;
		_invStashContext.SetInventoryRef((CR4InventoryMenu)this);
		m_currentContext = _invStashContext;
		ActivateContext(m_currentContext);
	}
	// W3EE - End
	...
	protected function createStatContext():void
	{
		...
		_statsContext.SetInventoryRef((CR4InventoryMenu) this); // -= WMK:modQuickSlots =-
	...
	protected function createPaperdollContext():void
	{
		...
		_paperdollContext.SetInventoryRef((CR4InventoryMenu) this); // -= WMK:modQuickSlots =-
	...
	protected function createInventoryContext():void
	{
		...
		_invContext.SetInventoryRef((CR4InventoryMenu) this); // -= WMK:modQuickSlots =-
	...
	protected function createExternalContext():void
	{
		...
		_externGridContext.SetInventoryRef((CR4InventoryMenu) this); // -= WMK:modQuickSlots =-
	...
	function ReadBook( item : SItemUniqueId )
	{
		...
		ShowBookPopup( _inv.GetItemLocNameByID( item ), _playerInv.GetBookText( item ), item);
	}
	...
	function ShowBookPopup(BookTitle:string, BookText:string, item : SItemUniqueId, optional singleBookMode : bool )
	{
		...
		_bookPopupData.inventoryRef = (CR4InventoryMenu) this; // -= WMK:modQuickSlots =-
	...
	function ShowPainting(item : SItemUniqueId)
	{
		...
		_paintingPopupData.SetMessageTitle( _inv.GetItemLocNameByID(item) );
	...
	public function ShowItemInfoPopup( item : SItemUniqueId ) : void
	{
		...
		_itemInfoPopupData.inventoryRef = (CR4InventoryMenu) this; // -= WMK:modQuickSlots =-
	...
	public function ShowSelectionMode( sourceItem : SItemUniqueId, targetSlotList : array< int > )
	{
		...
		// W3EE - Begin
		GetWitcherPlayer().SetSelectionActive(m_selectionModeActive);
		// W3EE - End
	}
	...
	event  OnSelectionModeTargetChosen( targetSlot : int )
	{
		...
		{
			// -= WMK:modQuickSlots =-
			if (WmkIsNewPotionSlot(targetSlot) && (theGame.m_quickSlotsConfig.MAX_EQUIPPED_CONSUMABLES == MAXC_Vanilla)) {
				showNotification(GetLocStringByKeyExt("mqs_message_panel_inventory_new_quick_slots_disabled"));
				OnPlaySoundEvent("gui_global_denied");
				HideSelectionMode();
				return false;
	...
	private function ApplyUpgrade(itemId : SItemUniqueId,  targetSlot : int) : void
	{
		...
		{
			// W3EE - Begin
			if( !Options().GetUpgradeAnywhere() )
			{
				GetWitcherPlayer().AdvanceTimeSeconds( 600 );
			}
			// W3EE - End
	...
	protected function ApplyOil(itemId : SItemUniqueId,  targetSlot : int) : void // -= WMK:modQuickSlots =-
	{
		var curItemInSlot : SItemUniqueId;
	...
	protected function ApplyRepairKit(itemId : SItemUniqueId,  targetSlot : int) : void // -= WMK:modQuickSlots =-
	{
		var curItemInSlot : SItemUniqueId;
		...
		if ( _inv.ItemHasTag( itemId, 'WeaponReapairKit' ) && _inv.GetItemEquippedOnSlot(targetSlot, curItemInSlot) && _inv.ItemHasTag( curItemInSlot, 'Weapon' ) )
		{
		...
		if ( _inv.ItemHasTag( itemId, 'ArmorReapairKit_Weak' ) && _inv.GetItemEquippedOnSlot(targetSlot, curItemInSlot) && _inv.ItemHasTag( curItemInSlot, 'LightArmor' ) )
		{
			GetWitcherPlayer().RepairItem (itemId, curItemInSlot);
			OnPlaySoundEvent("gui_inventory_repair");
			if (_inv.IsIdValid(itemId) && _inv.GetItemQuantity( itemId ) > 0)
		...
		if ( _inv.ItemHasTag( itemId, 'ArmorReapairKit_Medium' ) && _inv.GetItemEquippedOnSlot(targetSlot, curItemInSlot) && _inv.ItemHasTag( curItemInSlot, 'MediumArmor' ) )
		{
			GetWitcherPlayer().RepairItem (itemId, curItemInSlot);
			OnPlaySoundEvent("gui_inventory_repair");
			if (_inv.IsIdValid(itemId) && _inv.GetItemQuantity( itemId ) > 0)
		...
		if ( _inv.ItemHasTag( itemId, 'ArmorReapairKit_Strong' ) && _inv.GetItemEquippedOnSlot(targetSlot, curItemInSlot) && _inv.ItemHasTag( curItemInSlot, 'HeavyArmor' ) )
		{
			GetWitcherPlayer().RepairItem (itemId, curItemInSlot);
			OnPlaySoundEvent("gui_inventory_repair");
			if (_inv.IsIdValid(itemId) && _inv.GetItemQuantity( itemId ) > 0)
	...
	// W3EE - Begin
	function GetCurrentWeight( inv : CInventoryComponent ) : float
	{
		var i: int;
		...
		for(i=0; i<items.Size(); i+=1)
			if( !(_horseInv.isHorseItem(items[i]) && GetWitcherPlayer().GetHorseManager().IsItemEquipped(items[i])) && inv.IsItemEncumbranceItem(items[i]) ) //Kolaris - Trophy Weight Fix
				currWeight += inv.GetItemEncumbrance( items[i] );
			
		return currWeight;
	}
	
	public function GetItemEncumbranceWithQuantity( invComponent : CInventoryComponent, item : SItemUniqueId, quantity : int ) : float
	{
		return invComponent.GetItemEncumbrance(item, quantity);
	}
	
	public function TransferMiscItems()
	{
		...
		//Kolaris - Transfer Misc Items Fix
		if( !Options().InvEverywhere() && Equipment().GetHorseDistance() > 81 )
		{
		...
		for(i=0; i<items.Size(); i+=1)
		{
			...
			if( _inv.IsItemEncumbranceItem(items[i]) )
			{
				if( _inv.IsItemUsable(items[i]) || _inv.IsItemQuickslotItem(items[i]) || _inv.IsItemMounted(items[i]) || _inv.IsItemHeld(items[i]) || _inv.IsItemFood(items[i]) )
					continue;
					
				if( _inv.IsItemAlchemyIngredient(items[i]) )
				{
					mut = false;
					for(j=0; j<mutagenSlots.Size(); j+=1)
						if( items[i] == mutagenSlots[j].item )
							mut = true;
					
					if( !mut )
						if ( !handleMoveToStashQuantity(items[i], _inv.GetItemQuantity(items[i])) )
							stop = true;
				}
				else
				if( _inv.IsItemCraftingIngredient(items[i]) || _inv.GetItemName(items[i]) == 'Florens' || _inv.GetItemName(items[i]) == 'Orens' || _inv.GetItemName(items[i]) == 'Crowns' )
				{
					if( !handleMoveToStashQuantity(items[i], _inv.GetItemQuantity(items[i])) )
						stop = true;
				}
				else
				if( !_inv.IsItemAnyArmor(items[i]) && !_inv.IsItemWeapon(items[i]) && !_inv.IsItemSingletonItem(items[i]) && !_inv.IsItemAlchemyItem(items[i]) && !_inv.IsItemQuest(items[i]) )
				{
					if( !handleMoveToStashQuantity(items[i], _inv.GetItemQuantity(items[i])) )
						stop = true;
				}
	...
	public function MoveToStash(item : SItemUniqueId) : bool
	{
		var currWeight, maxWeight, newWeight : float;
		var witcher : W3PlayerWitcher;
		var quantity : int;
		...
		if( quantity > 1 )
			OpenQuantityPopup( item, QTF_MoveToStash, quantity );
		else
		{
			witcher = GetWitcherPlayer();
            maxWeight = CalculateAttributeValue( witcher.GetHorseManager().GetHorseAttributeValue('encumbrance', false) ) + Options().BaseCWRoach();
            currWeight = GetCurrentWeight( _horseInv.GetInventoryComponent() );
            newWeight = GetItemEncumbranceWithQuantity(witcher.GetInventory(), item, 1) + currWeight;
			//Kolaris - Fixed Stashes
			if( newWeight > maxWeight && GameplayFactsQuerySum("inFixedStash") < 1 )
			{
				showNotification(GetLocStringByKeyExt("W3EE_SaddlebagFull"));
				OnPlaySoundEvent("gui_global_denied");
				return false;
			}
			if( GameplayFactsQuerySum("inFixedStash") > 0 && !Equipment().IsStashableItem(_inv, item) && !Equipment().IsStashableIngredient(_inv.GetItemName(item)) )
			{
				OnPlaySoundEvent("gui_global_denied");
				return false;
			}
			
            StashUpdateItem(witcher.GetHorseManager().MoveItemToHorse(item, 1));
            InventoryRemoveItem(item, false);
            _playerInv.clearGridPosition(item);
            UpdateEncumbranceInfo();
			StashUpdateEncumbrance();
	...
	public function handleMoveToStashQuantity(item : SItemUniqueId, quantity : int) : bool
	{
		// W3EE - Begin
		var currWeight, maxWeight, newWeight : float;
		var witcher : W3PlayerWitcher;
        ...
        maxWeight = CalculateAttributeValue( witcher.GetHorseManager().GetHorseAttributeValue('encumbrance', false) ) + Options().BaseCWRoach();
        currWeight = GetCurrentWeight( _horseInv.GetInventoryComponent() );
        newWeight = GetItemEncumbranceWithQuantity(witcher.GetInventory(), item, quantity) + currWeight;
		//Kolaris - Fixed Stashes
		if( newWeight > maxWeight && GameplayFactsQuerySum("inFixedStash") < 1 )
		{
			showNotification(GetLocStringByKeyExt("W3EE_SaddlebagFull"));
		...
		if( GameplayFactsQuerySum("inFixedStash") > 0 && !Equipment().IsStashableItem(_inv, item) && !Equipment().IsStashableIngredient(_inv.GetItemName(item)) )
		{
 		...
 		StashUpdateItem(witcher.GetHorseManager().MoveItemToHorse(item, quantity)); 
		if( _inv.IsItemSingletonItem(item) )
			currQuantity = _inv.SingletonItemGetAmmo(item);
		else
			currQuantity = _inv.GetItemQuantity(item);
		if( quantity >= currQuantity + quantity )
		{
			InventoryRemoveItem(item, false);
			_playerInv.clearGridPosition(item);
		}
		else InventoryUpdateItem(item);
	...
	public function TakeItemFromStash(item : SItemUniqueId) : void
	{
		...
		if( _horseInv.GetInventoryComponent().IsIdValid(item) )
		{
			//---=== modFriendlyHUD ===---
			if( _horseInv.GetInventoryComponent().IsItemSingletonItem(item) )
				quantity = _horseInv.GetInventoryComponent().SingletonItemGetAmmo(item);
			else
				quantity = _horseInv.GetInventoryComponent().GetItemQuantity(item);
