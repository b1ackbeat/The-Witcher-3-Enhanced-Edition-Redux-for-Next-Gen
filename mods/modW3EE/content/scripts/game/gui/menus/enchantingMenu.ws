...
class CR4EnchantingMenu extends CR4MenuBase
{
	...
	private function populateData():void
	{
		...
		// W3EE - Begin
		getEnchantmentsListData(m_runewordsList, TYPE_RUNEWORD, m_runewordsData, true);
		// W3EE - End
		//Kolaris - Enchantment Overhaul
		getEnchantmentsListData(m_glyphwordsList, TYPE_GLYPHWORD, m_glyphwordsData, true);
	...
	// W3EE - Begin
	private function getEnchantmentsListData(nameList : array<CName>, type:int, out enchantmentsList:CScriptedFlashArray, optional shouldCheckReq : bool):void
	// W3EE - End
	{
		var i, j : int;
		var itemsCount : int;
		...
		//Kolaris - Enchantment Overhaul
		var curIngredientNameArray  : array<CName>;
		...
		//Kolaris - Enchantment List Sorting
		/*for(i=nameList.Size(); i>0; i-=1)
		{
			for (j=0; j<nameList.Size(); j+=1)
			{
				if(StrContains(NameToString(nameList[j]), IntToString(i)) && !nameListSort.Contains(nameList[j]))
				{
					nameListSort.PushBack(nameList[j]);
					break;
				}
			}
		}
		...
		for (i = 0; i < itemsCount; i=i+1)
		{
			...
			if (ingredientsCount > 0)
			{
				...
				for (j = 0; j < ingredientsCount; j = j +1)
				{
					...
					// W3EE - Begin
					availableIngredientsCount = Equipment().GetItemQuantityByNameForCrafting(curIngredientName);
					// W3EE - End
					...
					//Kolaris - Enchantment Overhaul
					if( curIngredientNameArray.Contains(curIngredientName) )
					{
						requiredIngredientsCount += 1;
					}
					curIngredientNameArray.PushBack(curIngredientName);
			...
			// W3EE - Begin
			if( shouldCheckReq && m_playerInventory.IsIdValid(m_currentItem) )
			{
				if( !Equipment().HasPreviousTierRuneword(curSchematicName, m_currentItem) )
					canApply = false;
			}
			// W3EE - End
	...
	// W3EE - Begin
	private var missingPrevRunewordError : bool;
	private var alreadyHasRunewordError : bool;
	private var m_currentItem : SItemUniqueId;
	event  OnUnexpectedError()
	{
		if( !m_playerInventory.IsIdValid(m_currentItem) )
		{
			showNotification(GetLocStringByKeyExt("W3EE_NoItem"));
		}
		else
		if( missingPrevRunewordError )
		{
			showNotification(GetLocStringByKeyExt("W3EE_PrevEnchReq"));
		}
		else
		if( alreadyHasRunewordError )
		{
			showNotification(GetLocStringByKeyExt("W3EE_AlreadyHasEnch"));
		}
		else
		{
			showNotification(GetLocStringByKeyExt("W3EE_MissingCrafting"));
		}
	...
	event  OnEnchantItem(itemId : SItemUniqueId, enchantmentName : name)
	{
		...
		if (enchantResult)
		{
			...
			for (i = 0; i < ingredientsCount; i=i+1 )
			{
				...
				// W3EE - Begin
				Equipment().RemoveItemByNameForCrafting(curIngredient.itemName, curIngredient.quantity);
				// W3EE - End
			}
			
			if( reequip )
				GetWitcherPlayer().EquipItem(itemId);
	...
	event  OnRemoveEnchantment(itemId : SItemUniqueId):void
	{
		...
		enchantmentName = m_playerInventory.GetEnchantment(itemId);
		...
		if (unenchantResult)
		{
			//Kolaris - Return Enchant Ingredients
			ingredientsCount = schematic.ingredients.Size();
			for (i = 0; i < ingredientsCount; i=i+1 )
			{
				curIngredient = schematic.ingredients[i];
				m_playerInventory.AddAnItem(curIngredient.itemName);
			}
	...
	event  OnShowAllEnchantments():void
	{
		...
		// W3EE - Begin
		m_currentItem = GetInvalidUniqueId();
		populateData();
		// W3EE - End
	...
	event  OnSelectEnchantment(EnchantmentName : name):void
	{
		...
		if (m_selectedEnchantment != EnchantmentName)
		{
			...
			// W3EE - Begin
			if( !Equipment().HasPreviousTierRuneword(EnchantmentName, m_currentItem) )
			{
				missingPrevRunewordError = true;
				canApply = false;
			}
			else
			{
				missingPrevRunewordError = false;
