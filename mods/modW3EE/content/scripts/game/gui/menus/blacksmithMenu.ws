...
class CR4BlacksmithMenu extends CR4MenuBase
{
	...
	// W3EE - Begin
	protected var _playerInv      : CInventoryComponent;
	protected var _horseInv	      : CInventoryComponent;
	private var m_blacksmithContext		: W3BlacksmithContext;
	// W3EE - End
	...
	{	
		...
		// W3EE - Begin
		_playerInv = thePlayer.GetInventory();
		_horseInv = GetWitcherPlayer().GetHorseManager().GetInventoryComponent();
		// W3EE - End
	...
	// W3EE - Begin
	protected function CreateBlacksmithContext()
	{
	...
	public function ResetShowStashItems()
	{
		_inv = _playerInv;
		_curInv.SetCurrentInventory( _inv );
		_tooltipDataProvider.setCurrentInventory( _inv );
	...
	event  OnDisassembleItem( item : SItemUniqueId, price : int)
	{
		...
		itemsListText += "<br/>" + ( _inv.GetItemLocNameByID( item ) + " x" + m_lastConfirmedDisassembleQuantity );
