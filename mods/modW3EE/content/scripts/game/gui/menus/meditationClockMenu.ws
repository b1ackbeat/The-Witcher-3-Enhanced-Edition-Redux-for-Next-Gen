...
class CR4MeditationClockMenu extends CR4MenuBase
{
	...
	{	
		...
		// W3EE - Begin
		if( !GetWitcherPlayer().IsMeditating() && !((CR4CommonMenu)m_parentMenu).GetIsPlayerMeditatingInBed() )
			m_fxSetBlockMeditation.InvokeSelfOneArg( FlashArgBool( !canMeditateWait ) );
		else
		{
			GetWitcherPlayer().SetInMeditationMenu(true);
		}
		// W3EE - End
		...
		// W3EE - Begin
		/*
		if( !GetWitcherPlayer().IsMeditating() || ( GetWitcherPlayer().IsMeditating() && Options().GetQuickMenuType() == 0 ) )
			theGame.Pause("menus");
		*/
		// W3EE - End
	}
	...
	event  OnClosingMenu()
	{
		...
		// W3EE - Begin
		GetWitcherPlayer().SetInMeditationMenu(false);
		// W3EE - End
	}
	...
	event  OnMeditate( dayTime : float )
	{
		...
		// W3EE - Begin
		var meditateCurrentTime : GameTime;
		var meditateStartTime : GameTime;
		var targetTime : GameTime;
		if( GetWitcherPlayer().IsMeditating() )
		{
			if( GetWitcherPlayer().GetMeditationState().IsActivelyMeditating() )
			{
				ShowDisallowedNotification();
				return false;
			}
			
			GetWitcherPlayer().SetWaitTargetHour(CeilF(dayTime));
			GetWitcherPlayer().MeditationStartFastforwardMenu();
