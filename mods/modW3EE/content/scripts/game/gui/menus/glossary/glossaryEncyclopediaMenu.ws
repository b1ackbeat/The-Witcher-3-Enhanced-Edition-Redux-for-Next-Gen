...
{	
	...
	private var lastSelectedTag	: name;
	private var UNREAD_COLOR : string; default UNREAD_COLOR = "<font color=\"#be8728\">";
	private function IsDescriptionRead( charEntry : CJournalCharacter ) : bool
	{
		var description : CJournalCharacterDescription;
		var i : int;
		
		for(i=0; i<charEntry.GetNumChildren(); i+=1)
		{
			description = (CJournalCharacterDescription)charEntry.GetChild(i);
			if( m_journalManager.GetEntryStatus(description) == JS_Active )
			{
				if( m_journalManager.IsEntryUnread(description) )
					return false;
			}
		}
		return true;
	...
	private function UpdateGlossaryDescription( charEntry : CJournalCharacter, wasRead : bool )
	{
		var i : int;
		var description : CJournalCharacterDescription;
		
		m_journalManager.SetEntryUnread(charEntry, !wasRead);
		for( i = 0; i < charEntry.GetNumChildren(); i += 1 )
		{
			description = (CJournalCharacterDescription)charEntry.GetChild(i);
			if( m_journalManager.GetEntryStatus(description) == JS_Active )
			{
				m_journalManager.SetEntryUnread(description, !wasRead);
			}
		}
	}
	...
	{
		var newEntry, currEntry : CJournalCharacter;
		
		if( tag )
		{
			super.OnEntrySelected(tag);
			
			newEntry = (CJournalCharacter)m_journalManager.GetEntryByTag(tag);
			if( newEntry && tag != lastSelectedTag )
			{
				if( lastSelectedTag )
				{
					currEntry = (CJournalCharacter)m_journalManager.GetEntryByTag(lastSelectedTag);
					UpdateGlossaryDescription(currEntry, true);
				}
				
				UpdateGlossaryDescription(newEntry, true);
				lastSelectedTag = tag;
			}
		}
	}
	...
	private function AddCharacterJournalEntryToArray(journalCharacter:CJournalCharacter, flashArray:CScriptedFlashArray):void
	{
		...
		l_IsNew	= !IsDescriptionRead(journalCharacter);
