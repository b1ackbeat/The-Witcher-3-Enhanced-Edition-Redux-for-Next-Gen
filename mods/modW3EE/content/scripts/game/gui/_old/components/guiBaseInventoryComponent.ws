...
abstract class W3GuiBaseInventoryComponent
{
	...
	// W3EE - Begin
	public var m_horseInv : CInventoryComponent;
	public var m_ownerInv : CInventoryComponent;
	...
	public function SetCurrentInventory( invComp : CInventoryComponent )
	{
		_inv = invComp;
	}

	public function SwitchToHorse( bDontShowEquipped : bool )
	{
		_inv = m_horseInv;
		dontShowEquipped = bDontShowEquipped;
	}

	public function SwitchToOwner( bDontShowEquipped : bool )
	{
		_inv = m_ownerInv;
		dontShowEquipped = bDontShowEquipped;
	}
	...
	public function CanDrop( item : SItemUniqueId ):bool
	{
		...
		canDrop = (!_inv.ItemHasTag(item, 'NoDrop') && !_inv.ItemHasTag(item, 'Quest')) || Options().DropQuestItems();
	...
	//Mutagen Sorter - Start
	public function GetInventoryFlashArray( out flashArray : CScriptedFlashArray, flashObject : CScriptedFlashObject, optional mut:array<SItemUniqueId> ) : void
	{
		var i : int;
	...
	public function ShouldShowItem( item : SItemUniqueId ) : bool
	{
		...
		var horse : CNewNPC = thePlayer.GetHorseWithInventory();
		var invent : CEntity = _inv.GetEntity();
		var invent2 : CNewNPC;
		_inv.GetItemTags( item, itemTags );
		
		invent2 = (CNewNPC)invent;
		//Kolaris - Fixed Stashes
		if ( Options().StashDebug() && itemTags.Contains( theGame.params.TAG_DONT_SHOW) && (Equipment().IsStashableItem(_inv, item) || Equipment().IsStashableIngredient(_inv.GetItemName(item))) )
			return true;
		if ( itemTags.Contains( theGame.params.TAG_DONT_SHOW) && GameplayFactsQuerySum("inFixedStash") < 1 )
			return false;
		else if( !(itemTags.Contains( theGame.params.TAG_DONT_SHOW)) && GameplayFactsQuerySum("inFixedStash") > 0 && (W3HorseManager)_inv.GetEntity())
			return false;
		else if( itemTags.Contains( theGame.params.TAG_DONT_SHOW) && ( _inv.ItemHasTag(item, 'NoDrop') || _inv.ItemHasTag(item, 'NoUse') || _inv.ItemHasTag(item, 'EncumbranceOff') || _inv.ItemHasTag(item, 'Quest') || !(Equipment().IsStashableItem(_inv, item) || Equipment().IsStashableIngredient(_inv.GetItemName(item)))) )
			return false;
		
		// W3EE - Begin
		if ( ( _inv.GetEntity() == thePlayer || (W3HorseManager)_inv.GetEntity() ) && _inv.GetItemName(item) == 'Crowns' )
			return true;
	...
	function SetInventoryFlashObjectForItem( item : SItemUniqueId, out flashObject : CScriptedFlashObject) : void
	{
		...
		// W3EE - Begin
		// if ( !_inv.IsItemOil(item) && Equipment().maxOilCharges != 0 )
		// {
			if (_inv.IsItemSingletonItem(item))
			{
				chargesCount = _inv.SingletonItemGetAmmo(item);
				if (chargesCount > 0)
		...
		// W3EE - Begin
		price = _inv.GetItemBasePrice(item) * _inv.GetItemQuantity(item);
		// W3EE - End
		...
		// W3EE - Begin
		flashObject.SetMemberFlashBool( "isSilverOil", _inv.IsItemOil(item) );
		flashObject.SetMemberFlashBool( "isSteelOil", _inv.IsItemOil(item) );
		// W3EE - End
...
/*		
		...
		*/
		...
		Equipment().UnableToEquip(flashObject, _inv, item);
		//W3EE - End
