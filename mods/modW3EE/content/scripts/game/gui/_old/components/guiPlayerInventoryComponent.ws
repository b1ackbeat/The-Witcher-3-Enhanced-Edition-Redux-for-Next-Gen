...
class W3GuiPlayerInventoryComponent extends W3GuiBaseInventoryComponent
{
	...
	public function GetCraftedItemInfo(craftedItemName:name, targetObject:CScriptedFlashObject, optional shouldCompareItems : bool) : void
	{
		...
		// W3EE - Begin
		var levelDifference : int;
		var attackPower, mean	: float;
		...
		itemDesc = Equipment().GetOilAbilityDescriptionByName(craftedItemName);
		itemDesc = itemDesc + GetLocStringByKeyExt(_inv.GetItemLocalizedDescriptionByName(craftedItemName));
		...
		{
			setType = GetWitcherPlayer().GetSetBonusStatusByName( craftedItemName, setBonusDescription1, setBonusDescription2, isSetBonus1Active, isSetBonus2Active );
			
			if( setType == EIST_LightArmor || setType == EIST_MediumArmor || setType == EIST_HeavyArmor )
			{
				setBonusDescription1 = "";
				setBonusDescription2 = "";
			}
			
			if( setBonusDescription1 != "" || setBonusDescription2 != "" )
				setBonusText += StrUpperUTF(Equipment().GetSetTypeString(setType));
			
			if( setBonusDescription1 != "" )
			{
				paramString.PushBack( (string)(Options().SetBonusCountFirst()) );
				setBonusText += "<font color='#9f977a'>" + GetLocStringByKeyExtWithParams( "crafting_bonus_set_title",,, paramString ) + ":</font><br/>"; // Gwynbleidd TTTweak
				setBonusText += "<font color='#818181'>" + Equipment().GetDescriptionColorByType(setBonusDescription1, setType, 1, 1, true) + "</font>";
			}
			...
			if( setBonusDescription2 != "" )
			{
				...
				if( setType == EIST_Gothic || setType == EIST_Meteorite || setType == EIST_Dimeritium )
					paramString.PushBack( (string)( Options().SetBonusCountFirst() ) );
				else
					paramString.PushBack( (string)( Options().SetBonusCountSecond() ) );
				
				if( setBonusDescription1 == "" )
				{
					setBonusText += "<font color='#9f977a'>" + GetLocStringByKeyExtWithParams( "crafting_bonus_set_title",,, paramString ) + ":</font><br/>"; // Gwynbleidd TTTweak
					setBonusText += Equipment().GetDescriptionColorByType(setBonusDescription2, setType, 1, 1, true);
				}
				else
				{
					setBonusText2 += "<font color='#9f977a'>" + GetLocStringByKeyExtWithParams( "crafting_bonus_set_title",,, paramString ) + ":</font><br/>";
					setBonusText2 += Equipment().GetDescriptionColorByType(setBonusDescription2, setType, 1, 1, true);
				}
			}
			//Kolaris - Matched Set Bonus Removal
			/*if( dm.IsItemAnyArmor(craftedItemName) )
			{
				if( setBonusText2 == "" )
				{
					if( setBonusText == "" )
					{
						setBonusText += StrUpperUTF(Equipment().GetSetTypeString(EIST_LightArmor));
						setBonusText += "<br>" + Equipment().GetDescriptionColorByType(Equipment().GetGeneralArmorAbilityDescrByName(craftedItemName), EIST_LightArmor, 1, 1, true);
					}
					else
					{
						setBonusText += "<br><br>" + StrUpperUTF(Equipment().GetSetTypeString(EIST_LightArmor));
						setBonusText += "<br>" + Equipment().GetDescriptionColorByType(Equipment().GetGeneralArmorAbilityDescrByName(craftedItemName), EIST_LightArmor, 1, 1, true);
					}
				}
				else
				{
					setBonusText2 += "<br><br>" + StrUpperUTF(Equipment().GetSetTypeString(EIST_LightArmor));
					setBonusText2 += "<br>" + Equipment().GetDescriptionColorByType(Equipment().GetGeneralArmorAbilityDescrByName(craftedItemName), EIST_LightArmor, 1, 1, true);
				}
			}*/
		...
		setBonusText = Equipment().GetRelicAbilityDescription(craftedItemName);
		if( setBonusText != "" )
		{
			targetObject.SetMemberFlashString("SetBonusDescription", setBonusText);
			targetObject.SetMemberFlashString("SetBonusDescription2", "");
		}
		// W3EE - End
		...
		dontCompare = itemCategory == 'potion' || itemCategory == 'petard' || itemCategory == 'oil' || !shouldCompareItems;
		...
		switch( Equipment().GetArmorTypeFromName(craftedItemName) )
		{
			case EAT_Heavy:
				if( itemDesc == "" )
					itemDesc = GetLocStringById(2116942040);
			break;
			
			case EAT_Medium:
				if( itemDesc == "" )
					itemDesc = GetLocStringById(2116942041);
			break;
			
			case EAT_Light:
				if( itemDesc == "" )
					itemDesc = GetLocStringById(2116942042);
			break;
		...
		{
			primaryStatValue = CeilF(primaryStatValue * 0.754f);
		}
		
		if( itemCategory == 'silversword' || itemCategory == 'steelsword' )
		{
			dm.GetItemAttributeValueNoRandom(craftedItemName, true, 'attack_power', minWeightAttribute, maxWeightAttribute);
			attackPower = 1.f + minWeightAttribute.valueMultiplicative;
			dm.GetItemAttributeValueNoRandom(craftedItemName, true, 'attack_power_fast_style', minWeightAttribute, maxWeightAttribute);
			mean = minWeightAttribute.valueMultiplicative;
			dm.GetItemAttributeValueNoRandom(craftedItemName, true, 'attack_power_heavy_style', minWeightAttribute, maxWeightAttribute);
			mean += minWeightAttribute.valueMultiplicative;
			attackPower += mean / 2.f;
			primaryStatValue *= attackPower;
		}
		...
		if ( !dontCompare )
		{
			...
			if( itemCategory == 'silversword')
			{				
				eqPrimaryStatValue = CeilF(eqPrimaryStatValue * 0.754f);
			}
			
			if( itemCategory == 'silversword' || itemCategory == 'steelsword' )
			{
				minWeightAttribute = playerInv.GetItemAttributeValue(equipedItemId, 'attack_power');
				minWeightAttribute.valueMultiplicative += 1.f;
				maxWeightAttribute = playerInv.GetItemAttributeValue(equipedItemId, 'attack_power_fast_style') + playerInv.GetItemAttributeValue(equipedItemId, 'attack_power_heavy_style');
				minWeightAttribute.valueMultiplicative += (maxWeightAttribute.valueMultiplicative / 2.f);
				eqPrimaryStatValue *= minWeightAttribute.valueMultiplicative;
			}
	...
	public  function SetInventoryFlashObjectForItem( itemId : SItemUniqueId, out flashObject : CScriptedFlashObject) : void
	{
		...
		if (_shopInvCmp)
		{
			...
			// W3EE - Begin
			itemCost = GetInventoryComponent().GetItemBasePrice( itemId );
		...
		else
		{
			...
			dropBlockedByTutorial = false; //FactsQuerySum("tut_forced_preparation") > 0 && _inv.GetItemName(itemId) == 'Thunderbolt 1';
			//Kolaris - Inventory Options
			canDrop = ((!_inv.ItemHasTag(itemId, 'NoDrop') && !_inv.ItemHasTag(itemId,'Quest')) || Options().DropQuestItems()) && !dropBlockedByTutorial && !isDefaultBolt;
		}
	...
	private function CheckIfShouldShowItem( item : SItemUniqueId ) : bool
	{
		...
		switch( _filterType )
		{
			...
			case IFT_AlchemyItems:
				shouldShow = isAlchemyItem( item ) && !isFoodItem( item ) && !isItemUsable( item ) && !IsItemDye( item ) && !_inv.ItemHasTag(item, 'ThrowingKnife');
				break;
