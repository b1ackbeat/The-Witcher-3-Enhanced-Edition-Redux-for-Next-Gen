...
class W3TooltipComponent 
{
	...
	// W3EE - Begin
	protected var m_ownerInv			: CInventoryComponent;
	protected var m_horseInv			: CInventoryComponent;
	
	public function SwitchToHorse()
	{
		m_playerInv = m_horseInv;
	}

	public function SwitchToOwner()
	{
		m_playerInv = m_ownerInv;
	}
	
	public function IsHorse() : bool
	{
		return m_playerInv == m_horseInv;
	}

	public function IsOwner() : bool
	{
		return m_playerInv == m_ownerInv;
	}
	// W3EE - End
	...
	{
		// W3EE - Begin
		m_horseInv = GetWitcherPlayer().GetHorseManager().GetInventoryComponent();
		m_ownerInv = inventory;
		// W3EE - End
	...
	public function GetBaseItemData( item : SItemUniqueId, itemInvComponent : CInventoryComponent, optional isShopItem : bool, optional compareWithItem : SItemUniqueId, optional compareItemInv : CInventoryComponent ) : CScriptedFlashObject
	{
		...
		if( GetWitcherPlayer().IsSelectionActive() )
			itemInvComponent = m_playerInv;
		...
		itemLabel = itemInvComponent.GetItemLocNameByID(item);
		...
		else
		{
			...
			// W3EE - Begin
			if (itemInvComponent.IsItemFood(item) )
			{
				itemInvComponent.GetPotionAttributesForTooltip(itemName, itemStats);
			}
			else if (itemInvComponent.IsItemPotion(item))
			{
				itemInvComponent.GetPotionAttributesForTooltip(itemName, itemStats);
				for (i = itemStats.Size() - 1; i >= 0; i -= 1)
				{
					if (itemStats[i].originName == 'duration')
						itemStats[i].value = RoundMath(wplayer.CalculatePotionDuration(item, definitionsMgr.ItemHasTag(itemName, 'Decoction') ));
				}
			}
			// W3EE - End
		...
		{
			additionalDescription += GetGwintCardDescription(wplayer.GetGwentCardIndex(itemName));
		}
		...
		AddItemStats(itemStats, statsList, tooltipData, ignorePrimaryStat, itemAttributePrefix, (isArmorOrWeapon || itemInvComponent.ItemHasTag(item, 'ArmorUpgrade')), itemInvComponent.ItemHasTag(item, 'Decoction'), itemInvComponent.ItemHasTag(item, 'Oriole'));
		...
		if( itemInvComponent.IsItemSingletonItem( item ) && ( itemInvComponent.SingletonItemGetMaxAmmo(item) > 0 ) )
		{
			if( itemInvComponent.IsItemOil( item ) )
			{
				...
				{
					// W3EE - Begin
					ammoBonus = 0.3; // CalculateAttributeValue( thePlayer.GetSkillAttributeValue( S_Alchemy_s06, 'ammo_bonus', false, false ) );
					ammoBonus = ammo * (1 + ammoBonus * thePlayer.GetSkillLevel( S_Alchemy_s06 ) ) - ammo;
					tmpStr = NoTrailZeros( ammo ) + " ( +" + NoTrailZeros( ammoBonus ) + " ) ";
				...
				{
					// W3EE - Begin
					tmpStr = NoTrailZeros( ammo ) + " ";
					//tooltipData.SetMemberFlashString( "charges", NoTrailZeros( ammo ) + " " + GetLocStringByKeyExt( "inventory_tooltip_charges" ) );
					// W3EE - End
				}
				// W3EE - Begin
				tmpStr = tmpStr + GetLocStringByKeyExt( "inventory_tooltip_charges" ) + " [" + itemInvComponent.SingletonItemGetAmmo( item ) + "/" + itemInvComponent.SingletonItemGetMaxAmmo( item ) + "]";
				tooltipData.SetMemberFlashString( "charges", tmpStr );
				// W3EE - End
			}
			else
			{
		...
		*/
		...
		if( itemInvComponent.ItemHasTag(item, 'SlottedHelm') )
		{
			primaryStatLabel = 'armor';
			primaryStatOriginValue = 354;
		}
		
		if (isArmorOrWeapon && !itemInvComponent.ItemHasTag(item, 'Artifact_weapon') )//W3EE
		{
			primaryStatOriginValue = RoundMath(primaryStatOriginValue);
			// W3EE - Begin
			ApplyStatModifiers(wplayer, itemInvComponent, item, primaryStatOriginValue);
			...
			if (canBeCompared)
			{
				...
				// W3EE - Begin
				ApplyStatModifiers(wplayer, compareItemInv, equipedItem, eqPrimaryStatValue);
				eqPrimaryStatValue = RoundMath(eqPrimaryStatValue);
				// W3EE - End
			}
			else
		...
		{
			if( itemInvComponent.ItemHasTag(item, 'ThrowingKnife') )
				categoryLabel = GetLocStringByKeyExt("w3ee_throwing_knife");
			else
		...
		uniqueDescription = "";
		// W3EE - Begin
		if( definitionsMgr.ItemHasTag(itemName, 'Weapon') )
			uniqueDescription = Equipment().GetRelicAbilityDescription(itemInvComponent.GetItemName(item));
		
		if( definitionsMgr.ItemHasTag(itemName, 'Artifact_weapon') )
		{
			uniqueDescription += GetLocStringByKeyExt("W3EE_artifact_description");
		}	
		if( definitionsMgr.ItemHasTag(itemName, 'SilverOil') )
			uniqueDescription = Equipment().GetOilAbilityDescription(item, itemInvComponent);
		
		if (definitionsMgr.ItemHasTag(itemName, 'Potion') ) 
		{
			if( definitionsMgr.ItemHasTag(itemName, 'Nigredo') )	
				uniqueDescription += GetLocStringByKeyExt("primer_susbstance_dominance") + " <font color=\"#4F4F4F\">" + GetLocStringByKeyExt("primer_nigredo") + ".<br/>" + "</font>";	
			else
			if( definitionsMgr.ItemHasTag(itemName, 'Rubedo') )
				uniqueDescription += GetLocStringByKeyExt("primer_susbstance_dominance") + " <font color=\"#C52C34\">" + GetLocStringByKeyExt("primer_rubedo") + ".<br/>" + "</font>";
			else
			if( definitionsMgr.ItemHasTag(itemName, 'Albedo') )
				uniqueDescription += GetLocStringByKeyExt("primer_susbstance_dominance") + " <font color=\"#FFFFFF\">" + GetLocStringByKeyExt("primer_albedo") + ".<br/>" + "</font>";
		}
		
		if( !isArmorOrWeapon || definitionsMgr.ItemHasTag(itemName, 'ThrowingKnife') )
		{
		...
		*/
		...
		if( definitionsMgr.ItemHasTag(itemName, 'AlchemyIngredient') )
		{
			alchMenu = new CR4AlchemyMenu in this;
			alchMenu.SetAlchemyCategories();
			uniqueDescription += " " + alchMenu.GetIngredientCategory(itemName, true);
		}
		...
		if (categoryName == 'armor'|| categoryName == 'pants' || categoryName == 'boots' || categoryName == 'gloves')
		{
			...
			//armorTypeGlyphWordBonus = (wplayer.HasAbility('Glyphword 2 _Stats', true) || wplayer.HasAbility('Glyphword 3 _Stats', true) || wplayer.HasAbility('Glyphword 4 _Stats', true) || wplayer.HasAbility('Glyphword 9 _Stats', true)) && wplayer.IsItemEquipped(item);			
		...
		// W3EE - Begin
		/*if( Options().LevelRequirement() && Options().ScalingCheck() != 2 )
		{
		...
		if (categoryName == 'alchemy_recipe' )
		{
			...
			m_recipeList     = wplayer.GetAlchemyRecipes();
		...
		else if (categoryName == 'crafting_schematic' )
		{
			...
			m_schematicList = wplayer.GetCraftingSchematicsNames();			
			itemInvComponent.GetAllItems( allItems );
		...
		invItemPriceString = FloatToStringPrec( weightValue, 2 );
		//---=== modFriendlyHUD ===---
		if(GetFHUDConfig().showStashItemQuantity)
		{
			if(itemInvComponent == m_playerInv)
			{
				...
				{
					tmpStr = "(" + IntToString(durabilityValue) + " " + GetLocStringByKey("Redux_item_quant_on_Roach");
					durabilityValue = wplayer.GetHorseManager().GetInventoryComponent().GetItemQuantityByName(Equipment().GetItemStashVariant(itemName));
					if(durabilityValue > 0)
					{
						invItemPriceString = tmpStr + ", " + IntToString(durabilityValue) + " " + GetLocStringByKey("fhud_item_quant_in_stash") + ") " + invItemPriceString;
					}
					else
					{
						invItemPriceString = tmpStr + ") " + invItemPriceString;
					}
				}
				else
				{
					...
					if(durabilityValue > 0)
					{
						invItemPriceString = "(" + IntToString(durabilityValue) + " " + GetLocStringByKey("fhud_item_quant_in_stash") + ") " + invItemPriceString;
					}
				}
			}
			else if(itemInvComponent == m_shopInv)
			{
				durabilityValue = m_playerInv.GetItemQuantityByName(itemName) + wplayer.GetHorseManager().GetInventoryComponent().GetItemQuantityByName(itemName) + wplayer.GetHorseManager().GetInventoryComponent().GetItemQuantityByName(Equipment().GetItemStashVariant(itemName));
				if(durabilityValue > 0)
				{
					invItemPriceString = "(" + IntToString(durabilityValue) + " " + GetLocStringByKey("fhud_item_quant_owned") + ") " + invItemPriceString;
				}
			}
		}
		//---=== modFriendlyHUD ===---
		addGFxItemStat(propsList, "weight", invItemPriceString, "attribute_name_weight");
		...
		if ( m_shopInv )
		{
			...
			{
				// W3EE - Begin
				invItemPrice = m_shopInv.GetItemSellPrice(item);
				// W3EE - End
				...
				if(GetFHUDConfig().showTrueItemPrice)
				{
					...
					invItemPrice = itemInvComponent.GetItemBasePrice(item);
			...
			else
			{
				...
				// W3EE - Begin
				invItemPrice = m_shopInv.GetInventoryItemBasePriceModified( invItem, m_playerInv, true, m_shopInv.GetEntity() );
				// W3EE - End
		...
		{
			// W3EE - Begin
			invItemPrice = itemInvComponent.GetItemSellPrice( item );
			// W3EE - End
			invItemPriceString = invItemPrice;
		...
		// W3EE - Begin
		tooltipData.SetMemberFlashBool("EnableFullScreenInfo", isArmorOrWeapon || itemInvComponent.ItemHasTag(item, 'SlottedHelm'));
		// W3EE - End
	...
	private function AddSetAttributes( itemId : SItemUniqueId, itemInvComponent : CInventoryComponent, out flashDataObj : CScriptedFlashObject ):bool
	{
		...
		if( setType != EIST_Undefined )
		{
			...
			requiredCount = Equipment().GetMaximumSetItemsBySet(setType);
			
			counterText = Equipment().GetSetCountStringColor(itemId, currentCount + "/" + requiredCount);
			...
			setAttribute.SetMemberFlashString( "name", Equipment().GetSetTypeString(setType) );
			setAttribute.SetMemberFlashBool( "active", false );
			//setAttributesList.PushBackFlashObject( setAttribute ); // Gwynbleidd TTTWeak
			
			if( desc1 != "" )
			{
				setAttribute = flashDataObj.CreateFlashObject();
				setAttribute.SetMemberFlashString( "value", Equipment().GetDescriptionColorByType(Options().SetBonusCountFirst(), setType, 1, currentCount, isActive1) );
				setAttribute.SetMemberFlashString( "name", Equipment().GetDescriptionColorByType(desc1, setType, 1, currentCount, isActive1) );
				setAttribute.SetMemberFlashBool( "active", false );
			...
			if( desc2 != "" )
			{
				...
				setAttribute = flashDataObj.CreateFlashObject();
				//Kolaris - Set Bonus Count Option
				setAttribute.SetMemberFlashString( "value", Equipment().GetDescriptionColorByType(Options().SetBonusCountSecond(), setType, 2, currentCount, isActive2) );
				setAttribute.SetMemberFlashString( "name", Equipment().GetDescriptionColorByType(desc2, setType, 2, currentCount, isActive2) );
				setAttribute.SetMemberFlashBool( "active", false );
				setAttributesList.PushBackFlashObject( setAttribute );
			}
			//Kolaris - Matched Set Bonus Removal
			/*if( setType != EIST_LightArmor && setType != EIST_MediumArmor && setType != EIST_HeavyArmor && GetWitcherPlayer().inv.IsItemAnyArmor(itemId) )
			{
				setAttribute = flashDataObj.CreateFlashObject();
				setAttribute.SetMemberFlashString( "value", "" );
				setAttribute.SetMemberFlashString( "name", Equipment().GetSetTypeString(EIST_LightArmor) );
				setAttribute.SetMemberFlashBool( "active", false );
				setAttributesList.PushBackFlashObject( setAttribute );
	...
	private function GetCrossbowPrimatyStat(itemId : SItemUniqueId, itemInvComponent : CInventoryComponent, out primaryStatLabel : string, out primaryStatValue : float):void
	{
		...
		itemExists = GetWitcherPlayer().GetItemEquippedOnSlot(EES_Bolt, itemOnSlot);
		if( itemExists )
		{
	...
	private function AddOilInfo(itemId : SItemUniqueId, itemInvComponent : CInventoryComponent, out flashDataObj : CScriptedFlashObject):void
	{
		...
		for( i = 0; i < count; i+=1 )
		{
			...
			if (oilName != '')
			{
				...
				oilCharge = CeilF(appliedOil.GetAmmoPercentage() * 100.f);
				
				if(oilCharge <= 0)
				{
					oilBonusValue = "<font color=\"#FF0000\">" + oilLocName + " (" + oilCharge + "%)"+ "</font>";
				}
				else
				{
					oilBonusValue =  oilLocName + " (" + oilCharge + "%)";
				}
	...
	private function AddItemStats(itemStats : array<SAttributeTooltip>, out resultGFxArray : CScriptedFlashArray, rootGFxObject : CScriptedFlashObject, ignorePrimaryStat : bool, defaultPrefix : string, isArmorOrWeapon : bool, isDecoction : bool, isOriole : bool):void
	{
		...
		for( i = 0; i < statsCount; i += 1) 
		{
			...
			if (!ignorePrimaryStat || !currentStat.primaryStat)
			{
				...
				if (currentStat.originName == 'toxicity_offset')
				{
					...
					if( isDecoction && thePlayer.CanUseSkill(S_Alchemy_s14) )
						currentStat.value -= 5 * thePlayer.GetSkillLevel(S_Alchemy_s14);
				}
				if (currentStat.originName == 'toxicity')
				{
					if( !isDecoction && !isArmorOrWeapon )
						currentStat.value = GetWitcherPlayer().GetFinalPotionToxicity(currentStat.value);
					if( isDecoction )
						currentStat.value = currentStat.value * Options().GetToxicityMultiplier() - (5 * thePlayer.GetSkillLevel(S_Alchemy_s14));
					
					if (!isArmorOrWeapon && currentStat.attributeType != 'base')
					{
				...
				//Kolaris - Swallow
				if (currentStat.originName == 'duration' || currentStat.originName == 'heal_injury_time')
				{
					valuePrefix = "";
					valuePostfix = " " + GetLocStringByKeyExt("per_second");
				}				
				else
				if (currentStat.originName == 'oil_silver_tooltip' || currentStat.originName == 'poison_stack_timer_reduction' || currentStat.originName == 'bleed_stack_timer_reduction' || currentStat.originName == 'active_toxicity_drain' || currentStat.originName == 'dormant_toxicity_drain' || currentStat.originName == 'poison_buildup_resist')
				{
					valuePrefix = "";
					valuePostfix = "";
				}				
				...
				{
					// W3EE - Begin
					currentStat.attributeName = GetLocStringByKeyExt("W3EE_Vigor");
					// W3EE - End
				}
				...
				// W3EE - Begin
				if(currentStat.percentageValue && StrContains(currentStat.originName, "vitality"))
				{
					valueString = NoTrailZeros(RoundTo(currentStat.value * 100, 1)) + " %";
				}
				...
				{
					valueString = NoTrailZeros( RoundTo( currentStat.value, 1 ) );
				}
				// W3EE - End
	...
	private function AddBuffStats(itemId : SItemUniqueId, itemInvComponent : CInventoryComponent, out resultGFxArray : CScriptedFlashArray, rootGFxObject : CScriptedFlashObject) : void
	{
		...
		if( /*itemInvComponent.GetItemName(itemId) == 'Silver Dust Bomb 3'*/ itemInvComponent.ItemHasTag(itemId, 'HideLoopDuration') )
			return;
			...
			if (!((curStat.statName == "duration" || curStat.statName == "toxicity" ) && RoundMath(curStat.value) <= 1))
			{
				...
				if (curStat.statName == "toxicity" || curStat.statName == "toxicity_offset" )
				{
					...
					if ( curStat.statName == "toxicity_offset" ) 
						...
						curStatData.SetMemberFlashString("value", RoundMath(GetWitcherPlayer().GetFinalPotionToxicity(curStat.value)) /*+ "/" + maxToxicity*/);
				}
	...
	public function GetAerondightTooltipDescription( sword : SItemUniqueId ) : string
	{
		...
		argsString.PushBack( "<font color='#ffffff'>" + NoTrailZeros(RoundF(min.valueAdditive * 100)) + "</font>" );
		...
		uniqueDesc += "<br><br>" + GetLocStringByKeyExt("attribute_name_aerondight_abilities");
...
function CalculateStatsComparance(itemStats : array<SAttributeTooltip>, compareItemStats : array<SAttributeTooltip>, rootGFxObject : CScriptedFlashObject, out compResult : CScriptedFlashArray, optional ignorePrimStat : bool, optional dontCompare : bool, optional extendedData:bool)
{
	...
	for( i = 0; i < statsCount; i += 1 ) 
	{
		if (!(itemStats[i].primaryStat && ignorePrimStat))
		{
			...
			{			
				for( j = 0; j < compareItemStats.Size(); j += 1 )
				{
					...
					if( itemStats[i].attributeName == compareItemStats[j].attributeName )
					{
						...
						{
							nDifference = RoundTo(itemStats[i].value * 100, 1) - RoundTo(compareItemStats[j].value * 100, 1);
							percentDiff = AbsF( nDifference / RoundTo(itemStats[i].value * 100, 1) );
						}
						else
						{
							nDifference = RoundTo(itemStats[i].value, 1) - RoundTo(compareItemStats[j].value, 1);
							percentDiff = AbsF( nDifference / RoundTo(itemStats[i].value, 1) );
						}
						...
						{
							if( itemStats[i].percentageValue )						
								strDiffValue = strDiffPrefix + NoTrailZeros(RoundTo(nDifference, 1)) + " %" + strDiffPostfix;
							else
								strDiffValue = strDiffPrefix + NoTrailZeros(RoundTo(nDifference, 1)) + strDiffPostfix;
						}
				...
				if (strDiffValue == "" && !statToCompareExist)
				{
					...
					else strDifference = "none";
					// W3EE - End
					if( itemStats[i].percentageValue )
					{
						strDiffValue = strDiffPrefix + NoTrailZeros(RoundTo(itemStats[i].value *100, 1)) + " %" + strDiffPostfix;
					}
					else
					{
						strDiffValue = strDiffPrefix + NoTrailZeros(RoundTo(itemStats[i].value, 1)) + strDiffPostfix;
					}
				}
			}
			else
			...
			if( itemStats[i].percentageValue )
			{
				l_flashObject.SetMemberFlashString("value", valuePrefix + NoTrailZeros(RoundTo( itemStats[i].value * 100, 1 )) + " %" + valuePostfix );
			}
			
			else
			{
				l_flashObject.SetMemberFlashString("value", valuePrefix + NoTrailZeros(RoundTo( itemStats[i].value, 1 )) + valuePostfix );
			}
	...
	if (!dontCompare)
	{
		...
		for( j = 0; j < compareItemStats.Size(); j += 1 )
		{
			if (!(compareItemStats[j].primaryStat && ignorePrimStat))
			{
				...
				{
					compareItemStats[j].value = AbsF(compareItemStats[j].value);
					strDiffPrefix = "<font color=\"#19D900\"> +";
					strDifference = "better";
				}
				else
				...
				if( compareItemStats[j].percentageValue )
				{
					...
					diffValue = strDiffPrefix + NoTrailZeros(RoundTo( compareItemStats[j].value * 100, 1 )) + " %</font>";
				}
				else
				{
					strValue = valuePrefix + "0";
					diffValue = strDiffPrefix + NoTrailZeros(RoundTo(compareItemStats[j].value, 1)) + "</font>";
				}
