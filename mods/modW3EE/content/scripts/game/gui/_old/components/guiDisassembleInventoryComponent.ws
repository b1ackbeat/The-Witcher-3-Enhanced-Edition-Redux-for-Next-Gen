...
class W3GuiDisassembleInventoryComponent extends W3GuiPlayerInventoryComponent
{
	...
	public  function ShouldShowItem( item : SItemUniqueId ):bool
	{
		...
		if( (GetWitcherPlayer().IsItemEquipped( item )  && !IsHorse()) || ( _inv.IsItemReadable( item ) && _inv.IsBookRead( item ) ) )//Dismantle Fix
		{
			return false;
		}
	...
	private function addRecyclingPartsList( item : SItemUniqueId, out flashObject : CScriptedFlashObject ) : void
	{
		...
		flashObject.SetMemberFlashBool("disableAction", (GetWitcherPlayer().IsItemEquipped(item) && !IsHorse() ));//Dismantle Fix
		
		itemQuantity = _inv.GetItemQuantity( item );
		// W3EE - Begin
		itemQuantityPrice = merchantInv.GetItemBasePriceDisassemble(invItem, _inv);
		// W3EE - End
