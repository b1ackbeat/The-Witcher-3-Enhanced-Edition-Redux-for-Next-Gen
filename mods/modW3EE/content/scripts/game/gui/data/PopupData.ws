		...
		if (KeyCode == "enter-gamepad_A")
		{
			switch(actionType)
			{
				...
				case QTF_DropHorse:
					...
					curInventory = GetWitcherPlayer().GetHorseManager().GetInventoryComponent();
					
					curInventory.HorseDropItemInBag(itemId, currentValue);
					
					if( currentValue == maxValue )
					{
						inventoryRef.ShopRemoveItem(itemId);
					}
					else
					{
						inventoryRef.StashUpdateItem(itemId);
					}
			...
			for( i = 0; i < count; i += 1 )
			{
				...
				if( !curInventory.IsIdValid( bookItemId ) || bookItemId != curItemId )
				{
					...
					if( !isItemRecipe && curInventory.ItemHasTag( curItemId, 'ReadableItem' ) && !curInventory.ItemHasTag( curItemId, 'NoShow' ) && !curInventory.IsBookRead( curItemId ) )
					{
						...
						objBookInfo.SetMemberFlashString( "TextTitle", curInventory.GetItemLocNameByID( curItemId ) );
