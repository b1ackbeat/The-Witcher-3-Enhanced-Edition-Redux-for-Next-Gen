...
{	
	...
	private function ShowItemInfo(itemId : SItemUniqueId):void
	{
		...
		itemName = playerInv.GetItemLocNameByID( itemId );
	...
	private function ShowItemInfoPopup( itemId : SItemUniqueId )
	{
		...
		if( playerInv.ItemHasTag( itemId, 'Painting' ) )
		{
			...
			m_paintingPopupData.SetMessageTitle( playerInv.GetItemLocNameByID( itemId ) );
		...
		else
		{
			...
			m_bookPopupData.SetMessageTitle( playerInv.GetItemLocNameByID(itemId) );
