...
class CR4HudModuleItemInfo extends CR4HudModuleBase
{
	...
	public var m_fxHideSlotsSFF : CScriptedFlashFunction; // -= WMK:modQuickSlots =-
	private var m_fxSetAlwaysDisplayed : CScriptedFlashFunction;
	private var m_flashValueStorage : CScriptedFlashValueStorage;
	public var m_fxSetItemInfo : CScriptedFlashFunction; // -= WMK:modQuickSlots =-
	...
	event OnTick( timeDelta : float )
	{
		...
		{
			// -= WMK:modQuickSlots =-
			if (WmkGetQuickSlotsInstance().OnItemInfoModuleTick(timeDelta, false)) {
		...
		else
		{
			...
			if ( theInput.LastUsedGamepad() && WmkGetQuickSlotsInstance().CanFlipEquippedItems() ) // -= WMK:modQuickSlots =-
			{
			...
			forcedIconUpdate = forcedIconUpdate || WmkGetQuickSlotsInstance().m_forceUpdateAllStdSlots; // -= WMK:modQuickSlots =-
		...
		WmkGetQuickSlotsInstance().OnItemInfoModuleTick(0, true); // -= WMK:modQuickSlots =-
	}
	...
	public function UpdateItem( out currItem : SItemUniqueId, out prevItem : SItemUniqueId, out prevItemAmmo : int, bindingID : HudItemInfoBinding, slotId : int, optional forceUpdate:bool )
	{
		...
		if ( thePlayer.GetInventory().IsIdValid( currItem ) )
		{
			...
			else if( thePlayer.GetInventory().ItemHasTag( currItem, 'Edibles' ) || thePlayer.GetInventory().ItemHasTag( currItem, 'ThrowingKnife' ) )
			{				
	...
	public function UpdateItemData(item : SItemUniqueId, bindingID : HudItemInfoBinding)
	{
		...
		else
		{
			...
			itemName = inventory.GetItemLocNameByID( item );
			// itemName = GetLocStringByKeyExt( itemName );
			fontColor = "<font color=\"#FFFFFF\">";
			
			if( inventory.ItemHasTag(item, 'Edibles' ) || thePlayer.GetInventory().ItemHasTag(item, 'ThrowingKnife') )
			{
				// W3EE - Begin
				/*
	...
	private function GetKeyByBinding(bindingName : HudItemInfoBinding) : int
	{
		...
		// -= WMK:modQuickSlots =-
		if (WmkGetQuickSlotsInstance().GetKeyByBindingForStdSlot(bindingName, outKeys)) {
			return outKeys[0];
		}
		// -= WMK:modQuickSlots =-
