...
statemachine import class CNewNPC extends CActor
{
	...
	// W3EE - Begin
	public var levelBonusesComputedAtPlayerLevel : int; 		default levelBonusesComputedAtPlayerLevel = -1;
	// W3EE - End
	...
	public var npcLevelToUpscaledLevelDifference	: int;
	...
	private var weatherAbilityHandler : W3EEAbilityHandler;
	...
	event OnPreAttackEvent(animEventName : name, animEventType : EAnimationEventType, data : CPreAttackEventData, animInfo : SAnimationEventAnimInfo )
	{
		...
		var i: int; //Plasticmetal - yrdenfix
		var yrdens: array<W3YrdenEntity>;//Plasticmetal - yrdenfix
		...
		if(animEventType == AET_DurationStart )
		{
			...
			if(GetTarget() == witcher )
			{
				...
				if( (blizzard) && (thePlayer.IsActionAllowed(EIAB_Dodge) || thePlayer.IsActionAllowed(EIAB_Roll)) )
					blizzard.KilledEnemy();
				
				//Plasticmetal - yrdenfix++
				yrdens = GetWitcherPlayer().yrdenEntities;
				for(i=0; i<yrdens.Size(); i+=1)
				{
					yrdenAlternate = (W3YrdenEntityStateYrdenShock)yrdens[i].GetCurrentState();
					yrdenAlternate.SetWardingGlyphTargets(this);
				}
				//Plasticmetal - yrdenfix--
			}
			...
			if ( IsCountering() )
			{
				if(GetTarget() == witcher && ( thePlayer.IsActionAllowed(EIAB_Dodge) || thePlayer.IsActionAllowed(EIAB_Roll) ) && witcher.GetStat(BCS_Toxicity, false) > theGame.params.TOXICITY_DAMAGE_THRESHOLD && witcher.CanUseSkill(S_Alchemy_s16))
					witcher.StartFrenzy();
			}
			*/
			// W3EE - End
	...
	event OnSpawned(spawnData : SEntitySpawnData )
	{
		...
		// W3EE - Begin
		Enemies().CacheNPCData(this, npcStats);
		// W3EE - End
	...
	timer function WeatherBonusCheck(dt : float, id : int)
	{
		...
		if (curEffect)
		{
			...
			{
				WeatherBonusToStatIncrease(curEffect.GetAbilityName(), true);
		...
		if (bonusName != 'None')
		{
			...
			WeatherBonusToStatIncrease(bonusName, false);
	...
	*/
	...
	protected function PerformParryCheck(parryInfo : SParryInfo) : bool
	{
		...
		var fistFightParry,a,b : bool;
		
        if ( !parryInfo.canBeParried || thePlayer.IsCounterAttack(parryInfo.attackActionName) )
			return false;
		a = a(); b = b();
		// W3EE - Begin
		if( ((W3Effect_NPCPoise)GetBuff(EET_NPCPoise)).IsPoiseBroken() )
			return false;
		if( IsHuman() && ((CHumanAICombatStorage)this.GetScriptStorageObject('CombatData')).IsProtectedByQuen() )
			return false;
		// W3EE - End
		if( !CanParryAttack() )
			return false;
	 ...
	 event OnProcessActionPost(action : W3DamageAction)
	{
		...
		//Kolaris - Posession
		/*if(HasBuff(EET_AxiiGuardMe) && (thePlayer.HasAbility('Glyphword 18 _Stats', true) || thePlayer.HasAbility('Glyphword 13 _Stats', true)) && action.DealtDamage())
		{
	...
	// W3EE - Begin
	public var npcStats : SOpponentStats;
	private var wasScaled : bool;	default wasScaled = false;
	
	public function IsImmuneToBuff( effect : EEffectType ) : bool
	{
		var resPrc : float;
		
		resPrc = GetNPCCustomStat(ResistStatToName(theGame.effectMgr.GetBuffResistStat(effect)));
		
		return super.IsImmuneToBuff(effect) || resPrc >= 1.f;
	}
	
	public function ResistStatToName( resistStat : ECharacterDefenseStats ) : name
	{
		switch(resistStat)
		{
			...
			case CDS_RendingRes:				return theGame.params.DAMAGE_NAME_PHYSICAL;
			
			case CDS_BleedingRes:
			case CDS_DoTBleedingDamageRes:		return theGame.params.DAMAGE_NAME_BLEEDING;
			
			case CDS_PoisonRes:
			case CDS_DoTPoisonDamageRes:		return theGame.params.DAMAGE_NAME_POISON;
			
			case CDS_FireRes:
			case CDS_BurningRes:
			case CDS_DoTBurningDamageRes:		return theGame.params.DAMAGE_NAME_FIRE;
			
			case CDS_FrostRes:
			case CDS_FreezeRes:					return theGame.params.DAMAGE_NAME_FROST;
			
			case CDS_ShockRes:					return theGame.params.DAMAGE_NAME_SHOCK;
			case CDS_ElementalRes:				return theGame.params.DAMAGE_NAME_ELEMENTAL;
			
			case CDS_ForceRes:					return theGame.params.DAMAGE_NAME_FORCE;
			
			case CDS_WillRes:					return theGame.params.DAMAGE_NAME_MENTAL;
			
			default: return '';
		}
	}
	
	public function GetNPCCustomStat( damageName : name ) : float
	{
		switch(damageName)
		{		
			case theGame.params.DAMAGE_NAME_FROST 		: return npcStats.frostResist;
			
			case theGame.params.DAMAGE_NAME_FORCE 		: return npcStats.forceResist;
			
			case theGame.params.DAMAGE_NAME_POISON 		: return npcStats.poisonResist;
			
			case theGame.params.DAMAGE_NAME_FIRE 		: return npcStats.fireResist;
			
			case theGame.params.DAMAGE_NAME_SLOW 		: return npcStats.slowResist;
			
			case theGame.params.DAMAGE_NAME_BLEEDING	: return npcStats.bleedingResist;
			
			case theGame.params.DAMAGE_NAME_MENTAL 		: return npcStats.confusionResist;
			
			case theGame.params.DAMAGE_NAME_STUN 		: return npcStats.stunResist;
			
			case theGame.params.DAMAGE_NAME_INJURY 		: return npcStats.injuryResist;
			
			case theGame.params.DAMAGE_NAME_ARMOR_PIERCE : return npcStats.armorPiercing;
			
			case theGame.params.DAMAGE_NAME_ARMOR_PIERCE_RANGED : return npcStats.rangedArmorPiercing;
			...
			case theGame.params.DAMAGE_NAME_RENDING		: return npcStats.physicalResist;
			
			case theGame.params.DAMAGE_NAME_ELEMENTAL	: return npcStats.elementalResist;
			
			case theGame.params.DAMAGE_NAME_SHOCK 		: return npcStats.shockResist;
			
			case theGame.params.DAMAGE_NAME_MORALE 		:
			case theGame.params.DAMAGE_NAME_DIRECT 		: return 0.f;
			default 									: return 0.f;
		}
	}
	
	public function SetNPCCustomStat( damageName : name, value : float )
	{
		switch(damageName)
		{		
			case theGame.params.DAMAGE_NAME_FROST 		: npcStats.frostResist = value;	break;
			
			case theGame.params.DAMAGE_NAME_FORCE 		: npcStats.forceResist = value;	break;
			
			case theGame.params.DAMAGE_NAME_POISON 		: npcStats.poisonResist = value;	break;
			
			case theGame.params.DAMAGE_NAME_FIRE 		: npcStats.fireResist = value;	break;
			
			case theGame.params.DAMAGE_NAME_BLEEDING 	: npcStats.bleedingResist = value;	break;
			
			case theGame.params.DAMAGE_NAME_SLOW 		: npcStats.slowResist = value;	break;
			
			case theGame.params.DAMAGE_NAME_MENTAL 		: npcStats.confusionResist = value;	break;
			
			case theGame.params.DAMAGE_NAME_STUN 		: npcStats.stunResist = value;	break;
			
			case theGame.params.DAMAGE_NAME_INJURY 		: npcStats.injuryResist = value;	break;
			
			case theGame.params.DAMAGE_NAME_ARMOR_PIERCE : npcStats.armorPiercing = value;	break;
			
			case theGame.params.DAMAGE_NAME_ARMOR_PIERCE_RANGED : npcStats.rangedArmorPiercing = value;	break;
			...
			case theGame.params.DAMAGE_NAME_PIERCING	:
			case theGame.params.DAMAGE_NAME_BLUDGEONING	:
			case theGame.params.DAMAGE_NAME_RENDING		: npcStats.physicalResist = value;	break;
			
			case theGame.params.DAMAGE_NAME_ELEMENTAL	: npcStats.elementalResist = value;	break;
			
			case theGame.params.DAMAGE_NAME_SHOCK 		: npcStats.shockResist = value;	break;
			
			default : return;
		}
	}
	
	public function AddDamageImmunity( damageName : name )
	{
		...
		switch(damageName)
		{		
			case theGame.params.DAMAGE_NAME_FROST 		: effects.PushBack(EET_Frozen);	effects.PushBack(EET_SlowdownFrost);	break;
			
			case theGame.params.DAMAGE_NAME_FORCE 		: effects.PushBack(EET_Knockdown); effects.PushBack(EET_HeavyKnockdown);	break;
			
			case theGame.params.DAMAGE_NAME_POISON 		: effects.PushBack(EET_Poison);	break;
			
			case theGame.params.DAMAGE_NAME_FIRE 		: effects.PushBack(EET_Burning);	break;
			
			case theGame.params.DAMAGE_NAME_BLEEDING 	: effects.PushBack(EET_Bleeding);	break;
			
			case theGame.params.DAMAGE_NAME_SLOW 		: effects.PushBack(EET_Slowdown);	break;
			
			case theGame.params.DAMAGE_NAME_MENTAL 		: effects.PushBack(EET_Hypnotized); effects.PushBack(EET_Confusion); effects.PushBack(EET_AxiiGuardMe);	break;
			
			case theGame.params.DAMAGE_NAME_INJURY 		: npcStats.injuryResist = 1.f;	break;
			
			case theGame.params.DAMAGE_NAME_STUN 		:
			case theGame.params.DAMAGE_NAME_PHYSICAL 	:
			case theGame.params.DAMAGE_NAME_SILVER 		:
			case theGame.params.DAMAGE_NAME_SLASHING	:
			case theGame.params.DAMAGE_NAME_PIERCING	:
			case theGame.params.DAMAGE_NAME_BLUDGEONING	:
			case theGame.params.DAMAGE_NAME_RENDING		: effects.PushBack(EET_Stagger); effects.PushBack(EET_LongStagger);	break;
			
			case theGame.params.DAMAGE_NAME_ELEMENTAL	:
			case theGame.params.DAMAGE_NAME_SHOCK 		: effects.PushBack(EET_VitalityDrain);	break;
			
			default : return;
		}
		
		for(i=0; i<effects.Size(); i+=1)
			AddBuffImmunity(effects[i], 'Resistances', false);
	}
	
    public function HasTwoHandedWeapon() : bool
    {
		switch(npcStats.meleeWeapon)
		{
			case EMWT_Spear:
			case EMWT_Halberd:
			case EMWT_Halberd:
	...
	public function GetNPCStats() : SOpponentStats
	{
		return npcStats;
	}
	
	public function CanGetCrippled() : bool
	{
		return npcStats.canGetCrippled;
	}
	
	public function IsProtectedByArmor() : bool
	{
		return npcStats.isArmored;
	}
	
	//Kolaris - Armor Reduction Fixes
	private var armorReducedBySign : float;	default armorReducedBySign = 0;
	private var armorReducedByOil : float;	default armorReducedByOil = 0;
	private var armorReducedBy : float;	default armorReducedBy = 0;
	public function GetTotalArmorReduction(optional source : name) : float
	{
		if( source == 'Oil' )
			return AbsF(armorReducedByOil);
		else if ( source == 'Sign' )
			return AbsF(armorReducedBySign);
		else
			return AbsF(armorReducedBy);
	}
	
	public function ModifyArmorValue( by : float, optional source : name )
	{
		//theGame.GetGuiManager().ShowNotification("Armor Reduced by: " + by);
		npcStats.physicalResist += by;
		//Kolaris - Tiger Set
		if( GetWitcherPlayer().IsSetBonusActive(EISB_Tiger_1) )
			ReduceNPCArmorPen(npcStats.armorPiercing - npcStats.armorPiercing * (1.f + by));
		armorReducedBy += AbsF(by);
		if( source == 'Oil' )
			armorReducedByOil += AbsF(by);
		else if ( source == 'Sign' )
			armorReducedBySign += AbsF(by);
	}
	
	//Kolaris - Penetration
	public function Runeword51ModifyArmorValue( reduce : bool )
	{
		if( reduce )
			npcStats.physicalResist -= 0.1f;
		else
			npcStats.physicalResist += 0.1f;
	}
	
	timer function ResetRuneword51ModifyArmorValue( dt : float, id : int )
	{	
		Runeword51ModifyArmorValue(false);
	}
	
	//W3EE - Begin
	private var fireResistReducedBy : float; default fireResistReducedBy = 0;
	public function GetTotalFireResistReduction() : float
	{
		return AbsF(fireResistReducedBy);
	}

	public function ModifyFireResistance( by : float )
	{
		npcStats.fireResist += by;
		fireResistReducedBy += AbsF(by);
	}
	
	//W3EE riving blasts
	private var hasBeenHitByBomb : bool; default hasBeenHitByBomb = false;
	
	public function HasBeenHitByBomb() : bool
	{
		return hasBeenHitByBomb;
	}
	
	public function SetHasBeenHitByBomb()
	{
		hasBeenHitByBomb = true;
	}
	
	public function MultiplyTypeResistBy( damageType : name, by : float )
	{	
		switch ( damageType )
		{
			case 'FireDamage'		:
			...
			case 'ElementalDamage'	:
				npcStats.elementalResist -= by;
				SetHasBeenHitByBomb();
			return;	
			case 'PoisonDamage'		:	
				npcStats.poisonResist 	-= by;
				SetHasBeenHitByBomb();
			return;	
			case 'FrostDamage'		:
				npcStats.frostResist	-= by;
				SetHasBeenHitByBomb();
			return;	
			case 'PhysicalDamage'		:
				npcStats.physicalResist -= by;
				...
				if( GetWitcherPlayer().IsSetBonusActive(EISB_Tiger_1) )
					ReduceNPCArmorPen(npcStats.armorPiercing - npcStats.armorPiercing * (1.f - by));
			return;	
			//Kolaris - Moon Dust Bombs
			case 'SilverDamage'		:
				npcStats.bleedingResist	-= by;
				SetHasBeenHitByBomb();
			return;	
			default : return;
		}
	...
	//Kolaris - Griffin Set
	public function GryphonReduceResists( amount : float )
	{	
		if( UsesEssence() )
		{
			npcStats.forceResist = npcStats.forceResist - amount / GetStatMax(BCS_Essence) / 2;
			npcStats.frostResist = npcStats.frostResist - amount / GetStatMax(BCS_Essence) / 2;
			npcStats.fireResist = npcStats.fireResist - amount / GetStatMax(BCS_Essence) / 2;
			npcStats.shockResist = npcStats.shockResist - amount / GetStatMax(BCS_Essence) / 2;
			npcStats.slowResist = npcStats.slowResist - amount / GetStatMax(BCS_Essence) / 2;
			npcStats.confusionResist = npcStats.confusionResist - amount / GetStatMax(BCS_Essence) / 2;
			npcStats.stunResist = npcStats.stunResist - amount / GetStatMax(BCS_Essence) / 2;
		}
		else
		{
			npcStats.forceResist = npcStats.forceResist - amount / GetStatMax(BCS_Vitality) / 2;
			npcStats.frostResist = npcStats.frostResist - amount / GetStatMax(BCS_Vitality) / 2;
			npcStats.fireResist = npcStats.fireResist - amount / GetStatMax(BCS_Vitality) / 2;
			npcStats.shockResist = npcStats.shockResist - amount / GetStatMax(BCS_Vitality) / 2;
			npcStats.slowResist = npcStats.slowResist - amount / GetStatMax(BCS_Vitality) / 2;
			npcStats.confusionResist = npcStats.confusionResist - amount / GetStatMax(BCS_Vitality) / 2;
			npcStats.stunResist = npcStats.stunResist - amount / GetStatMax(BCS_Vitality) / 2;
		}
	}
	
	//Kolaris - Bear Set
	public function ReduceNPCArmorPen( amount : float )
	{
		npcStats.armorPiercing = MaxF(0.05f, npcStats.armorPiercing - amount);
	}
	
	//Kolaris - Cat Set
	public function CatReduceDamage( amount : float )
	{
		npcStats.damageValue *= amount;
	}
	
	//Kolaris - Reduce NPC Stat
	public function ReduceNPCStat( type : name, amount : float )
	{
		switch(type)
		{
			case 'poise' : npcStats.poiseValue -= amount; break;
			case 'physical' : npcStats.physicalResist -= amount; break;
			case 'force' : npcStats.forceResist -= amount; break;
			case 'frost' : npcStats.frostResist -= amount; break;
			case 'fire' : npcStats.fireResist -= amount; break;
			case 'shock' : npcStats.shockResist -= amount; break;
			case 'ethereal' : npcStats.elementalResist -= amount; break;
			case 'slow' : npcStats.slowResist -= amount; break;
			case 'mental' : npcStats.confusionResist -= amount; break;
			case 'bleed' : npcStats.bleedingResist -= amount; break;
			case 'poison' : npcStats.poisonResist -= amount; break;
			case 'stun' : npcStats.stunResist -= amount; break;
			case 'injury' : npcStats.injuryResist -= amount; break;
			default: return;
		}
	}
	
	//Kolaris - Desolation
	private var desolationChance : int;	default desolationChance = 0;
	private var desolationDamage : int;	default desolationDamage = 0;
	public function SetDesolationStacks( chance : bool )
	{
		if( chance )
			desolationChance += 1;
		else
	...
	public function GetDesolationStacks( chance : bool ) : int
	{
		if( chance )
			return desolationChance;
		else
			return desolationDamage;
	}
	
	//W3EE weather abilities
	
	var weatherAbilities : array<SWeatherBonus>;
	
	//Kolaris - Weather Ability Fix
	public function AddWeatherAbility( daypart : EDayPart, weather : EWeatherEffect, moonState : EMoonState, weatherAbilityname : name )
	{
		var params : SWeatherBonus;
		var i : int;
		
		params.dayPart = daypart;
		params.weather = weather;
		params.moonState = moonState;
		params.ability = weatherAbilityname;
		params.isActive = false;
		
		for( i=0; i<weatherAbilities.Size(); i+=1 )
		{
			if( weatherAbilities[i].ability == weatherAbilityname )
			{
				if( weatherAbilities[i].dayPart == daypart && weatherAbilities[i].weather == weather && weatherAbilities[i].moonState == moonState )
					return;
			}
		}
		
		weatherAbilities.PushBack(params);
	}
	
	public function GetWeatherAbilities() : array<SWeatherBonus>
	{
		return weatherAbilities;
	}
	
	public timer function RefreshWeatherBonus( dt : float, id : int )
	{
		weatherAbilityHandler.ModifyWeatherBonus();
	}
	//W3EE - End
	public function IsHuge() : bool
	{
		return npcStats.isHuge;
	}
	
	public function GetHealthRegenFactor() : float
	{
		return npcStats.healthRegenFactor;
	}
	
	public timer function AddHealthRegenEffect( dt : float, id : int )
	{
		AddEffectDefault(EET_HealthRegen, this, "W3EEHealthRegen");
	}
	
	public function GetPoiseValue() : float
	{
		return npcStats.poiseValue;
	}
	
	public function GetHealthBarType() : bool
	{
		if( npcStats.healthType == BCS_Essence )
			return true;
		else return false;
	}
	
	public function GetOpponentType() : EMonsterCategory
	{
		return npcStats.opponentType;
	}
	
	public function GetScaledDamage() : float
	{
		return npcStats.damageValue;
	}
	
	public function GetScaledRangedDamage() : float
	{
		return npcStats.rangedDamageValue;
	}
	
	public function GetCachedHealth() : float
	{
		return npcStats.healthMult;
	}
	
	public function GetCachedDamage() : float
	{
		return npcStats.damageMult;
	}
	
	public function SetWasScaled( b : bool )
	{
		wasScaled = b;
	}
	
	public function GetWasScaled() : bool
	{
		return wasScaled;
	}
	...
	timer function AddLevelBonuses (dt : float, id : int)
	{
		...
		*/
		...
		AddEffectDefault(EET_NPCPoise, this, "Poise");
	}
	
	public timer function InitWeatherBonus( dt : float, id : int)
	{
		weatherAbilityHandler = new W3EEAbilityHandler in this;
		weatherAbilityHandler.Init(this);
	}
	// W3EE - End
	...
	public function GainStat( stat : EBaseCharacterStats, amount : float )
	{
		...
		if(stat == BCS_Panic && IsHorse() && thePlayer.GetUsedVehicle() == this)
		{
			return;
		}
	...
	public function ForceSetStat(stat : EBaseCharacterStats, val : float)
	{
		...
		if(stat == BCS_Panic && IsHorse() && thePlayer.GetUsedVehicle() == this && val >= GetStat(BCS_Panic))
		{
			return;
		}
	...
	event OnStartFistfightMinigame()
	{
		...
		abilityManager.SetStatPointMax(BCS_Stamina, abilityManager.GetStatMax(BCS_Stamina) * 4.f);
		abilityManager.SetStatPointCurrent(BCS_Stamina, abilityManager.GetStatMax(BCS_Stamina));
		SetHealthPerc(100);
		// W3EE - End
	...
	// W3EE - Begin
	var canGlyphHit : bool;	default canGlyphHit = true;
	public function SetCanGlyphHit( b : bool )
	{
		canGlyphHit = b;
	}
	
	public function CanGlyphHit() : bool
	{
		return canGlyphHit;
	}
	
	timer function StartGlyphCooldown( dt : float, id : int )
	{
		canGlyphHit = true;
	}
	// W3EE - End
	...
	public function ReactToBeingHit(damageAction : W3DamageAction, optional buffNotApplied : bool) : bool
	{
		...
		poiseValue = GetPoiseValue() * GetHealthPercents();
		
		//Kolaris - Flyer Knockdown Fix
		...
		if( boltCauser /*|| yrdenCauser*/ )
		{
			...
			if( IsFlying() )
			{
				if( HasAbility('mon_gryphon_base') || HasAbility('mon_wyvern_base') )
		...
		{
			//Kolaris - Warding Glyph
			if( RandF() <= MinF(1, (0.25f + 0.25f * ((W3PlayerWitcher)damageAction.attacker).GetSkillLevel(S_Magic_s03)) * (1.f - poiseValue)) )
			{
				ret = super.ReactToBeingHit(damageAction, buffNotApplied);
		...
		witcher = GetWitcherPlayer();
		//Kolaris - posession
		if ( damageAction.attacker == witcher && !damageAction.IsDoTDamage() && HasBuff( EET_AxiiGuardMe ) && !witcher.HasAbility('Glyphword 30 _Stats', true) )
		{
			//Kolaris - Axii
			IncAxiiHitCounter(4);
		...
		//Kolaris - Puppet
		else if ( HasBuff( EET_AxiiGuardMe ) && !damageAction.IsDoTDamage() )
		{
			if ( UsesEssence() )
			{
				if( RandF() > (GetStatPercents(BCS_Essence) + witcher.GetSkillLevel(S_Magic_s05) * 0.1) * 10 / VecDistance(GetWorldPosition(), witcher.GetWorldPosition(), ) )
					RemoveBuff(EET_AxiiGuardMe, true);
			}
			else
			{
				if( RandF() > (GetStatPercents(BCS_Vitality) + witcher.GetSkillLevel(S_Magic_s05) * 0.1) * 10 / VecDistance(GetWorldPosition(), witcher.GetWorldPosition(), ) )
					RemoveBuff(EET_AxiiGuardMe, true);
			}
		}
		...
		if(damageAction.attacker == thePlayer && damageAction.DealsAnyDamage() && !damageAction.IsDoTDamage())
		{
			...
			//Kolaris - Cremation
			if( HasBuff(EET_Burning) && damageAction.IsActionMelee() && (witcher.HasAbility('Runeword 10 _Stats', true) || witcher.HasAbility('Runeword 11 _Stats', true) || witcher.HasAbility('Runeword 12 _Stats', true)) )
			{
				gameplayEffect = GetBuff(EET_Burning);
				if( GetBehaviorVariable('CriticalStateType') == 0 )
				{
					gameplayEffect.SetTimeLeft( gameplayEffect.GetTimeLeft() + gameplayEffect.GetInitialDurationAfterResists() );
					CreateFXEntityAndPlayEffect('mutation2_critical', 'critical_igni');
				}
				else
				{
					gameplayEffect.SetTimeLeft( gameplayEffect.GetTimeLeft() + 0.5f );
					CreateFXEntityAndPlayEffect('mutation1_hit', 'mutation_1_hit_igni');
				}
			}
	...
	function Kill( source : name, optional ignoreImmortalityMode : bool, optional attacker : CGameplayEntity, optional playHitSound : bool )
	{
		...
		action = GetKillAction( source, ignoreImmortalityMode, attacker, playHitSound );
...
/*		if ( !Options().Scale() )
		{
			...
			return Experience().DisplayScaledLevel(this, displayLevel, strLevel);*/
		//W3EE - End
	...
	// W3EE - Begin
	timer function StopPossessionEffect( dt : float, id : int )
	{
		StopEffect('demonic_possession');
	}
	
	private var isInsideYrden : bool;
	public function IsInsideYrden() : bool
	{
		return isInsideYrden;
	}
	
	public function SetInsideYrden( b : bool )
	{
		isInsideYrden = b;
	}
	// W3EE - End
	...
	{		
		...
		var itemExpBonus, radius, runeword54bonus			: float;
		...
		ciriEntity = (W3ReplacerCiri)thePlayer;
		witcher = (W3PlayerWitcher)damageAction.attacker;
		
		deathTimestamp = theGame.GetEngineTimeAsSeconds();
		
		//Kolaris - Mutation 7
		if( witcher.IsMutationActive(EPMT_Mutation7) )
		{
			if( UsesEssence() )
				Combat().ManageMutation7(damageAction.attacker, damageAction.victim, GetStatMax(BCS_Essence));
			else
				Combat().ManageMutation7(damageAction.attacker, damageAction.victim, GetStatMax(BCS_Vitality));
		}
		
		//Kolaris - Dol Blathanna Set
		if( witcher.IsSetBonusActive( EISB_Elven_2 ) && !witcher.IsInFistFight() && !WillBeUnconscious() )
		{
			if( UsesEssence() && (GetStatMax(BCS_Essence) / Options().GetDifficultySettingMod()) > 1000.f )
				GetInventory().AddAnItem(Equipment().GetRandomHerbName(), FloorF(GetStatMax(BCS_Essence) / Options().GetDifficultySettingMod() / 1000.f));
			else if( (GetStatMax(BCS_Vitality) / Options().GetDifficultySettingMod()) > 1000.f )
				GetInventory().AddAnItem(Equipment().GetRandomHerbName(), FloorF(GetStatMax(BCS_Vitality) / Options().GetDifficultySettingMod() / 1000.f));
		}
		
		//Kolaris - Vampire Set
		if( witcher.IsSetBonusActive( EISB_Vampire_2 ) && damageAction.IsActionMelee() && !witcher.IsInFistFight() && !WillBeUnconscious() )
		{
			witcher.VampiricSetAbilityRegeneration(((W3Effect_Bleeding)GetBuff(EET_Bleeding)).GetStacks());
		}
		
		//Kolaris - Disintegration
		if( damageAction.GetBuffSourceName() == "Magic_s12" || damageAction.GetBuffSourceName() == "Glyphword 3" )
		{
		...
		// W3EE - Begin
		//Kolaris - Absorption
		if( (witcher.HasAbility('Glyphword 13 _Stats', true) || witcher.HasAbility('Glyphword 14 _Stats', true) || witcher.HasAbility('Glyphword 15 _Stats', true)) && isInsideYrden )
		{
			witcher.PlayEffect('ability_gryphon_start');
			if( UsesEssence() )
			{
				witcher.GainStat(BCS_Focus, witcher.GetStatMax(BCS_Focus) * ClampF(GetStatMax(BCS_Essence) * 0.00002f, 0.f, 1.f ));
				witcher.GainStat(BCS_Vitality, witcher.GetStatMax(BCS_Vitality) * ClampF(GetStatMax(BCS_Essence) * 0.00002f, 0.f, 1.f ));
			}
			else
			{
				witcher.GainStat(BCS_Focus, witcher.GetStatMax(BCS_Focus) * ClampF(GetStatMax(BCS_Vitality) * 0.00002f, 0.f, 1.f ));
				witcher.GainStat(BCS_Vitality, witcher.GetStatMax(BCS_Vitality) * ClampF(GetStatMax(BCS_Vitality) * 0.00002f, 0.f, 1.f ));
			}
		}
		
		//Kolaris - Perfection
		if( witcher.GetStatPercents(BCS_Vitality) >= 0.999f && witcher.HasBuff(EET_GlyphwordPerfection) )
			((W3Effect_GlyphwordPerfection)witcher.GetBuff(EET_GlyphwordPerfection)).IncrementPerfectionAbility(CeilF((GetStatMax(BCS_Vitality) + GetStatMax(BCS_Essence)) / 5000));
		
		//Kolaris - Purgation
		if( (HasBuff(EET_Burning) || (W3IgniProjectile)damageAction.causer) && witcher.HasAbility('Glyphword 12 _Stats', true) )
		{
			witcher.GainStat(BCS_Focus, 2.f);
			witcher.PlayEffectSingle('runeword_20_adrenaline');
		}
		
		//Kolaris - Necrophage Decoction
		((W3Decoction2_Effect)witcher.GetBuff(EET_Decoction2)).HealPlayer(damageAction, witcher);
		
		isInsideYrden = false;
		if( witcher && damageAction.IsActionMelee() )
		{
			witcher.GetAdrenalineEffect().AdrenalineGrantKill();
			((W3Effect_SwordKillBuff)witcher.GetBuff(EET_SwordKillBuff)).SetKillBuffActive(true);
			//((W3Decoction2_Effect)witcher.GetBuff(EET_Decoction2)).HealPlayer(damageAction, witcher);
			//Kolaris - Assassination
			if( witcher.HasAbility('Runeword 36 _Stats', true) )
			{
				witcher.AddAbility('Runeword 36 Ability');
				witcher.PlayEffect('runeword_20_adrenaline');
				witcher.AddTimer('ManageAssassinationVisuals', 0.5f,,,,,true);
			}
			//Kolaris - Fortification
			if( witcher.HasAbility('Runeword 54 _Stats', true) )
			{
				weaponID = ((W3Action_Attack)damageAction).GetWeaponId();
				runeword54bonus = witcher.GetInventory().GetItemModifierFloat( weaponID, 'PermDamageBoost' ) + (GetStatMax(BCS_Vitality) + GetStatMax(BCS_Essence)) / 100000;
				witcher.GetInventory().SetItemModifierFloat( weaponID, 'PermDamageBoost', runeword54bonus );
				//theGame.GetGuiManager().ShowNotification("New Bonus: " + runeword54bonus);
			}
		}
		//Kolaris - Posession
		/*if ( GetWitcherPlayer().HasAbility('Glyphword 13 _Stats', true) && (HasBuff(EET_AxiiGuardMe) || HasBuff(EET_Confusion)) )
		{
			abilityName = 'Glyphword 13 _Stats';
			min = GetWitcherPlayer().GetAbilityAttributeValue(abilityName, 'glyphword_range');
			FindGameplayEntitiesInRange(entities, this, min.valueAdditive, 10,, FLAG_OnlyAliveActors + FLAG_ExcludeTarget, this); 	
			// W3EE - End
		...
		//W3EE - Begin
		attackAction = (W3Action_Attack)damageAction;
		Combat().SeveranceRunewordStaminaAbsorb(attackAction);
		Combat().ObliterationRunewordExplosion(attackAction);
		//Combat().ObliterationRunewordGroundFX(attackAction);
		Combat().EruptionGlyphword(damageAction);
		
		if( witcher && damageAction.GetBuffSourceName() != "WinterBladeDamage" && witcher.HasBuff(EET_WinterBlade) )
		{
			witcher.PlayEffect('mutation_6_power');
			surface = theGame.GetSurfacePostFX();
			surface.AddSurfacePostFXGroup(GetWorldPosition(), 2, 15, 6, 3.5f, 0);
		}
		...
		{
			if( UsesEssence() )
				witcher.enemiesKilled += RoundMath(PowF(GetStatMax(BCS_Essence) / 1000.f, 1.5f));
			else
				witcher.enemiesKilled += RoundMath(PowF(GetStatMax(BCS_Vitality) / 1000.f, 1.5f));
		}
		//W3EE - End
		...
		/*if(damageAction.attacker == thePlayer && damageAction.IsActionMelee() && thePlayer.GetStat(BCS_Toxicity) > 0 && thePlayer.CanUseSkill(S_Alchemy_s17))
		{
			//thePlayer.AddAbilityMultiple( SkillEnumToName(S_Alchemy_s17), thePlayer.GetSkillLevel(S_Alchemy_s17) );
			witcher.GainStat(BCS_Stamina, witcher.GetStatMax(BCS_Stamina) * 0.05f * thePlayer.GetSkillLevel(S_Alchemy_s17) * thePlayer.GetStatPercents(BCS_Toxicity));
			witcher.GainStat(BCS_Focus, witcher.GetStatMax(BCS_Focus) * 0.05f * thePlayer.GetSkillLevel(S_Alchemy_s17) * thePlayer.GetStatPercents(BCS_Toxicity));
		}*/
		...
		*/
		...
		if ( damageAction.attacker == thePlayer )
		{
			...
			//plasticmetal - mutagenerator fix
			if( CalculateExperiencePoints() > 0 && !HasTag( 'AchievementKillDontCount' ) && thePlayer.inv.HasItem( 'q705_tissue_extractor' ) )
			{
	...
	event OnAardHit( sign : W3AardProjectile )
	{
		...
		{
			//Kolaris - Aard Stamina Drain
			DrainStamina(ESAT_FixedValue, staminaDrainPerc * GetStatMax(BCS_Stamina) * (1.f - GetNPCCustomStat(theGame.params.DAMAGE_NAME_FORCE)));
		}
	...
	{
		// W3EE - Begin
		var hasShield : CEntity;
		
		hasShield = GetInventory().GetItemEntityUnsafe( GetInventory().GetItemFromSlot( 'l_weapon' ) );
		
		if( HasTag( 'imlerith' ) || HasAbility( 'WildHunt_Imlerith') || !hasShield )
			return;
		...
		ToggleEffectOnShield( 'heavy_block', true );
		DropItemFromSlot( 'l_weapon', true );
		SignalGameplayEventParamInt('ChangePreferedCombatStyle', (int)EBG_Combat_1Handed_Any);
		// W3EE - End
	}
	
	// W3EE - Begin
	public function ProcessWeaponDisarm()
	{
		if( HasTag('IsBoss') )
			return;
	...
	// W3EE - Begin
	private var isRegenActive : bool;	default isRegenActive = true;
	public function GetIsRegenActive() : bool
	{
		return isRegenActive;
	}
	
	timer function SetRegenActive( dt : float, id : int )
	{
		isRegenActive = true;
	}
	
	public function StartRegenTimer(optional extraDelay : float)
	{
		isRegenActive = false;
		AddTimer('SetRegenActive', npcStats.regenDelay + extraDelay, false,,,,true);
	}
	// W3EE - End
	
	//Kolaris ++ Mutation Rework
	private var Mutation7Amount : float;
	private var Mutation7Timestamp : float;
	public function SetMutation7Amount(amount : float)
	{
		Mutation7Amount = amount;
	}
	
	public function GetMutation7Amount() : float
	{
		return Mutation7Amount;
	}
	
	public function SetMutation7Timestamp()
	{
		Mutation7Timestamp = theGame.GetEngineTimeAsSeconds();
	}
	
	public function GetMutation7Timestamp() : float
	{
		return Mutation7Timestamp;
	}
	//Kolaris -- Mutation Rework
	...
	event OnTakeDamage( action : W3DamageAction )
	{
		...
		// W3EE - Begin
		if( npcStats.healthRegenFactor > 0 && npcStats.regenDelay > 0 && !action.IsDoTDamage() && action.DealsAnyDamage() && isRegenActive == true )
			StartRegenTimer();
		// W3EE - End
		...
		if( ((W3PlayerWitcher)action.attacker).IsMutationActive(EPMT_Mutation7) && !action.IsDoTDamage() )
			Combat().ManageMutation7(action.attacker, action.victim, action.GetDamageDealt());
		//Kolaris -- Mutation Rework
		//Kolaris - Absorption
		if( ( IsInsideYrden() || (W3Effect_YrdenHealthDrain)action.causer || (W3YrdenEntityStateYrdenShock)action.causer ) && GetWitcherPlayer().HasAbility('Glyphword 15 _Stats', true) )
		{
			GetWitcherPlayer().GainStat(BCS_Focus, GetWitcherPlayer().GetStatMax(BCS_Focus) * action.GetDamageDealt() / 30000.f );
			GetWitcherPlayer().GainStat(BCS_Vitality, GetWitcherPlayer().GetStatMax(BCS_Vitality) * action.GetDamageDealt() / 100000.f );
			if( action.GetDamageDealt() >= 2000.f )
			{
				GetWitcherPlayer().PlayEffect('runeword_8');
				GetWitcherPlayer().StopEffect('runeword_8');
			}
		}
		//Kolaris - Enervation
		if( (W3YrdenEntityStateYrdenShock)action.causer && (GetWitcherPlayer().HasAbility('Glyphword 16 _Stats', true) || GetWitcherPlayer().HasAbility('Glyphword 17 _Stats', true) || GetWitcherPlayer().HasAbility('Glyphword 18 _Stats', true)) )
		{
			DrainStamina(ESAT_FixedValue, GetStatMax(BCS_Stamina) * 0.25f, 1);
		}
		//Kolaris - Invocation
		if( (W3PlayerWitcher)action.attacker && action.IsActionMelee() && ((W3PlayerWitcher)action.attacker).IsHeavyAttack(((W3Action_Attack)action).GetAttackName()) && (GetWitcherPlayer().HasAbility('Runeword 41 _Stats', true) || GetWitcherPlayer().HasAbility('Runeword 42 _Stats', true)) )
		{
			SetCanGlyphHit(true);			
			//Plasticmetal - yrdenfix++
			yrdens = GetWitcherPlayer().yrdenEntities;
