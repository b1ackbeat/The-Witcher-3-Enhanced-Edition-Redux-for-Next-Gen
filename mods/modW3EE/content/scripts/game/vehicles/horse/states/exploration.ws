...
state Exploration in W3HorseComponent
{
	...
	//Kolaris - Gallop Toggle
	private var gallopToggle : bool;
	...
	// W3EE - Begin
	public function GetHorseSpeed() : float
	{
		return currSpeed;
	}
	// W3EE - End
	...
	event OnEnterState( prevStateName : name )
	{
		...
		//Kolaris - Gallop Toggle
		theInput.RegisterListener( this, 'OnGallopToggle', 'GallopToggle' );
	...
	private function UnregisterInput()
	{
		...
		//Kolaris - Gallop Toggle
		theInput.UnregisterListener( this, 'GallopToggle' );
	...
	{	// W3EE - Begin
		...
		if( parentActor.HasAbility( 'DisableHorsePanic' ) )
			return false;
		
		actorAttacker = (CNewNPC)action.attacker;
		parentActor.AddPanic( actorAttacker.GetThreatLevel() );
		// W3EE - End
	}
	...
	private function ShouldApplyCorrection( stickInputX : float, stickInputY : float ) : bool
	{
		...
		{
			//Kolaris - Gallop Toggle
			if( theInput.IsActionPressed( 'Canter' ) || gallopToggle )
				return true;
			else
				return false;
		}	
	...
	private final function UpdateLogic( dt : float )
	{
		...
		{
			//Kolaris - Horseman
			if( parent.user == thePlayer && thePlayer.CanUseSkill(S_Perk_02) )
				actorParent.DrainStamina( ESAT_FixedValue, 3.33f*dt, 1.f, '', 0.f, 0.8f );
			else
		...
		//Kolaris - Gallop Toggle
		if( currSpeed < GALLOP_SPEED && gallopToggle )
			gallopToggle = false;
	...
	//Kolaris - Gallop Toggle
	event OnGallopToggle( action : SInputAction )
	{
		if( IsPressed(action) )
		{
			if( gallopToggle || GALLOP_SPEED > currSpeed )
			{
				gallopToggle = false;
				if( currSpeed >= GALLOP_SPEED )
					ToggleSpeedLock( 'OnGallop', false );
