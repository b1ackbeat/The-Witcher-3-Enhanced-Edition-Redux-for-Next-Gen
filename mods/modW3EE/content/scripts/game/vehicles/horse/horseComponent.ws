...
statemachine import class W3HorseComponent extends CVehicleComponent
{
	...
	{
		return ((W3HorseComponentStateExploration)GetState('Exploration')).GetHorseSpeed();
	}
	// W3EE - End
	...
	event OnDismountFinished( entity : CEntity, vehicleSlot : EVehicleSlot )
	{
		...
		// W3EE - Begin
		((W3PlayerWitcher)riderActor).UpdateWoundedState();
		// W3EE - End
	}
	...
	private function UpdatePanic( dt : float )
	{
		...
		if( ShouldUpdatePanic() )
		{
			...
			{	
				...
				{
					totalThreat += tempThreat * (1.f - 0.4f * tempThreat / 100);
				}
			}
		}
		else
		...
		//Kolaris - Horseman
		if( thePlayer.GetUsedHorseComponent() == this && thePlayer.CanUseSkill(S_Perk_02) )
			staticPanic = RoundF(totalThreat /** THREAT_MULT*/ * panicMult * 0.5f);
		else
