...
class W3HorseManager extends CPeristentEntity
{
	...
	public function EquipItem(id : SItemUniqueId) : SItemUniqueId
	{
		...
		if ( slot == EES_HorseSaddle )
		{
			abls.Clear();
			inv.GetItemAbilities(id, abls);
			for (i=0; i < abls.Size(); i += 1)
			{
				if ( abls[i] == 'HorseAttackPower1' )
					thePlayer.AddAbility(abls[i]);
				else
				if ( abls[i] == 'HorseAttackPower2' )
					thePlayer.AddAbility(abls[i]);
				else
				if ( abls[i] == 'HorseAttackPower3' )
					thePlayer.AddAbility(abls[i]);
				else
				if ( abls[i] == 'HorseAttackPower4' )
					thePlayer.AddAbility(abls[i]);
				else
				if ( abls[i] == 'HorseAttackPower5' )
					thePlayer.AddAbility(abls[i]);
			}
		}
		// W3EE - End	
	...
	// W3EE - Begin
	private var shouldHorseRest : bool;	default shouldHorseRest = false;
	
	public function ShouldRest() : bool
	{
		if( Equipment().GetHorseDistance() > 81 )
			return true;
		else
		if( !shouldHorseRest )
			return false;
		else
		if( Equipment().GetHorseDistance() <= 81 && shouldHorseRest )
		{
			shouldHorseRest = false;
			AddTimer('SetHorseRest', 60.f, false);
			return false;
		}
		else
			return true;
	...
	private timer function SetHorseRest( dt : float, id : int )
	{
		shouldHorseRest = true;
	}
	// W3EE - End
}