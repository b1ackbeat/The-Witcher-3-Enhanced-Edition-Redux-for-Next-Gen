...
import statemachine class CStoryScenePlayer extends CEntity
{
	...
	event OnBlockingSceneStarted( scene: CStoryScene )
	{
		...
		((W3Effect_ToxicityFever)GetWitcherPlayer().GetBuff(EET_ToxicityFever)).CureFever();
		((W3Potion_Blizzard)GetWitcherPlayer().GetBuff(EET_Blizzard)).RemoveSlowMo();
		gameTime = GameTimeToSeconds(theGame.GetGameTime());
	...
	event OnBlockingSceneEnded( output : CStorySceneOutput )
	{
		...
		if( (GameTimeToSeconds(theGame.GetGameTime()) - gameTime) / 3600.f > 1 )
			((W3Effect_Toxicity)GetWitcherPlayer().GetBuff(EET_Toxicity)).ClearToxicityHoney(1, 1);
	...
	{	
		...
		GetWitcherPlayer().CustomVigorTimerEnd(0, 0);
		GetWitcherPlayer().ResumeVigorRegen(0, 0);
	}
...
state Cutscene in CStoryScenePlayer
{
	...
	event OnCutsceneEnded()
	{
		...
		GetWitcherPlayer().CustomVigorTimerEnd(0, 0);
		GetWitcherPlayer().ResumeVigorRegen(0, 0);
		if( ((W3Effect_ToxicityFever)GetWitcherPlayer().GetBuff(EET_ToxicityFever)).IsFeverActive() )
			EnableDrunkFx(1.f);
	}
}
