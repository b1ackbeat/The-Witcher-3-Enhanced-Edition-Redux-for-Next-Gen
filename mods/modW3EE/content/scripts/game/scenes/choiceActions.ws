...
class CPayStorySceneChoiceAction extends CStorySceneChoiceLineActionScripted
{
	...
	// W3EE - Begin
	private var moneyLFE : int;
	
	function CanUseAction() : bool 
	{
		if ( money >= 500)
		{
			moneyLFE = RoundF(money * LFEGetDialogMult());
		}
		else
		{
			moneyLFE = money;
		...
		if( money == 130 && dontGrantExp && actionIcon == DialogAction_NONE )
			moneyLFE = 325;
			
		return thePlayer.GetMoney() >= moneyLFE; 
	}
	
	function PerformAction()
	{
		thePlayer.RemoveMoney( moneyLFE );
