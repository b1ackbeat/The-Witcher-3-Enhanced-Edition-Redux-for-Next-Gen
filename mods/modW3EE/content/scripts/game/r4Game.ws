...
import class CR4Game extends CCommonGame
{
	...
	public var m_quickSlotsConfig : WmkQuickSlotsConfig; // -= WMK:modQuickSlots =-
	...
	//W3EE - Begin
	public var Experience : W3EEExperienceHandler;
	public var Combat : W3EECombatHandler;
	public var Scaling : W3EEScalingHandler;
	public var Options : W3EEOptionHandler;
	public var Enemies : W3EEEnemyHandler;
	public var Blood : W3EEBloodEffectsHandler;
	public var Damage : W3EEDamageHandler;
	public var Equipment : W3EEEquipmentHandler;
	public var Alchemy : W3EEAlchemyExtender;
	public var CampfireManager : CWitcherCampfireManager;
	//W3EE - End
	
	//Kolaris - Gaunter Mode
	public var GaunterMode : GaunterModeManager;
	
	//Mutagen Sorter
	public var MutagenSorter : CMutagenSorter;
	...
	private function SetTimescaleSources()
	{
		...
		timescaleSources[ ETS_SkillFrenzy ].sourcePriority = 27;
		...
		timescaleSources[ ETS_TutorialFight ].sourcePriority = 60;
		...
		timescaleSources[ ETS_Yrden ].sourceType = ETS_Yrden;
		timescaleSources[ ETS_Yrden ].sourceName = 'enchanted_glyphs';
		timescaleSources[ ETS_Yrden ].sourcePriority = 26;
		
		timescaleSources[ ETS_CockatriceMutagen ].sourceType = ETS_CockatriceMutagen;
		timescaleSources[ ETS_CockatriceMutagen ].sourceName = 'cockatrice_mutagen';
		timescaleSources[ ETS_CockatriceMutagen ].sourcePriority = 20;
		
		timescaleSources[ ETS_ReflexBlast ].sourceType = ETS_ReflexBlast;
		timescaleSources[ ETS_ReflexBlast ].sourceName = 'reflex_blast';
		timescaleSources[ ETS_ReflexBlast ].sourcePriority = 30;
		
		timescaleSources[ ETS_Adrenaline ].sourceType = ETS_Adrenaline;
		timescaleSources[ ETS_Adrenaline ].sourceName = 'adrenaline';
		timescaleSources[ ETS_Adrenaline ].sourcePriority = 25;
		
		timescaleSources[ ETS_Meditation ].sourceType = ETS_Meditation;
		timescaleSources[ ETS_Meditation ].sourceName = 'meditation';
		timescaleSources[ ETS_Meditation ].sourcePriority = 50;
		// W3EE - End
	}
	...
	event OnGameStarting(restored : bool )
	{
		...
		// W3EE - Begin
		Experience = new W3EEExperienceHandler in this;
		Combat = new W3EECombatHandler in this;
		Scaling = new W3EEScalingHandler in this;
		Enemies = new W3EEEnemyHandler in this;
		Blood = new W3EEBloodEffectsHandler in this;
		Damage = new W3EEDamageHandler in this;
		Equipment = new W3EEEquipmentHandler in this;
		Alchemy = new W3EEAlchemyExtender in this;
		CampfireManager = new CWitcherCampfireManager in this;
		Blood.Init();
		// W3EE - End
		
		//Kolaris - GaunterMode
		GaunterMode = new GaunterModeManager in this;
		MutagenSorter = new CMutagenSorter in this;
	}
	...
	event OnGameStarted(restored : bool)
	{
		...
		// W3EE - Begin
		CampfireManager.RestoreFirecamps();
		// W3EE - End
	}
	...
	event OnGiveReward( target : CEntity, rewardName : name, rewrd : SReward )
	{
		...
		{
			// W3EE - Begin
			Experience().AwardGeneralXP(true);
			// W3EE - End
			
			//Kolaris - Gaunter Mode
			GaunterMode().ProcessQuestRewards(rewardName);
			...
			if ( inv )
			{
				...
				for ( i = 0; i < rewrd.items.Size(); i += 1 )
				{
					...
					if( itemsCount > 0 )
					{
						...
						{
							//inv.ReadSchematicsAndRecipes( ids[0] ); Kolaris - Reward Schematics Read Twice
						}						
	...
	public function PopulateMenuQueueMainAlways( out menus : array< name > )
	{
		...
		// W3EE - Begin
		if( !Options )
			Options = new W3EEOptionHandler in this;
	...
	public function LoadHudSettings()
	{
		...
		// -= WMK:modQuickSlots =-
		if (!m_quickSlotsConfig)
		{
			m_quickSlotsConfig = new WmkQuickSlotsConfig in this;
			m_quickSlotsConfig.SyncConfigVars();
			SaveUserSettings();
		}
		// -= WMK:modQuickSlots =-
	}
...
// W3EE - Begin
function Experience() : W3EEExperienceHandler
{
	return theGame.Experience;
}

function Combat() : W3EECombatHandler
{
	return theGame.Combat;
}

function Scaling() : W3EEScalingHandler
{
	return theGame.Scaling;
}

function Options() : W3EEOptionHandler
{
	return theGame.Options;
}

function Enemies() : W3EEEnemyHandler
{
	return theGame.Enemies;
}

function Blood() : W3EEBloodEffectsHandler
{
	return theGame.Blood;
}

function Damage() : W3EEDamageHandler
{
	return theGame.Damage;
}

function Equipment() : W3EEEquipmentHandler
{
	return theGame.Equipment;
}

function Alchemy() : W3EEAlchemyExtender
{
	return theGame.Alchemy;
}

function CampfireManager() : CWitcherCampfireManager
{
	return theGame.CampfireManager;
}
// W3EE - End

//Kolaris - Gaunter Mode
function GaunterMode() : GaunterModeManager
{
	return theGame.GaunterMode;
}

//Mutagen Sorter
function MutSort() : CMutagenSorter
{
	return theGame.MutagenSorter;
}