...
import class CDefinitionsManagerAccessor extends CObject
{
	...
	public function GetItemAttributeValue(itemName : name, playerItem : bool, attributeName : name) : SAbilityAttributeValue
	{
		var abs : array<name>;
		var temp : array<float>;
		var tempInt : int;
	...
	// W3EE - Begin
	public function IsItemFood(item : name) : bool					{return ItemHasTag(item, 'Edibles') || ItemHasTag(item, 'Drinks');}
	public function IsItemCraftingIngredient(item : name) : bool	{return ItemHasTag(item, 'CraftingIngredient');}
	public function IsItemPotion(item : name) : bool							{return ItemHasTag(item, 'Potion') && !IsItemFood(item);}
	public function IsItemJewelry(item : name) : bool						{return ItemHasTag(item, 'jewelry');}
	public function IsItemAlchemicalAlcohol( item : name ) : bool			{return ItemHasTag(item, 'StrongAlcohol');}
	public function IsItemHerb( item : name ) : bool							{return ItemHasTag(item, 'primer_herb');}
	public function IsItemMonsterPart( item : name ) : bool					{return ItemHasTag(item, 'primer_monstrous');}
	public function IsItemMineral( item : name ) : bool						{return ItemHasTag(item, 'primer_mineral');}
	public function IsItemMoney( item : name ) : bool						{return ItemHasTag(item, 'primer_mineral');}
	public function IsItemReadable(item : name) : bool						{return ItemHasTag(item, 'ReadableItem');}
	public function IsItemTrophy(item : name) : bool							{return GetItemCategory(item) == 'trophy';}
	public function IsItemJunk( item : name ) : bool							{return ItemHasTag( item,'junk' ) || GetItemCategory( item ) == 'junk';}
	public function IsItemHorseItem(item : name) : bool						{return ItemHasTag(item, 'Saddle') || ItemHasTag(item, 'HorseBag') || ItemHasTag(item, 'Trophy') || ItemHasTag(item, 'Blinders'); }
	public function IsItemUpgrade(item : name) : bool						{return GetItemCategory(item) ==  'upgrade';}
	// W3EE - End
	...
	public final function IsRecipeForMutagenPotion(recipeName : name) : bool
	{
		...
		for(i=0; i<main.subNodes.Size(); i+=1)
		{
			if(GetCustomNodeAttributeValueName(main.subNodes[i], 'name_name', checkedRecipeName) && checkedRecipeName == recipeName)
			{
				...
				{
					if(ItemHasTag(cookedItemName, 'Decoction') && GetItemCategory(cookedItemName) == 'potion')
						return true;
				}
				return false;
			}
	...
	public function GetItemQuality( itemName : name ) : int
	{
		var min, max : SAbilityAttributeValue;
	...
	public final function IsItemSetItem( itemName : name ) : bool
	{
		return
			...
			ItemHasTag(itemName, theGame.params.ITEM_SET_TAG_VAMPIRE_ALT) ||
			ItemHasTag(itemName, theGame.params.ITEM_SET_TAG_TEMERIAN) ||
			ItemHasTag(itemName, theGame.params.ITEM_SET_TAG_NILFGAARD) ||
			ItemHasTag(itemName, theGame.params.ITEM_SET_TAG_SKELLIGE) ||
			ItemHasTag(itemName, theGame.params.ITEM_SET_TAG_OFIERI) ||
			ItemHasTag(itemName, theGame.params.ITEM_SET_TAG_NEW_MOON) ||
			ItemHasTag(itemName, theGame.params.ITEM_SET_TAG_VIPER) ||
			ItemHasTag(itemName, theGame.params.ITEM_SET_TAG_GOTHIC) ||
			ItemHasTag(itemName, theGame.params.ITEM_SET_TAG_DIMERITIUM) ||
			ItemHasTag(itemName, theGame.params.ITEM_SET_TAG_METEORITE) ||
			ItemHasTag(itemName, theGame.params.ITEM_SET_TAG_NETFLIX) ||
			ItemHasTag(itemName, theGame.params.ITEM_SET_TAG_ELVEN) ||
			ItemHasTag(itemName, theGame.params.ITEM_SET_TAG_TIGER);
	}
