...
import abstract class CActor extends CGameplayEntity
{
	...
	public function IsTargetableByPlayer() : bool
	{
		...
		{
			//Kolaris - Hard Target Lock Fix
			if ( this.GetAttitude( thePlayer ) != AIA_Hostile && !HasBuff(EET_AxiiGuardMe) && GetHealthPercents() > 0 )
				return false;
		}
	...
	{
		var weaponDamage, innateDamage : SAbilityAttributeValue;
		var ret : float;
	...
	//W3EE - Begin
	public function IsWeaponDamaged() : bool
	{
		var heldWeapons				: array<SItemUniqueId>;
		var i						: int;
		var IsWeaponDamaged			: bool;
		...
		{
			IsWeaponDamaged = inv.ItemHasTag( heldWeapons[i], 'DamagedWeapon' );
			
			if ( IsWeaponDamaged )
				break;
		}			
		
		return IsWeaponDamaged;
	}
	
	public function SetAnimationSpeedMultiplier( mul : float, optional overrideExistingId : int, optional forceValue : bool ) : int
	{
		...
		if( overrideExistingId != -1 )
		{
			for( i = 0; i < size; i += 1 )
			{
				if( animationMultiplierCausers[i].id == overrideExistingId )
				{
					...
					SetAnimationTimeMultiplier( CalculateFinalAnimationSpeedMultiplier(forceValue) );
					return animationMultiplierCausers[i].id;
				}
			}
		}
		
		nextFreeAnimMultCauserId = RandRange(1000, 1);
		for( i = 0; i < size; i += 1 )
		{
			if( animationMultiplierCausers[i].id == nextFreeAnimMultCauserId )
			{
				nextFreeAnimMultCauserId = RandRange(1000, 1);
				i = 0;
			}
	...
	private function CalculateFinalAnimationSpeedMultiplier( optional forceValue : bool ) : float
	{
		var i,size : int;
	...
	// W3EE - Begin
	private var injurySystem : W3EEInjurySystem;
	public function GetInjuryManager() : W3EEInjurySystem
	{
		return injurySystem;
	}
	
	public function InitInjuryManager()
	{
		injurySystem = new W3EEInjurySystem in this;
		injurySystem.Init(this);
	}
	
	timer function StopHeadHitEffect( dt : float, id : int )
	{
		StopEffect('stunned_ghost');
	}
	
	public timer function PlayActorHitAnimation( dt : float, id : int )
	{
		var damageAction : W3DamageAction;
		var hitReaction : EHitReactionType;
		
		if( ((CNewNPC)this).IsFlying() || HasBuff(EET_Stagger) || HasBuff(EET_LongStagger) || HasBuff(EET_Knockdown) || HasBuff(EET_HeavyKnockdown) || HasBuff(EET_Immobilized) || HasBuff(EET_CounterStrikeHit) )
			return;
		
		damageAction = new W3DamageAction in theGame.damageMgr;
		damageAction.Initialize(GetWitcherPlayer(), this, GetWitcherPlayer(), "ParriedCounter", EHRT_Light, CPS_Undefined, true, false, false, false);
	...
	private var stumbleCooldown : bool;
	default stumbleCooldown = false;
	timer function ResetStumbleCooldown( dt : float, id : int )
	{
		stumbleCooldown = false;
	}
	
	private function SetStumbleCooldown()
	{
		stumbleCooldown = true;
		AddTimer('ResetStumbleCooldown', 5.f, false);
	}
	
	public timer function RollAttackStumble( dt : float, id : int )
	{
		var damageAction : W3DamageAction;
		var hitReaction : EHitReactionType;
		...
		if( RandRange(100,0) <= 35 )
		{
			damageAction = new W3DamageAction in theGame.damageMgr;
			if( RandRange(100,0) <= 35 )
				hitReaction = EHRT_Heavy;
			else
				hitReaction = EHRT_Light;
			damageAction.Initialize(NULL, this, NULL, "CombatInjury", hitReaction, CPS_Undefined, false, false, false, true);
			damageAction.SetHitAnimationPlayType(EAHA_ForceYes);
			damageAction.SetCannotReturnDamage(true);
			damageAction.SetSuppressHitSounds(true);
			damageAction.SetCanPlayHitParticle(false);
	...
	public timer function RollRunningAttackStumble( dt : float, id : int )
	{
		var damageAction : W3DamageAction;
		var hitReaction : EHitReactionType;
		var chance : int;
		
		if( stumbleCooldown || ((CNewNPC)this).IsFlying() || HasBuff(EET_Stagger) || HasBuff(EET_LongStagger) || HasBuff(EET_Knockdown) || HasBuff(EET_HeavyKnockdown) || HasBuff(EET_Immobilized) || HasBuff(EET_CounterStrikeHit) )
			return;
		...
		if( RandRange(100,0) <= chance )
		{
			...
			if( RandRange(100,0) <= 50 )
				...
				hitReaction = EHRT_Light;
			damageAction.Initialize(NULL, this, NULL, "CombatInjury", hitReaction, CPS_Undefined, false, false, false, true);
			SetOverrideDirection(true);
			damageAction.SetHitAnimationPlayType(EAHA_ForceYes);
			damageAction.SetCannotReturnDamage(true);
			damageAction.SetSuppressHitSounds(true);
			damageAction.SetCanPlayHitParticle(false);
			theGame.damageMgr.ProcessAction(damageAction);
			SetOverrideDirection(false);
			delete damageAction;
			SetStumbleCooldown();
		}
	}
	
	public timer function RollDamagedBlindness( dt : float, id : int )
	{
		var blindnessEffect : SCustomEffectParams;
		var hitReaction : EHitReactionType;
		var chance : int;
		
		if( stumbleCooldown || ((CNewNPC)this).IsFlying() || HasBuff(EET_Blindness) || HasBuff(EET_Confusion) || HasBuff(EET_CounterStrikeHit) )
			return;
		...
		if( RandRange(100,0) <= chance )
		{
			if( (W3PlayerWitcher)this )
				blindnessEffect.effectType = EET_Blindness;
			else
				blindnessEffect.effectType = EET_Confusion;
			blindnessEffect.sourceName = 'CombatInjury';
			blindnessEffect.duration = 5.f;
			AddEffectCustom(blindnessEffect);
			SetStumbleCooldown();
		}
	}
	
	public timer function RollParryStumble( dt : float, id : int )
	{
		var damageAction : W3DamageAction;
		var hitReaction : EHitReactionType;
		var chance : int;
	...
	public timer function RollDodgeStumble( dt : float, id : int )
	{
		var damageAction : W3DamageAction;
		var hitReaction : EHitReactionType;
		var chance : int;
	...
	{	
		...
		// W3EE - Begom
		InitInjuryManager();
		AddTimer('SetIsQuest', 1.5f, false);
		// W3EE - End
	}
	...
	protected function GetKillAction( source : name, optional ignoreImmortalityMode : bool, optional attacker : CGameplayEntity, optional playHitSound : bool ) : W3DamageAction
	{
		var vit, ess : float;
		...
		action.SetSuppressHitSounds(!playHitSound);
		return action;
	}
	...
	{
		// W3EE - Begin
		var l_delayToWait   : float = 0.f; //CalculateAttributeValue( GetAttributeValue('delay_between_raise_guard') );
		// W3EE - End
	...
	{
		//Kolaris - Absorption
		Combat().AbsorptionStaminaRestore(((CNewNPC)this), fixedCost);
	...
	public function HasStaminaToUseAction( action : EStaminaActionType, optional abilityName : name, optional dt :float, optional multiplier : float ) : bool
	{
		...
		if( abilityManager && abilityManager.IsInitialized() && IsAlive() )
		{
			...
			if( this == thePlayer && Options().StamCostGlobal() > 0 )
	...
	public function GetStaminaActionCost( action : EStaminaActionType, optional abilityName : name, optional dt : float, optional mult : float ) : float
	{
		var cost, delay : float;
	
		if(abilityManager && abilityManager.IsInitialized() && IsAlive())
		{
			abilityManager.GetStaminaActionCost(action, cost, delay, 0, 0, abilityName, dt, mult);
			return cost;
		}
	...
	// W3EE - Begin
	private var slowdownFactor : float;		default slowdownFactor = 0;
	private var isCrippled : bool;		default isCrippled = false;
	
	timer function RemoveCripplingInjury( dt : float, id : int )
	{
		var npcStats : SOpponentStats;
		
		npcStats = ((CNewNPC)this).GetNPCStats();
		ResetAnimationSpeedMultiplier(npcStats.spdMultID2);
		SetCrippled(false);
	}
	
	public function SetCrippled( i : bool )
	{
		isCrippled = i;
	}
	
	public function GetCrippled() : bool
	{
		return isCrippled;
	}
	
	public function SetSlowdownFactor( i : float )
	{
		slowdownFactor = i;
	}
	
	public function GetSlowdownFactor() : float
	{
	...
	{		
		...
		if( ((W3Effect_YrdenHealthDrain)action.causer) || ((W3YrdenEntityStateYrdenShock)action.causer) )
			OnYrdenHit(action.attacker);
		
		witcher = GetWitcherPlayer();
		attackAction = (W3Action_Attack)action;
		/*
		if( attackAction && attackAction.IsActionMelee() && (!attackAction.WasDodged() || (Options().EnemyDodgeNegateDamage() && Combat().DamagePercentageTaken() > 0) || !Options().EnemyDodgeNegateDamage()) )
		{
			if( witcher.HasAbility('Runeword 11 _Stats', true) && (playerAttacker || attackAction.IsParried()) )
			{
				SoundEvent('sign_igni_charge_begin');
			}
			else
			
			if( playerAttacker && witcher.HasAbility('Runeword 6 _Stats', true) && !attackAction.IsParried() )
			{
				witcher.Heal(witcher.GetStatMax(BCS_Vitality) * 0.03f);
		...
		*/
		...
		//Kolaris - Injury Effects
		if( !action.IsDoTDamage() && ((action.processedDmg.vitalityDamage > 0 && UsesVitality()) || (action.processedDmg.essenceDamage > 0 && UsesEssence())) )
			GetInjuryManager().DamagedBlindness();
		...
		if( ((action.attacker && action.attacker == thePlayer) || (CBaseGameplayEffect)action.causer) && !action.GetUnderwaterDisplayDamageHack() )
		{
			...
			// W3EE - Begin
			size = action.GetDTsNames( damageTypes );
			for(i=0; i<size; i+=1)
			{
			...
			if(!dontShowDamage)
				ShowFloatingValue(hudModuleDamageType, action.GetDamageDealt(), (hudModuleDamageType == EFVT_DoT), , damageType);
			// W3EE - End
		}
		
		//Kolaris - Vampire Lifesteal
		if( ((CActor)action.attacker).IsVampire() && !HasBuff(EET_BlackBlood) )
		{
			if( UsesEssence() )
			{
				((CActor)action.attacker).GainStat(BCS_Essence, action.processedDmg.essenceDamage * 0.02f * ((W3Effect_Bleeding)GetBuff(EET_Bleeding)).GetStacks());
				((CActor)action.attacker).ShowFloatingValue(EFVT_Heal, action.processedDmg.essenceDamage * 0.02f * ((W3Effect_Bleeding)GetBuff(EET_Bleeding)).GetStacks(), false);
			}
			else if( UsesVitality() )
			{
				((CActor)action.attacker).GainStat(BCS_Essence, action.processedDmg.vitalityDamage * 0.02f * ((W3Effect_Bleeding)GetBuff(EET_Bleeding)).GetStacks());
				((CActor)action.attacker).ShowFloatingValue(EFVT_Heal, action.processedDmg.vitalityDamage * 0.02f * ((W3Effect_Bleeding)GetBuff(EET_Bleeding)).GetStacks(), false);
			}
		}
	...
	// W3EE - Begin
	private var burnCounter : float;	default burnCounter = 0.f;
	public function GetBurnCounter() : float
	{
		//Kolaris - Purgation
		if( GetWitcherPlayer().HasAbility('Glyphword 10 _Stats', true) )
			return 0.f;
		else
			return burnCounter;
	}
	
	public function IncBurnCounter(optional by : float)
	{
		if ( by )
			burnCounter += by;
		else
			burnCounter += 1;
		//Kolaris - Burnout
		ClampF(burnCounter, 0, 5);
	}
	
	/*public function DecBurnCounter(optional by : float)
	{
		if( by )
			burnCounter -= by;
		else
	...
	//Kolaris - Axii
	private var axiiHitCounter : float;	default axiiHitCounter = 0.f;
	public function GetAxiiHitCounter() : float
	{
		return axiiHitCounter;
	}
	
	public function IncAxiiHitCounter(optional by : float)
	{
		if ( by )
			axiiHitCounter += by;
		else
			axiiHitCounter += 1;
		ClampF(axiiHitCounter, 0, 4);
	}
	
	//Kolaris - Cremation
	private var cremationCounter : float; default cremationCounter = 0.f;
	public function GetCremationCounter() : float
	{
		return cremationCounter;
	}
	
	public function IncCremationCounter()
	{
		cremationCounter += 0.01f;
	}
	
	private var damageTakenMultiplier : float; default damageTakenMultiplier = 1.f;
	public function GetDamageTakenMultiplier() : float
	{
		return damageTakenMultiplier;
	}
	
	public function SetDamageTakenMultiplier( to : float )
	{
		damageTakenMultiplier = to;
	}
	
	private var isFrozenEffectPlaying : bool; default isFrozenEffectPlaying = false;
	
	public function GetIsFrozenEffectPlaying() : bool
	{
		return isFrozenEffectPlaying;
	}
	
	public function SetIsFrozenEffectPlaying( to : bool )
	{
		isFrozenEffectPlaying = to;
	}
	
	public timer function RemoveFreezeEffects( dt : float, id : int )
	{
		StopEffect('critical_frozen');
		isFrozenEffectPlaying = false;
	}
	
	private var isRevoveringFromParalysis : bool; default isRevoveringFromParalysis = false;
	
	public function GetIsRecoveringFromParalysis() : bool
	{
		return isRevoveringFromParalysis;
	}
	
	public function SetIsRecoveringFromParalysis( to : bool )
	{
		isRevoveringFromParalysis = to;
	}
	
	public timer function ParalysisRecovery( dt : float, id : int )
	{
		var npcStats : SOpponentStats;
		
		isRevoveringFromParalysis = false;
	...
	public function IncHitCounter()
	{
		...
		totalHitCounter += 1;
		// W3EE - Begin
		AddTimer('ResetHitCounter',2.0,false,,,,true);
		// W3EE - End
	}
	...
	public function IncDefendCounter()
	{
		...
		// W3EE - Begin
		AddTimer('ResetDefendCounter',2.0,false,,,,true);
		// W3EE - End
	}
	...
	public function ReactToBeingHit(damageAction : W3DamageAction, optional buffNotApplied : bool) : bool
	{
		...
		// W3EE - Begin
		receivedAnyDamage = true; // damageAction.DealtDamage();
		// W3EE - End
		...
		{					
			...
			else
			{
				...
				//Kolaris - Puppet
				else if (HasBuff(EET_AxiiGuardMe))
				{
					playHitAnim = false;
				}
			...
			if(playHitAnim)
			{
				...
				//Kolaris - Aard Hit Reaction
				if( ( useAdditiveHits || oneTimeAdditiveHit ) && !( criticalCancelAdditiveHit && damageAction.IsCriticalHit() ) && damageAction.GetBuffSourceName() != "AardStun")
				{
					// W3EE - Begin
	...
	// W3EE - Begin
	var overrideDirection : bool;
	public function SetOverrideDirection( b : bool )	{overrideDirection = b;}
	// W3EE - End;
	...
	event OnDeath( damageAction : W3DamageAction  )
	{
		...
		// W3EE - Begin
		if( GetCrippled() )
			AddTimer('RemoveCripplingInjury', 0.1, false);
		// W3EE - End
	...
	public function CriticalStateAnimStopped(forceRemoveBuff : bool)
	{
		...
		if(buff)
		{
			...
			//Kolaris - Burn DoT
			/*if(forceRemoveBuff && (W3Effect_Burning)buff)
			{
				((W3Effect_Burning)buff).AddDuration();
			...
			else*/ if((forceRemoveBuff && !((W3Effect_Burning)buff)) || buff.GetDurationLeft() < 0 || !buff.IsActive() || CriticalBuffIsDestroyedOnInterrupt(buff))
			{
	...
	public function IsHelpless() : bool
	{
		if( HasBuff(EET_Immobilized) || 
			HasBuff(EET_Burning) || 
			HasBuff(EET_Knockdown) || 
			HasBuff(EET_HeavyKnockdown) || 
			HasBuff(EET_Blindness) || 
			HasBuff(EET_Confusion) || 
			HasBuff(EET_Paralyzed) || 
			HasBuff(EET_Hypnotized) || 
			HasBuff(EET_Stagger) || 
			HasBuff(EET_LongStagger) ||
			HasBuff(EET_Tangled) ||
			HasBuff(EET_Ragdoll) ||
			HasBuff(EET_Frozen) ||
			HasBuff(EET_Trap) ||
			HasBuff(EET_KnockdownTypeApplicator) ||
			HasBuff(EET_CounterStrikeHit) 
		  )
			return true;
	...
	// W3EE - Begin
	public function CountEffectsOfType( type : EEffectType ) : int
	{
		if( effectManager && effectManager.IsReady() )
			return effectManager.CountEffectsOfType(type);
		
		return 0;
	}
	
	public function HasDecoctionEffect() : bool
	{
		if( effectManager && effectManager.IsReady() )
			return effectManager.HasDecoctionEffect();
		
		return 0;
	}
	
	public function RemoveFirstBuffWithSource( source : string )
	{
		if( effectManager && effectManager.IsReady() )
			effectManager.RemoveFirstBuffWithSource( source );
	}
	// W3EE - End
	...
	public function IsImmuneToBuff(effect : EEffectType) : bool
	{
		...
		//Kolaris - Wild Hunt Bleed & Poison Immunities
		if ( HasAbility( 'mon_wild_hunt_default' ) && HasTag('IsBoss') )
		{
			if( effect == EET_Bleeding || effect == EET_BleedingTracking || effect == EET_Poison || effect == EET_PoisonCritical )
				return false;
		}
	...
	public final function CanBleed() : bool
	{
		...
		// W3EE - Begin
		if( ((CNewNPC)this) )
			perc = ((CNewNPC)this).GetNPCCustomStat(theGame.params.DAMAGE_NAME_BLEEDING);
		else
		...
		for(i=hitTargets.Size()-1; i>=0; i-=1)
		{
			...
			if(!countered)
			{
				...
				if( thePlayer.IsInFistFightMiniGame() || /*levelDiff < theGame.params.LEVEL_DIFF_DEADLY &&*/ (!playerTarget || playerTarget.IsActionAllowed(EIAB_Parry)) )
				{
					...
					// W3EE - End
					parried = actor.PerformPerfectParryCheck(parryInfo);
					perfectParried = parried;
					if( !parried )
	...
	protected function DoAttack(animData : CPreAttackEventData, weaponId : SItemUniqueId, parried : bool, countered : bool, parriedBy : array<CActor>, attackAnimationName : name, hitTime : float)
	{
		...
		// W3EE - Begin
		if( ((W3PlayerWitcher)this).HasBuff(EET_WinterBlade) && IsLightAttack(attackActionName) && hitTargets.Size() && !countered )
			((W3Effect_WinterBlade)((W3PlayerWitcher)this).GetBuff(EET_WinterBlade)).IncreaseCounter();
		
		GetInjuryManager().AttackStumbles();
		// W3EE - End
		
		if( GetInventory().ItemHasTag( weaponId, 'PhantomWeapon' ) )
		{	
			// W3EE - Begin
			if( this == thePlayer && IsLightAttack( attackActionName ) && hitTargets.Size() && !countered )
			// W3EE - End
			{	
	...
	protected function PrepareAttackAction( hitTarget : CGameplayEntity, animData : CPreAttackEventData, weaponId : SItemUniqueId, parried : bool, countered : bool, parriedBy : array<CActor>, attackAnimationName : name, hitTime : float, weaponEntity : CItemEntity, out attackAction : W3Action_Attack) : bool
	{
		var actor : CActor;
		// W3EE - Begin
		var phantomDmgInfo : SAbilityAttributeValue;
		var phantomDmg, winterDmg : W3DamageAction;
		var npc : CNewNPC;
		var ent, fx : CEntity;
		...
		//Kolaris - Test
		//theGame.witcherLog.AddCombatMessage(theGame.witcherLog.COLOR_GOLD_BEGIN + NameToString(attackAnimationName) + theGame.witcherLog.COLOR_GOLD_END, GetWitcherPlayer(), GetWitcherPlayer());
		//theGame.witcherLog.AddCombatMessage(theGame.witcherLog.COLOR_GOLD_BEGIN + "Current Stamina: " + theGame.witcherLog.COLOR_GOLD_END + GetStat(BCS_Stamina), GetWitcherPlayer(), GetWitcherPlayer());
		
		if(actor && parriedBy.Contains(actor))
		{
			if(perfectParried)
			{
				attackAction.SetIsPerfectParried(true);
		...
		//Kolaris - Reflect On Being Parried
		...
		// W3EE - Begin
		((W3Effect_WinterBlade)((W3PlayerWitcher)this).GetBuff(EET_WinterBlade)).DealDischargeDamage(attackAction);
		// W3EE - End
		...
		if(phantomStrike)
		{
			...
			if( attackAction.attacker == thePlayer )
			{
				...
				phantomDmg = new W3DamageAction in theGame.damageMgr;
				phantomDmg.Initialize(attackAction.attacker, attackAction.victim, attackAction.causer, attackAction.GetBuffSourceName(), EHRT_None, CPS_Undefined, attackAction.IsActionMelee(), attackAction.IsActionRanged(), attackAction.IsActionWitcherSign(), attackAction.IsActionEnvironment());
				if( npc && npc.IsShielded( thePlayer ) )
				{
					npc.ProcessShieldDestruction();
					phantomDmg.AddEffectInfo(EET_LongStagger);
				}
				else actor.AddEffectDefault(EET_HeavyKnockdown, attackAction.attacker);
				phantomDmgInfo = GetWitcherPlayer().GetInventory().GetItemAttributeValue(GetWitcherPlayer().GetHeldSword(), 'SlashingDamage');
				phantomDmg.SetForceExplosionDismemberment();
				phantomDmg.SetCannotReturnDamage(true);
				phantomDmg.SetCanPlayHitParticle(false);
				phantomDmg.AddDamage(theGame.params.DAMAGE_NAME_FIRE, (phantomDmgInfo.valueBase * (1.f + phantomDmgInfo.valueMultiplicative) + phantomDmgInfo.valueAdditive));
				phantomDmg.SetHitAnimationPlayType(EAHA_ForceNo);
				
				theGame.damageMgr.ProcessAction(phantomDmg);
				delete phantomDmg;
				...
				npc.AddTimer('Runeword1DisableFireFX', 6);	
				npc.PlayEffect('critical_burning');
				npc.PlayEffect('critical_burning_csx');
				
				surface = theGame.GetSurfacePostFX();
				surface.AddSurfacePostFXGroup(npc.GetWorldPosition(), 1.f, 30, 3, 9, 1);
	...
	// W3EE - Begin
	private var CBActive : bool;
	private var CBCharge : int;
	
	public function IsColdBloodActive() : bool
	{
		return CBActive;
	}
	
	public function ColdBloodCharge() : int
	{
		return CBCharge;
	}
		
	public function SetColdBloodActive( i : bool )
	{
		CBActive = i;
	}
	
	public function IncColdBloodCharge()
	{
		CBCharge += 1;
	}
	
	timer function ResetCB( dt : float, id : int )
	{
		SetColdBloodActive(false);
		CBCharge = 0;
	}
	// W3EE - End
	...
	public function ReduceDamage( out damageData : W3DamageAction )
	{
		...
		// W3EE - Begin
		var crushThroughBlocks 		: SAbilityAttributeValue;
		// W3EE - End
		...
		*/
		...
		if( thisNPC && damageData.attacker == thePlayer && HasBuff(EET_AxiiGuardMe) && thePlayer.HasAbility('Glyphword 30 _Stats', true) &&
		...
		*/
		...
		Combat().EnemyDodge(damageData, this);
		//W3EE - End
		...
		else
		{
			...
			if( attackAction && damageData.IsActionMelee() && attackAction.CanBeParried() && (attackAction.IsParried() || attackAction.IsCountered()) )
			{
				...
				//Kolaris - Bear Set, Kolaris - Sundering Strikes
				/*if( ((W3PlayerWitcher)actorAttacker).IsSetBonusActive(EISB_Bear_2) && ((W3PlayerWitcher)actorAttacker).IsHeavyAttack(attackAction.GetAttackName()) && Combat().GetWolvenEffect().GetStacks() >= 10 )
					crushThroughBlocks.valueMultiplicative += 1.f;*/
				crushThroughBlocks.valueMultiplicative += ((W3PlayerWitcher)actorAttacker).GetSkillLevel(S_Sword_s06) * 0.05f;
		...
		*/
		...
		if( HasAbility('HealthBuff') && HasAbility('mh207_wraith_boss') )
		{
			damageData.SetAllProcessedDamageAs(0);
		...
		if( HasAbility( 'ShadowFormActive' ) )
		{
			...
			if( !((CR4Player)actorAttacker).HasBuff(EET_Decoction6) )
			{
		...
		if(damageData.victim != thePlayer)
		{
			...
			if(!damageData.GetIgnoreImmortalityMode())
			{
				...
				if( IsInvulnerable() && !HasAbility('mon_gargoyle') )
				{
					if ( canLog )
					{
	...
	// W3EE - Begin
	private final function CacheHudModuleDoTDamageFloater(dmg : float, optional damageType : name)
	{
		cachedDoTDamage += dmg;
	...
	var cachedDamageType : name;
	
	timer function HudModuleDoTUpdate(dt : float, id : int)
	{
		if(cachedDoTDamage >= 1.0f)
		{
			ShowFloatingValue(EFVT_DoT, cachedDoTDamage, false, , cachedDamageType);
			cachedDoTDamage = 0.f;
		}	
	...
	public function ShowFloatingValue(type : EFloatingValueType, value : float, cache : bool, optional stringParam : string, optional damageType : name, optional statusType : EEffectType)
	{
		var hud : CR4ScriptedHud;
		...
		{
			CacheHudModuleDoTDamageFloater(value, damageType);
		}
		else if(thePlayer.GetTarget() == this)
		{
			...
			if(hud)
			{
				...
				if(module)
				{
					module.ShowDamageType(type, value, stringParam, , damageType, statusType);
				}
			}
		}
	}
	...
	{
		if( (CNewNPC)this )
			return ((CNewNPC)this).IsHuge();
		else
			return HasAbility( 'mon_type_huge' );
	}

	public function ApplyBleeding( numberStacks : int, causer : CGameplayEntity, sourceName : string, optional forceEffect : bool )
	{
		var bleedingEffect : W3Effect_Bleeding;
		var bleedingParams : SCustomEffectParams;
		var effectValue : SAbilityAttributeValue;
		var resistPts, resistPerc : float;
		
		if( GetDisplayName() == "Tree's Heart" )
			return;
		
		bleedingEffect = (W3Effect_Bleeding)GetBuff(EET_Bleeding);
		
		effectValue.valueMultiplicative = Max(1, numberStacks);
		//Kolaris - Exsanguination
		if( ((W3PlayerWitcher)causer).HasAbility('Runeword 19 _Stats', true) || ((W3PlayerWitcher)causer).HasAbility('Runeword 20 _Stats', true) || ((W3PlayerWitcher)causer).HasAbility('Runeword 21 _Stats', true) )
		{
			if( UsesEssence() && RandF() < GetStatPercents(BCS_Essence) )
				effectValue.valueMultiplicative += 1;
			else if( UsesVitality() && RandF() < GetStatPercents(BCS_Vitality) )
				effectValue.valueMultiplicative += 1;
		}
		
		bleedingParams.effectType = EET_Bleeding;
		bleedingParams.creator = causer;
		bleedingParams.sourceName = sourceName;
		bleedingParams.duration = -1;
		bleedingParams.effectValue = effectValue;
		bleedingParams.customAbilityName = '';
		bleedingParams.customFXName = '';
		bleedingParams.isSignEffect = false;
		
		if( (CR4Player)this )
		{
			GetResistValue(CDS_BleedingRes, resistPts, resistPerc);
			if( forceEffect || RandRangeF(1.f) > resistPerc )
			{
				if( !bleedingEffect )
					AddEffectCustom(bleedingParams);
				else
					bleedingEffect.IncreaseStacks((int)effectValue.valueMultiplicative);
			}
		}
		else
		{
			if( forceEffect || RandRangeF(1.f) > ((CNewNPC)this).GetNPCCustomStat(theGame.params.DAMAGE_NAME_BLEEDING) )
			{
				if( !bleedingEffect )
					AddEffectCustom(bleedingParams);
				else
				{
					bleedingEffect.IncreaseStacks((int)effectValue.valueMultiplicative);
					ShowFloatingValue(EFVT_Buff, 0, false,, theGame.params.DAMAGE_NAME_DIRECT, EET_Bleeding);
				}
				//Kolaris - Wolf Set
				if( (W3PlayerWitcher)causer )
				{
					if( ((W3PlayerWitcher)causer).IsSetBonusActive(EISB_Wolf_1) )
						((W3PlayerWitcher)causer).GetAdrenalineEffect().AddAdrenalineWithMult(((int)effectValue.valueMultiplicative) * 0.25f * ((W3PlayerWitcher)causer).GetSetPartsEquipped(EIST_Wolf));
				}
			}
		}
	}
	
	public function ApplyPoisoning( numberStacks : int, causer : CGameplayEntity, sourceName : string, optional forceEffect : bool )
	{
		var poisonEffect : W3Effect_Poison;
		var poisonParams : SCustomEffectParams;
		var effectValue : SAbilityAttributeValue;
		var resistPts, resistPerc : float;
		
		poisonEffect = (W3Effect_Poison)GetBuff(EET_Poison);
		
		effectValue.valueMultiplicative = Max(1, numberStacks);
		
		poisonParams.effectType = EET_Poison;
		poisonParams.creator = causer;
		poisonParams.sourceName = sourceName;
		poisonParams.duration = -1;
		poisonParams.effectValue = effectValue;
		poisonParams.customAbilityName = '';
		poisonParams.customFXName = '';
		poisonParams.isSignEffect = false;
		
		if( (CR4Player)this )
		{
			GetResistValue(CDS_PoisonRes, resistPts, resistPerc);
			if( forceEffect || RandRangeF(1.f) > resistPerc )
			{
				if( !poisonEffect )
					AddEffectCustom(poisonParams);
				else
					poisonEffect.IncreaseStacks((int)effectValue.valueMultiplicative);
			}
		}
		else
		{
			if( forceEffect || RandRangeF(1.f) > ((CNewNPC)this).GetNPCCustomStat(theGame.params.DAMAGE_NAME_POISON) )
			{
				if( !poisonEffect )
					AddEffectCustom(poisonParams);
				else
				{
					poisonEffect.IncreaseStacks((int)effectValue.valueMultiplicative);
					ShowFloatingValue(EFVT_Buff, 0, false,, theGame.params.DAMAGE_NAME_POISON, EET_Poison);
				}
				
				//Kolaris - Insectoid Decoction
				if( ((W3PlayerWitcher)causer).HasBuff(EET_Decoction10) )
					((CR4Player)causer).DrainToxicity(1);
			}
		}
	}
	
	private var isImmobilized : bool;	default isImmobilized = false;
	public function SetIsImmobilized( b : bool )
	{
		isImmobilized = b;
	}
	
	public function IsImmobilized() : bool
	{
		return isImmobilized;
	}
	
	private var isQuest : bool;	default isQuest = false;
	timer function SetIsQuest( dt : float, id : int )
	{
		//Kolaris - Quest NPC Tags
		isQuest = 
		( GetAttitudeGroup() == 'mq1005_scavengers_hostile'
		|| GetAttitudeGroup() == 'mq3005_noble'
		|| GetAttitudeGroup() == 'mq3031_husband'
		|| HasTag('poi_bar_a_12_knight')
		|| HasTag('poi_gor_a_09_knight')
		|| HasTag('poi_gor_d_07_knight')
		|| HasAbility('_canBeFollower') 
		|| HasAbility('_combatFollowerHardcoreV') 
		|| HasAbility('_combatFollowerHardcoreE') 
		|| HasAbility('_combatFollowerHardV') 
		|| HasAbility('_combatFollowerHardE') 
		|| ((CNewNPC)this).GetNPCType() == ENGT_Quest) && (CNewNPC)this;
	}
	
	public function IsQuestActor() : bool
	{
		return isQuest && !HasAbility('mon_q703_arena_knights') && !HasAbility('SkillBoss') && !HasAbility('SkillShieldHard');
	}
	
	private var yrdenImmobilizeCooldown : float;	default yrdenImmobilizeCooldown = 0.f;
	public function SetYrdenCooldown( cooldown : float )
	{
		yrdenImmobilizeCooldown = cooldown;
	}
	
	public function GetYrdenCooldown() : float
	{
		return yrdenImmobilizeCooldown;
	}
	// W3EE - End
	...
	//Kolaris - Hit Fx
	public function CreateFXEntityAndPlayEffect( entityName : name, effectName : name )
	{
		var fx : CEntity;
		
		fx = CreateFXEntityAtPelvis( entityName, true);
		fx.PlayEffect(effectName);
	}
	...
	timer function Runeword1DisableFireFX(dt : float, id : int)
	{
		...
		if(IsEffectActive('critical_burning') && !HasBuff(EET_Burning))
		{
			...
			StopEffect('critical_burning_csx');
		}
	}
	
	// W3EE - Begin
	//Kolaris - Poison Fx
	timer function DisablePoisonFX( dt : float, id : int )
	{
		var poisonEffect : W3Effect_Poison;
		
		poisonEffect = (W3Effect_Poison)GetBuff(EET_Poison);
		
		if( IsEffectActive('critical_poison') && poisonEffect.GetStacks() < 5 )
		{
			StopEffect('critical_poison');
		}
	}
	
	private var wasPoiseBroken : bool;
	public function SetWasPoiseBroken( b : bool )
	{
		wasPoiseBroken = b;
	}
	
	public function GetWasPoiseBroken() : bool
	{
		return wasPoiseBroken;
	}
	
	timer function NotifyStaggerEndTimed( dt : float, id : int )
	{
		NotifyStaggerEnd();
	}
	
	public function NotifyStaggerEnd()
	{
		var playerPoise : W3Effect_Poise;
		var npcPoise : W3Effect_NPCPoise;
		
		wasPoiseBroken = false;
		playerPoise = (W3Effect_Poise)GetBuff(EET_Poise);
		npcPoise = (W3Effect_NPCPoise)GetBuff(EET_NPCPoise);
		if( playerPoise.IsPoiseBroken(true) )
			playerPoise.SetPoiseBreakTimer(0);
			
		if( npcPoise.IsPoiseBroken() )
			npcPoise.SetPoiseBreakTimer(0);
	}
	// W3EE - End
