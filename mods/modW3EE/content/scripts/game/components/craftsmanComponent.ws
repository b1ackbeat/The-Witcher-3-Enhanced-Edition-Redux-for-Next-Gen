	...
	public function CalculateCostOfCrafting( craftedItemName : name ) : int
	{
		...
		if ( owner )
		{
			if ( owner.invComp )
			{
				...
				{
					// W3EE - Begin
					craftingCost = owner.invComp.GetItemBasePriceCrafting( owner.invComp.GetItem(items[0]), owner.invComp );
					
					if( owner.invComp.IsItemCrossbow(items[0]) )
						owner.invComp.RemoveItemByName('Bodkin Bolt', 60);
					// W3EE - End
