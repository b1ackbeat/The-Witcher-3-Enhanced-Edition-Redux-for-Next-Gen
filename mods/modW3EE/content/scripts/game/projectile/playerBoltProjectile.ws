...
class W3BoltProjectile extends W3ArrowProjectile
{
	...
	private saved var damageValues			: array<SRawDamage>;
	private saved var boltBuffs				: array<SEffectInfo>;
	
		default dodgeable = true;
	
	
	private function GetDamageValues()
	{
		var inv : CInventoryComponent;
		var dmgTypes : array<name>;
		var i : int;
		
		inv = ((CR4Player)GetOwner()).GetInventory();
		damageValues.Clear();
		inv.GetWeaponDTNames(itemId, dmgTypes);
		for(i=0; i<dmgTypes.Size(); i+=1)
			damageValues.PushBack(SRawDamage(dmgTypes[i], CalculateAttributeValue(inv.GetItemAttributeValue(itemId, dmgTypes[i])), 1.f));
	}
	
	private function CacheBoltBuffs()
	{
		var inv : CInventoryComponent;
		var i : int;
		
		inv = ((CR4Player)GetOwner()).GetInventory();
		inv.GetItemBuffs(itemId, boltBuffs);
	}
	
	public function GetBoltDamage( dmgType : name ) : float
	{
		var i : int;
		for(i=0; i<damageValues.Size(); i+=1)
		{
			if( damageValues[i].dmgType == dmgType )
				return damageValues[i].dmgVal;
		}
		
		return 0.f;
	}
	
	public function GetDTNames( out damageTypes : array<name> ) : int
	{
		var i : int;
		
		damageTypes.Clear();
		for(i=0; i<damageValues.Size(); i+=1)
			damageTypes.PushBack(damageValues[i].dmgType);
			
		return i;
	}
	
	public function GetBoltBuffs( out buffs : array<SEffectInfo> ) : int
	{
		var i : int;
		
		buffs.Clear();
		for(i=0; i<boltBuffs.Size(); i+=1)
			buffs.PushBack(boltBuffs[i]);
