...
abstract class W3AdvancedProjectile extends CThrowable
{
	...
	//W3EE - Begin
	default projSpeed = 20.f;
	default projAngle = 1.f;
	//W3EE - End
	...
	final function ShootProjectileAtPosition( angle : float, velocity : float, target : Vector, optional range : float, optional collisionGroups : array<name> )
	{
		...
		/*
		if (((CActor)caster).GetTarget() == witcher && (thePlayer.IsActionAllowed(EIAB_Dodge) || thePlayer.IsActionAllowed(EIAB_Roll)) && witcher.GetStat(BCS_Toxicity, false) > theGame.params.TOXICITY_DAMAGE_THRESHOLD && witcher.CanUseSkill(S_Alchemy_s16))
			witcher.StartFrenzy();
		*/
		// W3EE - End
	}
	...
	event OnProjectileInit()
	{
		...
		if( (CNewNPC)caster )
		{
			if( casterNPC.GetScaledRangedDamage() )
				projDMG = casterNPC.GetScaledRangedDamage();
			else
				projDMG = casterNPC.GetScaledDamage();
		}
		// W3EE - End
	...
	event OnProjectileCollision( pos, normal : Vector, collidingComponent : CComponent, hitCollisionsGroups : array< name >, actorIndex : int, shapeIndex : int )
	{
		...
		//Kolaris - Deflection
		else if( victim == thePlayer && thePlayer.HasAbility( 'Glyphword 36 _Stats', true ) )
		{
