...
class W3ArrowProjectile extends W3AdvancedProjectile
{
	...
	event OnProjectileCollision( pos, normal : Vector, collidingComponent : CComponent, hitCollisionsGroups : array< name >, actorIndex : int, shapeIndex : int )
	{
		...
		else if (victim == thePlayer)
		{
			...
			if( shieldItem && shieldItem.ProcessShieldParry(thePlayer, parryInfo, this) )
				return true;
			// W3EE - End
			
			//Kolaris - Arrow Poise Cost, Kolaris - Heightened Senses
			playerPoise = (W3Effect_Poise)thePlayer.GetBuff(EET_Poise);
			poiseCost = 30.f * (1.f - thePlayer.GetSkillLevel(S_Sword_s10) * 0.1f);
			if( ((W3PlayerWitcher)thePlayer).IsQuenActive(false) )
				poiseCost *= 0.8f - 0.04f * thePlayer.GetSkillLevel(S_Magic_s15);
			if( thePlayer.HasBuff(EET_Overexertion) )
				poiseCost *= 2.f;
			...
			//Kolaris - Deflection
			else if( thePlayer.HasAbility( 'Glyphword 34 _Stats', true ) || thePlayer.HasAbility( 'Glyphword 35 _Stats', true ) || thePlayer.HasAbility( 'Glyphword 36 _Stats', true ) )
			{
				...
				//W3EE - Begin
				if ( thePlayer.CheckCounterSpamming( (CActor)caster ) && playerPoise.GetPoisePercentage() > 0.8f && thePlayer.GetTarget() )
				{
					//casterPos = caster.GetWorldPosition();
					casterPos = thePlayer.GetTarget().GetWorldPosition();
					casterPos.Z += 1.5;
					this.Init(thePlayer);
					this.projDMG *= 0.5f;
			...
			{			
				...
				{
					if ( thePlayer.CheckCounterSpamming( (CActor)caster ) && playerPoise.GetPoisePercentage() > 0.2f )
					{
						if( playerPoise.GetPoisePercentage() > 0.8f && thePlayer.GetTarget() ) 
						{
							casterPos = thePlayer.GetTarget().GetWorldPosition();
							casterPos.Z += 1.5;
							this.Init(thePlayer);
						...
						Combat().StaminaLoss(ESAT_Parry, 1.f - thePlayer.GetSkillLevel(S_Sword_s10) * 0.1f);
						playerPoise.ReducePoise(poiseCost * 0.5f, 1.f, 'Counter');
						bounce = true;
						return true;
					}
					else if( playerPoise.GetPoisePercentage() > 0.5f )
					{
						Combat().StaminaLoss(ESAT_Parry, 1.f - thePlayer.GetSkillLevel(S_Sword_s10) * 0.1f);
						playerPoise.ReducePoise(poiseCost, 3.f, 'Parry');
						bounce = true;
					}
				}
			}
