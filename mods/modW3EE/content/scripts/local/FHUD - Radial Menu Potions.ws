...
class CModRadialMenuPotions
{
	...
	private function ConsumePotion( potionID : SItemUniqueId )
	{
		...
		if ( GetWitcherPlayer().ToxicityLowEnoughToDrinkPotion( EES_Potion1, potionID ) && playerInventory.SingletonItemGetAmmo( potionID ) != 0 )
		{
			if( !Options().GetUseDrinkAnimation() )
				...
				GetWitcherPlayer().GetAnimManager().PerformAnimation(EES_Potion1, potionID);
	...
	private function ApplyOil( oilID : SItemUniqueId )
	{
		...
		if( (disallowOilsInCombat && thePlayer.IsInCombat() && !Options().GetUseOilAnimation()) || (oilsHaveAmmo && playerInventory.SingletonItemGetAmmo( oilID ) < 1) )
		{
			theSound.SoundEvent( "gui_global_denied" );
			return;
		}
		...
		{
			GetWitcherPlayer().ApplyOil( oilID, swordId );
